...
#Changing the start date
  ...
  if ( NR == 2 ) {
     ...
     yy = $5 + 0   #year from input file (1st argument of line 39)
     if (yy%4 == 0) m[2] = 29
     dd = $6 + 0   #day of year from input file (2nd argument from line 39)
     mm = 1
     while (dd > m[mm]) {  #loop produces day and month
        dd -= m[mm]
        mm++ }
  #Generating year, day of year and hour that are needed in the input file
     dt1 = mktime(sprintf("%s %s %s %i 00 00", yy, mm, dd, $7))  #Generates time in seconds
     ...
     dt5 = strftime("%H", dt2)  #Produces hour of day (e.g. 16 for 16:00)
        ...
        printf ("%4i%5i%5i%4i%6i%4i%4i   # Start year, day, hour\n", $1, $2, $3, $4,  dt3, dt4, dt5 ) #prints date in the input file
