...
C     * INTEGER CONSTANTS.
C
      INTEGER,PARAMETER :: NLAT=84,NMOS=8,ILG=NLAT*NMOS
      INTEGER,PARAMETER :: ICAN=4,IGND=3,ICP1=ICAN+1
      INTEGER,PARAMETER :: M_X=16,M_Y=12,M_S=40,M_R=3,M_C=5
C
...
C     * OPEN FILES FOR READING AND WRITING.
      ...
      OPEN(UNIT=50,FILE='CLASS.INI',
     1             STATUS='OLD')
      OPEN(UNIT=51,FILE='WATCLASS.BIN',STATUS='OLD',
     +           FORM='UNFORMATTED')
      OPEN(UNIT=52,FILE='Soil_3lev',STATUS='OLD')
      OPEN(UNIT=70,FILE='spl.csv')
...
C READ IN TOP OF SHED FILE TO GET WATERSHED PARAMETERS
      open(unit=20,file='watflood.shd',status='old')
...
c      do i=1, nltest
c
c      latlength=wf_al/1000./(111.136-0.5623*cos(2*(DEGLAT*PI/180.0))+
c     +0.0011*cos(4*(DEGLAT*PI/180.0)))
c	longlength=wf_al/1000./(111.4172*cos((DEGLAT*PI/180.0))-
c     +0.094*cos(3*(DEGLAT*PI/180.0))+0.0002*cos(5*(DEGLAT*PI/180.0)))
c
c      RADJGRD(i)=( (deglat-(real(wf_imax)/2.0)*latlength)
c     ++(wf_yy(i)-0.5)*latlength)*PI/180.
c      DLONGRD(i)=(deglon-(real(wf_jmax)/2.0)*longlength) 
c     ++(wf_xx(i)-0.5)*longlength
...
C     * CHECK TO SEE IF THERE IS AN WATCLASS.INI FILE

      open(unit=23,file='watclass.ini',status='old',iostat=ios)

      if(ios.ne.0)then

      wf_num_points=1

      wf_pt_dir(1)="./class_out/" 
      n_out(1)=1       
      ii_out(1)=1         

        print*,'The WATCLASS.INI file was NOT found'
        print*,'Will default output to : class_out/'
        print*,'for grid square ',1
        print*,'for land class ',1
        print*,'Run will be made for all grid squares.'
        print*
      else
...
c make sure there are no more than 5 outpoints
 ...
 10   format(A10,2I7)
      ...
      print *, 'Found WATCLASS.INI file with these output locations:'
...
c read in reservoir file
       open(unit=21,file='WATFLOOD.REL',status='old')
...
C READ IN STREAMFLOW FILES
      ...
      open(unit=22,file='WATFLOOD.STR',status='old')
      read(22,*) 
...
c otherwise depending on the hour of the first time step there might not be any data in wf_qrel, wf_qhyd
      if(mod(IHOUR,wf_ktr).eq.0.and.imin.eq.0.and.
     +wf_noresv_ctrl.gt.0) then
...
cric smooth values
         ...
         smooth=1.0
         if(smooth.lt.1.0)then
           do l=1,wf_no
             wf_qsyn(l)=max(0.0,
     +	 	 wf_qsyn(l)+smooth*(wf_qo2(wf_s(l))-wf_qsyn(l)))
             if(wf_qhyd(l).ge.0.0)then
	         wf_qhyd_avg(l)=max(0.0,
     +	       wf_qhyd_avg(l)+smooth*(wf_qhyd(l)-wf_qhyd_avg(l)))
	       else
