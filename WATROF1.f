...
c           if(j .lt. ig)ztop(i,j+1) = ztop(i,j) - delzw(i,j)then
...
c         IF (WD3 == 1) THEN
c             CALL WATDRN3 (ASAT_T0,ASAT_T1,KSAT,GRKEFF,DELT,
c     1            SUBFLWJ,BASFLWJ,
c     2            IG,NA,NTYPE,ILG,IL1,IL2,ILMOS,JLMOS,
c     3            BTC,BCAP,DCOEFF,BFCAP,BFCOEFF,BFMIN,BQMAX)
c         ELSE
             call watdrn (delzwj,bij,sand,thpor_avail,ksat,grkeff,dd,
     1            thpor,asat_t0,asat_t1,subflwj,basflwj,satfc,xslope,
     2            ilg,ig,il1,il2,delt)
c         ENDIF
