...
g95:
	@nmake $(target) g95= target= wat=$(WAT) /nologo

ifort:
	@nmake $(target) ifort= target= wat=$(WAT) /nologo

mesh: check_errors mesh.$(SUBD).exe

rte: check_errors rte.$(SUBD).exe

soil: check_errors soilInitialization.$(SUBD).exe
...
soilInitialization.$(SUBD).exe: $(SI_M)
	@for %i in ($(SI_M)) do @copy %i .\Build > nul
	@cd Build
	@nmake SI.exe $(SUBD)= SPECIAL=$(SPECIAL) /nologo
	@del $(SI_S) 2> nul
	@cd..
...
Mesh.$(SUBD).exe: tmodules tclass $(MESH_M) $(MESHRTE_M)
	...
	@cd Build
	@nmake Mesh.exe $(SUBD)= SPECIAL=$(SPECIAL) WAT=$(WAT) /nologo
...
clean: mclean cclean rclean meclean sclean
	@del mesh.*.exe rte.*.exe soilinitialization.*.exe 2> nul
	@echo Done!
