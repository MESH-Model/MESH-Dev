      SUBROUTINE grdran
     1      (iwrflag,IVEG,THLIQ,THICE,
     a       ovrflw,tovrfl,subflw,tsubfl,
     b       TBARW,FDT,TFDT,BASFLW,TBASFL,
     1                  RUNOFF,TRUNOF,QFG,WLOST,FI,EVAP,R,ZPOND,DT,
     2                  WEXCES,THLMAX,THTEST,THPOR,THLRET,THLMIN,
     3                  BI,PSISAT,ksat,THFC,DELZW,XDRAIN,ISAND,LZF,
...
C     * MAR 23/05 - R.SOULIS/D.VERSEGHY. CALCULATE ksat AND
C     *                         PSISAT AT LAYER BOUNDARIES USING 
C     *                         GEOMETRIC MEAN; SET BASEFLOW TO
C     *                         ksat IF THLIQ > THFC; ADD
...
C     *                         OCCURRING WITHIN CURRENT TIMESTEP,iwrflag).
C
      IMPLICIT NONE
C
C     * INTEGER CONSTANTS.
C
      INTEGER IVEG,iwf,IG,IGP1,IGP2,ILG,IL1,IL2,JL,
     1      I,J,K,IPTBAD,N,iwrflag 
      INTEGER NA,NTYPE
C
C     * INPUT/OUTPUT FIELDS.
C
      REAL THLIQ (ILG,IG),  TBARW (ILG,IG),  
     1     THLmin (ILG,IG),  Thpor (ILG,IG),  
...
C
      REAL THPORF (ILG,IG), THLRET(ILG,IG), 
     1     BI    (ILG,IG), PSISAT(ILG,IG),  
     2     THFC  (ILG,IG), DELZW (ILG,IG)
C  
CC     * WATDRN3
                      
      INTEGER, DIMENSION(ILG) :: ILMOS,JLMOS
      real, dimension(ilg) :: dover,didrn,
     1   tovrfl,tpond,xslope,
     2   ovrflw,dden

      REAL, DIMENSION(ilg,IG) :: ksat,
     1   ksatf,grkeff,bulkfc,satsfc,sand,thice,satfc,dodrn,
     2   bfmin,bqmax,subflw,tsubfl   
      REAL VEL_T0(ILG),NUC_DOVER(ILG),MANNING_N(ILG),
     1    ovrflwj(ilg),zplim(ilg),
     3     DAVAIL,DTOT,XLAMBDA,ktop,rk1,kl,h0,c1,c2,
     +     ztop(ilg,ig)
C      real DCOEFF,BFCAP,BFCOEFF,bcap,btc,bfc
      integer isand(ilg,ig)

C     * TEMPORARY VARIABLES.
C
      REAL THPBND,THLBND,DTHLDZ,DTHPDZ,BBND,GRSBND,PSSBND,GRK,PSI,
     1     WLIMIT,THSUBL,THLTHR,CCH,ASAT1,asat2,ASATC,SATB
C
C     * COMMON BLOCK PARAMETERS.
C
      REAL DELT,TFREZ,HCPW,HCPICE,HCPSOL,HCPOM,HCPSND,HCPCLY,SPHW,
     1     SPHICE,SPHVEG,SPHAIR,RHOW,RHOICE,TCGLAC,CLHMLT,CLHVAP,asat
C
...
c      REAL, DIMENSION(ilg,IG) :: BTC,BCAP,DCOEFF,BFCAP,BFCOEFF,
c     1  BFMIN,BQMAX,b,delzw,thliq,ksat,bulkfc,isand
C      REAL VEL_T0(ILG),NUC_DOVER(ILG),MANNING_N(ILG),
c     1    ovrflwj(ilg),zplim(ilg),
c     2    THPORJ(ILG),SATsFC(ILG),
c     3     DAVAIL,DTOT,SUBFLWJ(ILG),TSUBFL(ILG),
c     4     BASFLWJ(ILG),XLAMBDA,ktop,kl,h0,c1,c2,
c     +     ztop(ilg,ig),runoffj(ilg)
...
c     1       ISAND(I,1).GT.-4 .AND.DT(I).GT.0. .AND.
...
c      print*,iwrflag
      if(iwrflag==1)then
          CALL WATROF(THLIQ,THICE,ZPOND,TPOND,OVRFLW,TOVRFL,
     ...
     7                grkeff,
c*added to be consistent with grdran
     8                THLMAX,
     3                PSISAT,THFC,LZF,
     4                IGP1,IGP2,JL,N)
       endif
...
C
     ...
     1            THLIQ(I,J),THLMIN(I,J))                
              ksatF(I,J)=ksat(I,J)*(1.0-MAX(0.0,MIN((THPOR(I,J)-
...
C     * ADJUST ksat FOR VISCOSITY OF WATER AND PRESENCE OF ICE;
C     * ADJUST THPOR FOR PRESENCE OF ICE.
C
ccc      if(.true.)then
C    also tried watrof here
c          
ccc      endif
      ...
      if(.true.)then
             ...
             IF(DELZW(I,IG).GT.0.0001)                    THEN
                 IF(THLIQ(I,IG).GT.THFC(I,IG))               THEN
                     ...
                     FDT(I,IG+1)=ksatF(I,IG)*DT(I)*XDRAIN(I)*
     1                   SATB**CCH
                 ELSE                                                                        
                     FDT(I,IG+1)=0.0                                                           
                 ENDIF
...
cric11jun revised(hopefully corrected algebra
C                  GRSBND=ksat(I,J)**(DELZW(I,J)/(DELZW(I,J)+
C     1                DELZW(I,J+1)))*ksat(I,J+1)**(DELZW(I,J+1)/
C     2                (DELZW(I,J)+DELZW(I,J+1)))
c                  GRSBND=ksatF(I,J)*ksatF(I,J+1)*(DELZW(I,J)+
c     1                DELZW(I,J+1))/(ksatF(I,J)*DELZW(I,J+1)+
c     2                ksatF(I,J+1)*DELZW(I,J))
                GRSBND
     1            =KSAT(I,J)*DELZW(I,J)/(DELZW(I,J)+DELZW(I,J+1))
     2            +KSAT(i,j+1)*DELZW(I,J+1)/(DELZW(I,J)+DELZW(I,J+1))
c                  PSSBND=PSISAT(I,J)**(DELZW(I,J)/(DELZW(I,J)+
c     1                DELZW(I,J+1)))* (I,J+1)**(DELZW(I,J+1)/
c     2                ((DELZW(I,J)+DELZW(I,J+1)))
                PSSBND
     1            =PSISAT(I,J)*DELZW(I,J)/(DELZW(I,J)+DELZW(I,J+1))
     2            +psisat(i,j+1)*DELZW(I,J+1)/(DELZW(I,J)+DELZW(I,J+1))
