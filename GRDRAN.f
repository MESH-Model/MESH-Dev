...
C
     ...
     1     WLIMIT,THSUBL,THLTHR,THAVAIL
C RIC CHANGE AUG 2007
      REAL CC,THPOREFF,ZTOP,GRK_EFF,BFLOW_T1,ASAT_T1,ASAT_T2,THFC_T0,
     1GRKEFF
      REAL GRKSATH(ILG),    XSLOPE (ILG),     DDEN(ILG)    
C
...
C
     ...
     1            THLIQ(I,J),THLMIN(I,J))                
              GRKSATF(I,J)=GRKSAT(I,J)*1.7915E-3/(2.0319E-4+1.5883E-3*
     1            EXP(-((MAX(TBARW(I,J),0.0)**0.9)/22.0)))
              GRKSATF(I,J)=GRKSATF(I,J)*(1.0-MAX(0.0,MIN(1.0,THICE(I,J)/
     1            THPOR(I,J))))**2
...
C RIC CHANGE AUG 2007 START

c             IF(DELZW(I,IG).GT.0.0001) THEN
c                 THAVAIL=THFC(I,IG)+GRKSATF(I,IG)*DT(I)*XDRAIN(I)/
c     1               DELZW(I,IG)
c                 IF(THLIQ(I,IG).GT.THAVAIL)                     THEN
c                     FDT(I,IG+1)=GRKSATF(I,IG)*DT(I)*XDRAIN(I)
c                 ELSEIF(THLIQ(I,IG).GT.THLRET(I,IG))            THEN 
c                     FDT(I,IG+1)=GRKSATF(I,IG)*DT(I)*XDRAIN(I)*
c     1                   ((THLIQ(I,IG)/THPORF(I,IG))**(2.*BI(I,IG)+3.))
c                 ELSE                                                                        
c                     FDT(I,IG+1)=0.0                                                           
c                 ENDIF


             IF(THLIQ(I,IG)-THLMIN(I,IG).GT.0.0001)        THEN

             FDT(I,IG+1)=GRKSATF(I,IG)*DT(I)*XDRAIN(I)

             CC=2.*BI(I,IG)+3
             THPOREFF =
     1	   MAX(THLIQ(I,IG),THLMIN(I,IG),THPOR(I,IG)-THICE(I,IG))
	       ...
	       ENDDO
                ...
                GRKEFF = GRKSATH(I)*XSLOPE(I)/SQRT(1.0 + XSLOPE(I)**2)/
     1			  1000.0/(2.0*DDEN(I))/THPOREFF 
     1            *1.7915E-3/(2.0319E-4+1.5883E-3*
     1            EXP(-((MAX(TBARW(I,J),0.0)**0.9)/22.0)))
     1            *(1.0-MAX(0.0,MIN(1.0,THICE(I,J)/
     1            THPOR(I,J))))**2

	       CALL WAT_DRAIN(3,DELZW(I,IG),1.0,ZTOP,GRKFAC(I),CC,GRKEFF,
     +       MIN(1.0,THFC(I,IG)/THPOR(I,IG)),BFLOW_T1,
     +       THLIQ(I,IG)/THPOREFF,ASAT_T2)


             FDT(I,IG+1) = FDT(I,IG+1)*BFLOW_T1*XDRAIN(I)

C RIC CHANGE AUG 2007 END
...
C
     ...
     1                                                            THEN 
              ...
              ENDIF
          ENDIF   
          IF(IGRD(I).GT.0. .AND. THLIQ(I,J).LE.THLRET(I,J))    THEN
...
c                  THLTHR=THLMIN(I,J)
c RIC CHANGE AUG 2007
                THLTHR=MIN(THLMIN(I,J),THLIQ(I,J))
              ELSE
