     ...
     9                  THPOR,THLRET,THLMIN,BI,PSISAT,ksat,
...
C     * OCT 31/06 - R.SOULIS.   ADJUST ksat FOR VISCOSITY OF WATER
...
C
     ...
     1     BI    (ILG,IG), PSISAT(ILG,IG), ksat(ILG,IG), 
...
C     * COMMON BLOCK PARAMETERS.
      ...
      real, dimension(ilg) ::  ovrflw,tovrfl,subflw,tsubfl
...
C
      ...
      DO 100 I=IL1,IL2
          IF(IGRN(I).GT.0)                                         THEN
              ...
              IF(ISAND(I,J).GT.-3)                             THEN
                  ksatF(I,J)=ksat(I,J)*(1.0-MAX(0.0,MIN(1.0,
     ...
     2                        THICE(I,J)-0.00001))
                  GRKINF(I,J)=ksatF(I,J)*(THLINF(I,J)/THPORF(I,J))
     1                        **(2.*BI(I,J)+3.)
              ELSE
                  ksatF(I,J)=0.0
                  THPORF(I,J)=0.0
...
C                                                 
     ...
     1                        (-BI(I,J)),PSISAT(I,J))                    
                 GRK=MIN(ksatF(I,J)*(THLIQ(I,J)/THPORF(I,J))**
     1                     (2.*BI(I,J)+3.),ksatF(I,J))                   
...
C
     ...
     6          THPOR,THLRET,THLMIN,BI,PSISAT,ksat,
...
C
      CALL grdran      
     a      (0,IVEG,THLIQ,THICE,ovrflw,tovrfl,subflw,tsubfl,
     a            TBARW,FDUMMY,TDUMMY,BASFLW,
     1            TBASFL,RUNOFF,TRUNOF,QFG,WLOST,FI,EVAP,ZERO,ZERO,
     2            TRMDR,WEXCES,THLMAX,THTEST,THPOR,THLRET,THLMIN,
     3            BI,PSISAT,ksat,THFC,DELZW,XDRAIN,ISAND,IZERO,
