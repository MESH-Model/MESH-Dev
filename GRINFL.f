     ...
     9                  THPOR,THLRET,THLMIN,BI,PSISAT,GRKSAT,GRKTLD,
...
C
     ...
     3     GRKTLD(ILG,IG), THLRAT(ILG,IG), THFC  (ILG,IG),
...
C RIC CHANGE AUG 2007
      REAL GRKSATH(ILG),    XSLOPE (ILG),     DDEN(ILG),   GRKADJ(ILG)    
      REAL THLIQAVAIL, CC, THPOREFF, ASAT_T0, ZTOP, GRKEFF,BFLOW_T0,
     1ASAT_T1
C
...
C
      ...
      DO 100 I=IL1,IL2
          IF(IGRN(I).GT.0)                                         THEN
              THLPOT=MAX((THPOR(I,J)-THICE(I,J)),0.0)
              ...
              IF(ISAND(I,J).GT.-3)                             THEN
                  GRKSATF(I,J)=GRKSAT(I,J)*1.7915E-3/(2.0319E-4+
     1                1.5883E-3*EXP(-((MAX(TBARW(I,J),0.0)**0.9)/22.0)))
                  GRKSATF(I,J)=GRKSATF(I,J)*(1.0-MAX(0.0,MIN(1.0,
     1                THICE(I,J)/THPOR(I,J))))**2
                  GRKTLD(I,J)=GRKSATF(I,J)/2.0
                  THPORF(I,J)=MAX((THPOR(I,J)-THICE(I,J)-0.00001),
     1                THLIQ(I,J),THLMIN(I,J))                
                  THLINF(I,J)=MAX(THLIQ(I,J),THLMIN(I,J),
     1                        MIN(THLRAT(I,J)*THLPOT,THPOR(I,J)-
     2                        THICE(I,J)-0.00001))
