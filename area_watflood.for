...
!Dan Princz changed wfo_pick from integer*1 to integer
...
!Dan Princz renamed imax,imin,jmin,jmax by adding an i 
!        integer      :: iopt,itype,iymin,iymax,jxmin,jxmax,imax,imin,
!     *                  jmin,jmax,ib,it,no,ndam,ni,id,nl,mhtot,mhrd,kt,
        integer      :: iopt,itype,iymin,iymax,jxmin,jxmax,imaxi,imini,
     *                  jmini,jmaxi,ib,it,no,ndam,ni,id,nl,mhtot,mhrd,
     *                  kt,
     ...
     *                  ktri,glacier_class_number
        ...
        integer(2)   :: year,month,day,hrs,mins,secs,hsecs,hh,dday
        integer*4, parameter :: flen=9000
!Dan Princz changed al from real*4 to real*8
!        real*4       :: ver,grdn,grde,al,astep,step2,scalesnw,scaleall,
        real*4       :: ver,grdn,grde,astep,step2,scalesnw,scaleall,
...
!     rev. 9.1.55  Jun.  12/04  - NK: write new str files to strfw\newfmt folder.
	  ...
	  character(40)   :: attribute_name,attribute_units,
     *       attribute_type,application,author,char_block,name
...
!      integer*4  noresv,local,mhto,index,nrel,noresvi,mhtoi,nreli,ktr
      integer*4  noresv,local,mhto,indexx,nrel,noresvi,mhtoi,nreli,ktr
!Craig Thompson renamed index to INDEXX, because index is a
! keyword in fortran
...
!Craig Thompson renamed delta to deltai, because it conflicts
! with mesh/class
!     *                               delta, frc,frcs,
     *                               deltai, frc,frcs,
...
!Dan Princz removed variable g
!     *                                         sumr,cltr,f,f1,sto,g,sum,
     *  SUMR, CLTR, F, F1, STO, SUM
     *	                                     smc_class
...
!Dan Princz removed x2 and x3
!     *                        pet,x2,x3,intevt,evt,ssumr,v1,rad,
     *  PET, INTEVT, EVT, SSUMR, V1, RAD
...
! Dan Princz removed qe and qh
!     *                         qsn,glmelt,qe,qh,qn,qp,refrz,fmadj
     *  QSN, GLMELT, QN, QP, REFRZ, FMADJ
...
!Dan Princz removed variable disp
!     * wISOdelta,tt,vel,disp,coeff
     *  WISODELTA, TT, VEL, COEFF

! Dan Princz removed variable nn
