     ...
     3wf_xx, wf_yy, wf_imax, wf_jmax, wfo_pick,M_Y, M_X, NLAT, NMOS,
     4PREGRD, DELT, TAGRD, TFREZ, FSDOWN, FDLGRD, UVGRD,
     5PRESGRD, QAGRD, ROFBROW, ROFSROW, ROFOROW, ROFROW, FAREROW,
     6QFSROW, QEVPROW, HFSROW, SBC, GTROW, ALVSROW,
     7ALIRROW, FC, FG, FCS, FGS, GZEROC, GZEROG, 
     8GZROCS, GZROGS, SNOROW, ZSNOW, FSNOW, TSNOROW,
     9TBARROW, THLQROW, THICROW, RHOW, DELZ,ILMOS,JLMOS, ILG)
C
c Saul Jan 28, 2008
...
98010 write (*,'(A)') ' '
     ...
     *           wfo_pick,wf_landclassname,NMOS)
...
!     WRITE DATA
  ...
  101 format(':FileType ',(A),' ',(A),' EnSim 1.0')
      ...
      call wfo_write_header_record('#')
      call wfo_write_header_record(
     *	'# DataType       Binary Watflow Output')
      call wfo_write_header_record('#')
...
! for binary output
      write(57,*,iostat=ios) lline(1:llen)
...
!     WRITE THE RECORD
      ...
      write(57, iostat=ios) seq,step,yy,mm,dd,hh,mi,ss,ms
	if(ios.ne.0)then
	  print*,' Error writing to unit 57'
...
!***********************************************************************
      INTEGER FUNCTION wfo_write_attribute_data(ny,nx,arr)
!***********************************************************************
!    REV. 9.00   Mar.  2000  -  TS: CONVERTED TO FORTRAN 90
...
!     Changes necessary to compile in g95
      CHARACTER :: ensimflg
