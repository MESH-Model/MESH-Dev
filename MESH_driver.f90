...
REAL, DIMENSION(:), ALLOCATABLE :: TPNDGAT, ZPNDGAT, TBASGAT, &
  ALBSGAT, TSNOGAT, RHOSGAT, SNOGAT, TCANGAT, rncangat, sncangat, &
...
REAL, DIMENSION(:), ALLOCATABLE :: PREACC, GTACC, QEVPACC, &
  ...
  TCANACC, rncanACC, sncanACC, GROACC, CANARE, SNOARE, ZPNDACC
...
REAL :: TOTAL_STORE, TOTAL_STORE_2, TOTAL_rncan, TOTAL_sncan, TOTAL_SNO, TOTAL_WSNO, TOTAL_ZPND
...
ALLOCATE (PREACC(NA), GTACC(NA), QEVPACC(NA), &
  ...
  TCANACC(NA), rncanACC(NA), sncanACC(NA), GROACC(NA), CANARE(NA), &
  SNOARE(NA), &
...
!> we give the same value for all the GRU that are in one cell
    ...
    cp%rncanROW(I,M) =  cp%rncanROW(1,M)
    cp%sncanROW(I,M) =  cp%sncanROW(1,M)
    ...
    if (MODELINFOOUTFLAG > 0) then
        ...
        do m = 1, NMTEST
            ...
            write(58, '(2F10.4,F10.2,F10.3,F10.4,F10.3,F10.3)') &
                cp%rncanROW(i, m), cp%sncanROW(i, m), cp%SNOROW(i, m), cp%ALBSROW(i, m), cp%RHOSROW(i, m), cp%GROROW(i, m)
            write(58, *)
        end do !m = 1, NMTEST
...
! write the next file
  WRITE(150+i*10+2,"('IDAY,IYEAR,"// TRIM(FMT)//&
   "TCN,rncanACC(I),sncanACC(I),TSN,ZSN')")
...
! write the next file
  WRITE(150+i*10+5,"('IHOUR,IMIN,IDAY,IYEAR,"//TRIM(FMT)// &
   "TCN,cp%rncanROW(I M),sncanROW(I M),TSN,ZSN')")
...
!> MAM - FOR FORCING DATA INTERPOLATION
   ...
   TCANGAT, rncangat, sncangat, GROGAT, FRZCGAT, CMAIGAT, &
   ...
   cp%TCANROW, cp%rncanROW, cp%sncanROW, cp%GROROW, CMAIROW, &
...
!END: PDMROF
             ...
             cp%rncanROW,cp%sncanROW,cp%GROROW, CMAIROW,cp%FCANROW, &
...
CALL CLASSS (cp%TBARROW,cp%THLQROW,cp%THICROW,GFLXROW,TSFSROW, &
             ...
             cp%RHOSROW,cp%SNOROW,cp%TCANROW,cp%rncanROW,cp%sncanROW, &
             ...
             RHOSGAT,SNOGAT,TCANGAT,rncangat,sncangat, &
...
TOTAL_THIC = 0.0
    ...
    if (BASINBALANCEOUTFLAG > 0) then
        ...
        wrt_900_1 = "DAY,YEAR,PREACC" // ",EVAPACC,ROFACC,ROFOACC," // &
            "ROFSACC,ROFBACC,PRE,EVAP,ROF,ROFO,ROFS,ROFB,sncan,rncan,SNO,WSNO,ZPND,"
        ...
        IF (FRAC(I) >= 0.0) THEN
            DO M = 1, NMTEST
                INIT_STORE = INIT_STORE + cp%FAREROW(I, M)* &
                    (cp%rncanROW(I, M) + cp%sncanROW(I, M) + cp%SNOROW(I, M) + WSNOROW(I, M) + cp%ZPNDROW(I, M)*RHOW)
...
!> Initialization of the Storage field
 IF(JAN==1) THEN
    DO M = 1, NMTEST
        ...
        STG_I(:) =  STG_I(:) + cp%FAREROW(:,M)*(cp%rncanROW(:,M) + &
                                                cp%sncanROW(:,M) + &
                                                cp%SNOROW(:,M)  + &
                                                cp%ZPNDROW(:,M)*RHOW)
        DO J = 1, IGND
...
CALL CLASSZ (0       , CTVSTP  , CTSSTP  , CT1STP, CT2STP, CT3STP  , &
             ...
             QFCGAT  , ROFGAT  , WTRGGAT , CMAIGAT,rncangat,sncangat , &
...
CALL CLASSA    (FC,     FG,     FCS,    FGS,    ALVSCN, ALIRCN, &
                ...
                THLQGAT,THICGAT,TBARGAT,rncangat,sncangat,TCANGAT, &
...
!
    ...
    CALL CLASSW  (THLQGAT,THICGAT,TBARGAT,TCANGAT,rncangat,sncangat, &
...
DO K=1, WF_NUM_POINTS
  IF(ilmos(I)==op%N_OUT(K).AND.jlmos(i)==op%II_OUT(k)) THEN
    ...
    WRITE(150+k*10+10,'((I2,","),(I3,","),(I5,","),(I6,","),11(F14.6,",")' &
                   ...
                   sncangat(I),rncangat(I),SNOgat(I),WSNOgat(I), &
...
!    TSFGRD(ilmos(I))=TSFGRD(ilmos(I))+TSFgat(I)*FAREGAT(i)
    ...
    wb_h%rncan(ilmos(I)) = wb_h%rncan(ilmos(I)) + FAREGAT(i)*rncangat(i)
    wb_h%sncan(ilmos(I)) = wb_h%sncan(ilmos(I)) + FAREGAT(i)*sncangat(i)
    ...
    enddo
    wb_h%stg(ilmos(I)) = wb%rncan(ilmos(I)) + wb%sncan(ilmos(I)) + wb%pndw(ilmos(I)) + wb%sno(ilmos(I)) + wb%wsno(ilmos(I)) + &
...
DO I = 1, nml
   IF(FRAC(ilmos(I)) /= 0.0)THEN
         ...
         rncanACC(ilmos(I)) = rncanACC(ilmos(I))+rncangat(I)*FAREGAT(i)
         sncanACC(ilmos(I)) = sncanACC(ilmos(I))+sncangat(I)*FAREGAT(i)
...
IF(NCOUNT==48) THEN !48 is the last half-hour period of the day
  ...
  IF(FRAC(I) /= 0.0)THEN
    ...
    rncanACC(I)=rncanACC(I)/REAL(NSUM)
    sncanACC(I)=sncanACC(I)/REAL(NSUM)
...
!> update components for final energy balance tally
  ...
  ENDIF
    ...
    DO I = 1, nml
        IF (FRAC(ilmos(I)) >= 0.0) THEN
!            DO M = 1, NMTEST
                TOTAL_sncan = TOTAL_sncan + FAREGAT(i)*sncangat(I)
                TOTAL_rncan = TOTAL_rncan + FAREGAT(i)*rncangat(I)
                ...
                wb%rncan(ilmos(I)) = wb%rncan(ilmos(I)) + FAREGAT(i)*sncangat(i)
                wb%sncan(ilmos(I)) = wb%sncan(ilmos(I)) + FAREGAT(i)*rncangat(i)
...
!            END DO
    ...
    !> Calculate storage
    wb%stg = wb%rncan + wb%sncan + wb%pndw + wb%sno + wb%wsno + &
                sum(wb%lqws, 2) + sum(wb%frws, 2)
    wb%dstg = wb%stg - wb%dstg

    TOTAL_STORE = TOTAL_sncan + TOTAL_rncan + TOTAL_SNO + TOTAL_WSNO + TOTAL_ZPND + &
    ...
    if (BASINBALANCEOUTFLAG > 0) then
        ...
        write(900, "((i4, ','), (i5, ','), *(e14.6, ','))") &
            ...
            TOTAL_sncan/TOTAL_AREA    , &   !15
            TOTAL_rncan/TOTAL_AREA    , &   !16
        ...
        TOTAL_sncan_M = TOTAL_sncan_M + TOTAL_sncan
        TOTAL_rncan_M = TOTAL_rncan_M + TOTAL_rncan
        ...
        if (imonth_now /= imonth_old) then
            ...
            write(902, "((i4, ','), (i5, ','), *(e14.6, ','))") &
                ...
                TOTAL_sncan_M/TOTAL_AREA, &
                TOTAL_rncan_M/TOTAL_AREA, &
...
!>  Added by Gonzalo Sapriza
    ...
    enddo
    DSTG = DSTG + rncanACC + sncanACC + SNOACC - STG_I
...
!> Save the last time step
    CALL save_init_prog_variables_class( CMAIROW     , QACROW     , TACROW      , &
                                         TBASROW     , TSFSROW    , WSNOROW     , &
                                         cp%ALBSROW  , cp%GROROW  , cp%rncanROW  , &
                                         cp%RHOSROW  , cp%sncanROW , cp%SNOROW   , &
...
!        DO M = 1, NMTEST
            FINAL_STORE = FINAL_STORE + FAREGAT(i)* &
                (rncangat(I) + sncangat(I) + SNOgat(I) + WSNOgat(I) + ZPNDgat(I)*RHOW)
            DO J = 1, IGND
                FINAL_STORE = FINAL_STORE + FAREGAT(i)* &
