...
!> DAN * EVENT FILE (HOLDS FLAGS AND FILE NAMES) (EVENT/EVENT.EVT):
!> DAN
!> DAN * ALL: ARE DUMMY ARGUMENTS SET BY RDEVT FILE ITSELF
!-      CHARACTER :: DATE*14
!-      INTEGER :: NHG, NHF
...
REAL, DIMENSION(:), ALLOCATABLE :: DRNGAT, XSLPGAT, GRKFGAT, &
  WFSFGAT, WFCIGAT, ALGWGAT, ALGDGAT, ASVDGAT, ASIDGAT, AGVDGAT, &
...
REAL, DIMENSION(:), ALLOCATABLE :: CDHGAT, CDMGAT, HFSGAT, &
  ...
  WTRGGAT, DRGAT, WTABGAT, ILMOGAT, UEGAT, HBLGAT, TROFGAT, &
  TROOGAT, TROSGAT, TROBGAT
...
REAL :: X1, X2, X3, X4, G, GAS, X5, X6, CPRES, GASV, X7, CPI, X8, &
  ...
  CKARM, CPD, AS, ASX, CI, BS, BETA, FACTN, HMIN
...
!TODO: UPDATE THIS (RELEASE(4)) WITH VERSION CHANGE
WRITE (6, "(' MESH 'A, ' ('A')'/)"), TRIM (RELEASE(4)), &
...
!> CANOPY AND SOIL INFORMATION (CLASS):
!- Diane moved allocation to read_initial_inputs
!- Diane ALLOCATE (LAMXROW(NA, NTYPE, ICAN), LAMNROW(NA, NTYPE, ICAN),
!- Diane 5  CMASROW(NA, NTYPE, ICAN), ROOTROW(NA, NTYPE, ICAN),
!- Diane +  RSMNROW(NA, NTYPE, ICAN), QA50ROW(NA, NTYPE, ICAN),
!- Diane +  VPDAROW(NA, NTYPE, ICAN), VPDBROW(NA, NTYPE, ICAN),
!- Diane +  PSGAROW(NA, NTYPE, ICAN),
!- Diane +  PSGBROW(NA, NTYPE, ICAN),DRNROW(NA, NTYPE),
!- Diane +  XSLPROW(NA, NTYPE), GRKFROW(NA, NTYPE),
!- Diane +  SDEPROW(NA, NTYPE), FAREROW(NA, NTYPE),
!- Diane +  MIDROW(NA, NTYPE), SANDROW(NA, NTYPE, IGND),
!- Diane +  CLAYROW(NA, NTYPE, IGND), ORGMROW(NA, NTYPE, IGND),
!- Diane + WFCIROW(NA, NTYPE),
!- Diane + FCANROW(NA, NTYPE, ICAN+1),LNZ0ROW(NA, NTYPE, ICP1),
!- Diane + ALVCROW(NA, NTYPE, ICP1), ALICROW(NA, NTYPE, ICP1),)
...
ALLOCATE ( &
  ...
  WFSFGAT(ILG), WFCIGAT(ILG), ALGWGAT(ILG), &
...
!> note444 see read_s_moisture_txt.f for more THLQROW information
    ...
    cp%GRKFROW(I,M)=     cp%GRKFROW(1,M)
    cp%DDROW(I,M)=       cp%DDROW(1,M)
    WFSFROW(I,M)=     WFSFROW(1,M)
    cp%WFCIROW(I,M)=     cp%WFCIROW(1,M)
...
WRITE(58,'(4E8.1,I8)') cp%XSLPROW(I,M),cp%GRKFROW(I,M), &
                  cp%MANNROW(I,M),cp%WFCIROW(I,M),cp%MIDROW(I,M)
...
call resume_state( &
   ...
   GRKFGAT, WFSFGAT, WFCIGAT, ALGWGAT, ALGDGAT, &
   ...
   cp%GRKFROW, WFSFROW, cp%WFCIROW, ALGWROW, ALGDROW, &
...
CALL CLASSG (TBARGAT,THLQGAT,THICGAT,TPNDGAT,ZPNDGAT, &
             ...
             DRNGAT, XSLPGAT,GRKFGAT,WFSFGAT,WFCIGAT, &
             ...
             cp%DRNROW, cp%XSLPROW,cp%GRKFROW,WFSFROW,cp%WFCIROW, &
...
CALL CLASSA    (FC,     FG,     FCS,    FGS,    ALVSCN, ALIRCN, &
                ...
                JLAT,   ICAN,   ICAN+1, IGND,   IDISP,  IZREF, &
...
CALL  CLASST     (TBARC,  TBARG,  TBARCS, TBARGS, THLIQC, THLIQG, &
  ...
  TRVSCN, TRIRCN, TRVSCS, TRIRCS, RC,     RCS, &
  ...
  RADJGAT,PREGAT, HCPSGAT,TCSGAT, TSFSGAT,sl%DELZ,DLZWGAT,ZBTWGAT, &
   ISNDGAT,ITC,    ITCG,   ITG,   ILG,    1,NML,  JLAT, ICAN, &
...
!> CALCULATE AVERAGES FOR WATROUTE INTPUT FILES
...
DO I=1,NA
  DO M=1,NMTEST
    ...
    DO J=1,IGND
      TBARACC(I,J)=TBARACC(I,J)+cp%TBARROW(I,M,J)*cp%FAREROW(I,M)
