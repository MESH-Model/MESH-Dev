...
!    Apr 2010 - Daniel Deacu 
!      - introduced the modification made by Vincent Fortin/Isabelle Dore in
!        a previous version; EOF -> ios in function CountDataLinesAfterHeader 
!      - inserted a write statement in ParseFlowColumnMetaData to prevent an 
!        unexplained crashing (to be investigated)
!
	MODULE EF_Module
...
c       enddo
	...
	else if(keyword(1:KeyLen) .eq. ':columnType')then
	   wordCount = CountWords(tmpString)
	   if(wordCount.le. 0)then
	      ParseTB0ColumnMetaData = -1
	      return
...
c let's look to the parent blocks
	...
	write(value,*) ''  ! silly, but useful; comment it out to  
                           ! see what happens (Daniel Deacu)
...
C
	...
	1	   (epHeader%attList(n)%val(index+xi),xi=1,xCount)
