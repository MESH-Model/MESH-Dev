...
C  Modifications:
C  
C  May 4, 2010: All of the releases available in the reservoir release file 
C               are read in if any of the Coeff1 values is zero (D. Deacu)
C
C               Changed ktr to 1 to set the start release value qrel(k,1) 
C               to a small positive value if found negative (V. Fortin)
C
C*****************************************************************************
...
!     print*,'inbsnflg allocated for ',no,' + ',noresv
...
!           DEALLOCATION OF ARRAYS FROM AREA5A:
     ...
     *           net_lake_inflow,net_lake_outflow,stat=iDeallocate)   
...
!         if(b1(1).eq.0.0)then
!
         if( int(minval(b1)) == 0 ) then 
!
!        Read in all of the releases from the reservoir release file 
!        if any of the Coeff1 values is zero (D. Deacu).
!
!           do j=ktr,nrel,ktr
...
!            write(*,*) 'Reservoir release data:'
!            do j=1,nrel
!               write(*,113) (qrel(k,j),k=1,noresv)
!            end do
! 113        FORMAT(7(F8.2, X))
