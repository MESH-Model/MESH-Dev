...
C     * NOV 21/06 - D.VERSEGHY. BUGFIX IN CALCULATION OF TPOND.
...
C     * INTEGER CONSTANTS.
C
      INTEGER IG,IGP1,IGP2,ILG,IL1,IL2,JL,I,NPNTS,NIT
C
      ...
      DO 300 I=IL1,IL2
          IF(ITER(I).EQ.1 .AND. ISIMP(I).NE.1)                      THEN
              ...
              IF(GRKINF(I,LZF(I)).GT.1.0E-12)                   THEN
                  ISIMP(I)=-2
              ELSE
                  TPOND(I)=(ZPOND(I)*(TPOND(I)+TFREZ)+R(I)*DTFLOW(I)*
     1                     (TR(I)+TFREZ))/(ZPOND(I)+R(I)*DTFLOW(I))-
     2                      TFREZ                    
