...
************************************************************************************************************
*     WATROF2 *** DRAFT VERSION 2.1 *** 05.MAY.2015 *** Ric Soulis *** Mateusz Tinel                       *
************************************************************************************************************
c
...
c     In WATROF2 the shape of typical recession curve is postulated to be a consequence of the variation
c     in velocity in pores of different sizes. Starting from saturation the flow continues at maximum
c     value Qmax until the largest pores stop flowing due to suction. Flow continues at near saturation
c     rate, while the saturation curvature changes from negative curvature to positive. This produces a 
c     recession curve pattern shown below. The curve can be conveniently represented 
c     by Q = min(Qmax, Seffective^Qcof). Baseflow is based on the average saturation of the element, 
c     represented by Seffective^Bcof.
      
c     In contrast, WATDRN and WATROF are based on decline in conductivity. WATROF2 computes overland flow, 
c     interflow, and baseflow. It relies on CLASSW and its various routines to compute for vertical flow.
...
***** INTERNAL SCALARS AND VECTORS *************************************************************************
      ...
      REAL*8  s1,sb1  , s2,sb2  , s3,sb3   !initial, and final interflow and baseflow soil saturation
...
!>    * DEFAULT VALUE IS 1.
      if (IWF.NE.1) then
          pause "WARNING: WATROF2 is not used, setting flag IWF to 1"
          iwf = 1
          !return
      endif
...
!          qcof    = (biij+2.0)/2.0
          qcof    = 4.0*(biij+2.0)*(biij+1.0)/(5.0*biij+6.0)
...
C----------------------------------------------------------------------C
            ...
            sb1 = ((bcof-1.0)*bmax*tb1+1.0)**(1.0/(1.0-bcof))
            sb3 = ((bcof-1.0)*bmax*tb2+1.0)**(1.0/(1.0-bcof))
            
            bsat1ij = sb1*(1.0-sfc)+sfc
            bsat1ij  = min(1.0,bsat1ij)
            bsat1ij  = max(bsat1ij, satmin)
          
            bsat3ij = sb3*(1.0-sfc)+sfc
            bsat3ij  = min(1.0,bsat1ij, bsat3ij)
            bsat3ij  = max(bsat3ij, satmin)
            
c            bflowij = bmax/(bcof+1)/(bsat1ij-bsat3ij)
c     1        *(bsat1ij**(bcof+1)-bsat3ij**(bcof+1))
c            
c            bflowij = 0.0 
...
C----------------------------------------------------------------------C
          ...
          bflowij = max(0.0,min(1.0,(bsat1ij-bsat3ij)))
...
C----------------------------------------------------------------------C
      OVRFLW(i) = OVRFLW(i) + ovrflwi
      SUBFLW(i) = SUBFLW(i) + subflwi 
      BASFLW(i) = BASFLW(i) + basflwi !+ subflwi 
      RUNOFF(i) = runoff(i) + ovrflwi+ subflwi + basflwi

      tovrfl(i) = tpond(i)      
c      trunof(i) = RUNOFF(i)*(trunof(i)-tfrez) +ovrflwi*(tpond(i)-tfrez) 
c     1            + subflwi*tsubfl(i) + basflwi*tbasfl(i)
c      trunof(i) = trunof(i)/runoff(i) + tfrez
