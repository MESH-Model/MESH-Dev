...
use MODELS, only : Nmod, bsm
USE AREA_WATFLOOD
USE EF_MODULE
USE MESH_INPUT_MODULE
USE FLAGS
IMPLICIT NONE
integer, allocatable:: shit(:)
INTRINSIC MAXLOC
...
REAL, DIMENSION(:, :), ALLOCATABLE :: SANDGAT, CLAYGAT
REAL, DIMENSION(:, :,:), ALLOCATABLE ::  TBASROW, &
  TACROW, QACROW, WSNOROW
REAL, DIMENSION(:, :), ALLOCATABLE :: CMAIROW
...
REAL, DIMENSION(:), ALLOCATABLE :: HCPSCS,HCPSGS,HCPSC,HCPSG, &
  TSNOWC,TSNOWG,RHOSC,RHOSG,ZSNOCS,ZSNOGS,ZSNOWC,ZSNOWG, &
...
REAL, DIMENSION(:,:), ALLOCATABLE :: TPNDGAT, ZPNDGAT, TBASGAT, &
  ...
  GROGAT, TACGAT, QACGAT, WSNOGAT
REAL, DIMENSION(:), ALLOCATABLE :: CMAIGAT, FRZCGAT
...
REAL, DIMENSION(:, :), ALLOCATABLE :: THPGAT, THRGAT, THMGAT, &
  ...
  PSIWGAT, DLZWGAT, ZBTWGAT
REAL, DIMENSION(:, :), ALLOCATABLE :: GFLXGAT
REAL, DIMENSION(:, :), ALLOCATABLE :: &
...
REAL, DIMENSION(:), ALLOCATABLE :: ZDMGRD, &
  ...
  TADPGRD, RHOAGRD, RPCPGRD, TRPCGRD, SPCPGRD, TSPCGRD, &
  FCLOGRD, DLONGRD, Z0ORGRD, GGEOGRD, UVGRD, XDIFFUS, &
  RPREGRD, SPREGRD, VMODGRD
REAL, DIMENSION(:,:), ALLOCATABLE :: RHSIGRD
...
REAL, DIMENSION(:), ALLOCATABLE :: ZRFMGAT, ZRFHGAT, ZDMGAT, &
  ...
  TSPCGAT, FCLOGAT, DLONGAT, Z0ORGAT, GGEOGAT, VMODGAT
REAL, DIMENSION(:,:), ALLOCATABLE :: RHSIGAT
!>
!>*******************************************************************
!>
!> LAND SURFACE DIAGNOSTIC VARIABLES:
...
REAL, DIMENSION(:,:), ALLOCATABLE :: FC, FG, FCS, FGS, ZSNOW, Sage
REAL, DIMENSION(:), ALLOCATABLE :: RBCOEF, &
  FSVF, FSVFS, ALVSCN, ALIRCN, ALVSG, &
...
!> LAND SURFACE PROGNOSTIC VARIABLES (CLASS.INI):
ALLOCATE ( &
  TBARGAT(ILG, IGND,Nmod), &
  THLQGAT(ILG, IGND,Nmod), THICGAT(ILG, IGND,Nmod), &
  SANDGAT(ILG, IGND), CLAYGAT(ILG, IGND), &
  TBASROW(NA, NTYPE,Nmod), &
  CMAIROW(NA, NTYPE), TACROW(NA, NTYPE,Nmod), &
  QACROW(NA, NTYPE,Nmod), WSNOROW(NA, NTYPE,Nmod), &
  TPNDGAT(ILG,Nmod), ZPNDGAT(ILG,Nmod), TBASGAT(ILG,Nmod), &
  ALBSGAT(ILG,Nmod), TSNOGAT(ILG,Nmod), RHOSGAT(ILG,Nmod), &
  SNOGAT(ILG,Nmod), TCANGAT(ILG,Nmod), RCANGAT(ILG,Nmod), &
  SCANGAT(ILG,Nmod), &
  GROGAT(ILG,Nmod), FRZCGAT(ILG), CMAIGAT(ILG), TACGAT(ILG,Nmod), &
  QACGAT(ILG,Nmod), WSNOGAT(ILG,Nmod), &
  TSFSROW(NA, NTYPE, 4,Nmod), &
...
ALLOCATE ( &
  DrySnowROW(NA, NTYPE,Nmod), SnowAgeROW(NA, NTYPE,Nmod), &
  DrySnowGAT(ILG,Nmod), SnowAgeGAT(ILG,Nmod), &
  TSNOdsROW(NA, NTYPE,Nmod), RHOSdsROW(NA, NTYPE,Nmod), &
  TSNOdsGAT(ILG,Nmod), RHOSdsGAT(ILG,Nmod), &
  DriftROW(NA, NTYPE,Nmod), SublROW(NA, NTYPE,Nmod), DepositionROW(NA, NTYPE,Nmod), &
  DriftGAT(ILG,Nmod), SublGAT(ILG,Nmod), DepositionGAT(ILG,Nmod), &
...
ALLOCATE ( &
  ...
  ISNDGAT(ILG, IGND), IGDRROW(NA, NTYPE), IGDRGAT(ILG),&
  fetchGAT(ILG),HtGAT(ILG), &
...
ALLOCATE ( ZDMGRD(NA), &
  ...
  SPCPGRD(NA), TSPCGRD(NA), RHSIGRD(NA,Nmod), &
...
!> OUTPUT VARIABLES:
ALLOCATE (PREACC(NA,Nmod), GTACC(NA,Nmod), QEVPACC(NA,Nmod), &
  HFSACC(NA,Nmod), ROFACC(NA,Nmod), SNOACC(NA,Nmod), ALVSACC(NA,Nmod), ALIRACC(NA,Nmod), &
  FSINACC(NA,Nmod), FLINACC(NA,Nmod), &
  TAACC(NA,Nmod), UVACC(NA,Nmod), PRESACC(NA,Nmod), QAACC(NA,Nmod), EVAPACC(NA,Nmod), &
  FLUTACC(NA,Nmod), ROFOACC(NA,Nmod), &
  ROFSACC(NA,Nmod), ROFBACC(NA,Nmod), HMFNACC(NA,Nmod), WTBLACC(NA,Nmod), ZPNDACC(NA,Nmod), &
  WSNOACC(NA,Nmod), RHOSACC(NA,Nmod), TSNOACC(NA,Nmod), &
  TCANACC(NA,Nmod), RCANACC(NA,Nmod), SCANACC(NA,Nmod), GROACC(NA,Nmod), CANARE(NA,Nmod), &
  SNOARE(NA,Nmod), &
  TBARACC(NA, IGND,Nmod), THLQACC(NA, IGND,Nmod), THICACC(NA, IGND,Nmod), &
  THALACC(NA, IGND,Nmod), STAT=PAS)
IF (PAS .NE. 0) THEN
  WRITE (6, *)
  WRITE (6, *)
...
ALLOCATE (TBARC(ILG, IGND), TBARG(ILG, IGND), &
  ...
  FC(ILG,Nmod), FG(ILG,Nmod), FCS(ILG,Nmod), &
  FGS(ILG,Nmod), RBCOEF(ILG), &
  ZSNOW(ILG,Nmod), &
  ...
  SNOCAN(ILG,Nmod), RAICNS(ILG), SNOCNS(ILG,Nmod), &
  ...
  ZPLIMC(ILG), ZPLIMG(ILG), Sage(ILG,Nmod), STAT=PAS)
IF (PAS .NE. 0) THEN
  WRITE (6, *)
  WRITE (6, *)
...
!> note333 see read_s_temperature_txt.f for more TBARROW information
      cp%TBARROW(I,M,J,1)=   cp%TBARROW(1,M,J,1)
!> note444 see read_s_moisture_txt.f for more THLQROW information
      cp%THLQROW(I,M,J,1)=   cp%THLQROW(1,M,J,1)
      cp%THICROW(I,M,J,1)=   cp%THICROW(1,M,J,1)
      do q=1,Nmod
        cp%TBARROW(I,M,J,q)=  cp%TBARROW(1,M,J,1)
        cp%THLQROW(I,M,J,q)=  cp%THLQROW(1,M,J,1)
        cp%THICROW(I,M,J,q)=  cp%THICROW(1,M,J,1)
        cp%TCANROW(I,M,q)=     cp%TCANROW(1,M,1)
        cp%TSNOROW(I,M,q)=     cp%TSNOROW(1,M,1)
        cp%TPNDROW(I,M,q)=     cp%TPNDROW(1,M,1)
        cp%ZPNDROW(I,M,q)=     cp%ZPNDROW(1,M,1)
        cp%RCANROW(I,M,q)=     cp%RCANROW(1,M,1)
        cp%SCANROW(I,M,q)=     cp%SCANROW(1,M,1)
        cp%SNOROW(I,M,q)=      cp%SNOROW(1,M,1)
        cp%ALBSROW(I,M,q)=     cp%ALBSROW(1,M,1)
        cp%RHOSROW(I,M,q)=     cp%RHOSROW(1,M,1)
        cp%GROROW(I,M,q)=      cp%GROROW(1,M,1)
      enddo
    ENDDO
...
DO I=1,NA
  DO M=1,NMTEST
    DO J=1,IGND
    do q=2,Nmod
      cp%TBARROW(I,M,J,q)=cp%TBARROW(I,M,J,1)+TFREZ
    end do
    END DO
    do J=1,IGND
        cp%TBARROW(I,M,J,1)=cp%TBARROW(I,M,J,1)+TFREZ
    end do
    do q=2,Nmod
     cp%TSNOROW(I,M,q)=cp%TSNOROW(I,M,1)+TFREZ
     cp%TCANROW(I,M,q)=cp%TCANROW(I,M,1)+TFREZ
     cp%TPNDROW(I,M,q)=cp%TPNDROW(I,M,1)+TFREZ
    end do
     cp%TSNOROW(I,M,1)=cp%TSNOROW(I,M,1)+TFREZ
     cp%TCANROW(I,M,1)=cp%TCANROW(I,M,1)+TFREZ
     cp%TPNDROW(I,M,1)=cp%TPNDROW(I,M,1)+TFREZ
    do q=1,Nmod
     TBASROW(I,M,1)=cp%TBARROW(I,M,3,1)
     WSNOROW(I,M,q)=0.
     TSFSROW(I,M,1,q)=TFREZ
     TSFSROW(I,M,2,q)=TFREZ
     TSFSROW(I,M,3,q)=cp%TBARROW(I,M,1,1)
     TSFSROW(I,M,4,q)=cp%TBARROW(I,M,1,1)
     TACROW (I,M,q)=cp%TCANROW(I,M,1)
     QACROW (I,M,q)=0.5E-2
    end do
    CMAIROW(I,M)=0.
    ...
    !todo - if we have time, change this so that soil.ini can take more than 3 layers.
      DO J=4,IGND
       do q=1,Nmod
        cp%THLQROW(I,M,J,q)=cp%THLQROW(I,M,3,1)
        cp%THICROW(I,M,J,q)=cp%THICROW(I,M,3,1)
        cp%TBARROW(I,M,J,q)=cp%TBARROW(I,M,3,1)
       end do
...
IF(FROZENSOILINFILFLAG == 1)THEN
   ...
   DO I = 5,INDEPPAR
      J = I - 4
      WRITE(58,'(a38,i2,a3,f6.2)')'OPPORTUNITY TIME FOR SIMULATION YEAR ', J, ' = ', t0_ACC(J,1)
      do q=2,Nmod
        t0_ACC(J,q) = t0_ACC(J,1)
      enddo
  ENDDO
ELSE
  DO I=1,INDEPPAR
...
WRITE(58,'(9F10.2)') (cp%TBARROW(I,M,J,1),J=1,IGND),cp%TCANROW(I,M,1), &
                  cp%TSNOROW(I,M,1),cp%TPNDROW(I,M,1)
WRITE(58,'(10F10.3)') (cp%THLQROW(I,M,J,1),J=1,IGND), &
                  (cp%THICROW(I,M,J,1),J=1,IGND),cp%ZPNDROW(I,M,1)
WRITE(58,'(2F10.4,F10.2,F10.3,F10.4,F10.3,F10.3)') &
                  cp%RCANROW(I,M,1),cp%SCANROW(I,M,1),cp%SNOROW(I,M,1), &
                  cp%ALBSROW(I,M,1),cp%RHOSROW(I,M,1),cp%GROROW(I,M,1)
WRITE(58,*)
ENDDO
    ...
    ALLOCATE(INFILTYPE(ILG,Nmod),SI(ILG,Nmod),TSI(ILG,Nmod), &
             SNOWMELTD(ILG,Nmod),SNOWMELTD_LAST(ILG,Nmod),SNOWINFIL(ILG,Nmod),&
             CUMSNOWINFILCS(ILG,Nmod),MELTRUNOFF(ILG,Nmod), CUMSNOWINFILGS(ILG,Nmod))
...
!
  IF (PRINTRUNOFFFILEFLAG == 1) THEN  !WRITE RUNOFF HEADER
...
END IF
!
  IF (PRINTLEAKAGEFILEFLAG == 1) THEN  !WRITE RECHARGE HEADER
...
IF(PBSMFLAG == 1)THEN
 DO I=1,NA  !I=2,NA
   DO M=1,NMTEST
    do j=1,Nmod
     IF(cp%SNOROW(I,M,j).LE.0.) THEN
       DrySnowROW(I,M,j) = 0. !1=snowpack is dry (i.e. cold)
       SnowAgeROW(I,M,j) = 0. !hours since last snowfall
!     ELSEIF(INTERPOLATIONFLAG == 0)THEN
       IF(TAGRD(I).GE.273.16) THEN
         DrySnowROW(I,M,j) = 0.
         SnowAgeROW(I,M,j) = 48. !assume 48 hours since last snowfall
       ELSE
         DrySnowROW(I,M,j) = 1.
         SnowAgeROW(I,M,j) = 48.
       ENDIF
!     ELSEIF(INTERPOLATIONFLAG == 1)THEN
!       IF(TAGATPRE(I).GE.273.16) THEN
!         DrySnowROW(I,M,j) = 0.
!         SnowAgeROW(I,M,j) = 48.
!       ELSE
!         DrySnowROW(I,M,j) = 1.
...
IF(JAN==1) THEN
  ...
  DO I=1,NA
     IF(FRAC(I)/=0.0)THEN
        DO M=1,NMTEST
           INIT_STORE = INIT_STORE + cp%FAREROW(I,M)*(cp%RCANROW(I,M,q)+cp%SCANROW(I,M,q)+cp%SNOROW(I,M,q)+cp%ZPNDROW(I,M,q)*RHOW)
           DO J = 1, IGND
              INIT_STORE = INIT_STORE + cp%FAREROW(I,M)*(cp%THLQROW(I,M,J,q)*RHOW+cp%THICROW(I,M,J,q)*RHOICE)*DLZWROW(I,M,J)
           ENDDO
        ENDDO
     ENDIF
  ENDDO
...
!FOR PDMROF
                  ...
                  XSNOWC,XSNOWG,XSNOCS,XSNOGS,q,NYEARS,NMELT)
...
CALL REDISTRIB_SNOW (ILG,1,NA,NTYPE,NML,TSNOGAT,ZSNOW, &
          ...
          TROOGAT,ROFOGAT,TROFGAT,ROFGAT,ROFNGAT,PCPGGAT,HTCSGAT,WSNOGAT,N,q)
ENDIF
!
!=======================================================================
!
...
DO I=1,NA
  DO M=1,NMTEST
    ...
    QSTR(I,M,q)=FSSTAR+FLSTAR
    QH(I,M,q)=HFSROW(I,M)
    QE(I,M,q)=QEVPROW(I,M)
    shart=QE(1,M,q)
    ...
    IF(cp%TCANROW(I,M,q)>0.01) THEN
      TCN=cp%TCANROW(I,M,q)-TFREZ
    ELSE
      TCN=0.0
    ENDIF

    IF(cp%TSNOROW(I,M,q)>0.01) THEN
      TSN=cp%TSNOROW(I,M,q)-TFREZ
    ELSE
      TSN=0.0
    ENDIF

    IF(cp%TPNDROW(I,M,q)>0.01) THEN
      TPN=cp%TPNDROW(I,M,q)-TFREZ
    ELSE
      TPN=0.0
    ENDIF
    ...
    ROF(I,M,q)=ROFROW(I,M)
    
    SCF(I,M,q)=(FCS(I,q)+FGS(I,q))/(FC(I,q)+FG(I,q)+FCS(I,q)+FGS(I,q))
...
!>        figure out ilmos and jlmos of grid square I
    ...
    ZPND = ZPNDPRECS(I_OUT) * FCS(I_OUT,q) + ZPONDPREC(I_OUT) * FC(I_OUT,q) + &
           ZPONDPREG(I_OUT) * FG(I_OUT,q) + ZPNDPREGS(I_OUT) * FGS(I_OUT,q)
    FSTR = FSTRCS(I_OUT)    * FCS(I_OUT,q) + FSTRC    (I_OUT) * FC(I_OUT,q) + &
           FSTRG    (I_OUT) * FG(I_OUT,q) + FSTRGS   (I_OUT) * FGS(I_OUT,q)
    
!-mm    WRITE(150+k*10+4,'((I2,","),(I3,","),(I5,","),(I6,","),'// &
!-mm                   '9(F8.2,","),2(F7.3,","),(E11.3,","),(F8.2,","),'// &
!-mm                   '3(F12.4,","))') &
!-mm                   IHOUR,IMIN,IDAY,IYEAR,FSSTAR,FLSTAR,QH, &
!-mm                   QE,SNOMLT,BEG,GTOUT,cp%SNOROW(I,M,q), &
!-mm                   cp%RHOSROW(I,M,q),WSNOROW(I,M,q),ALTOT,ROFROW(I,M), &
!-mm                   TPN,cp%ZPNDROW(I,M,q),ZPND,FSTR
!-mm    WRITE(150+k*10+5,'((I2,","),(I3,","),(I5,","),(I6,","),'// &
!-mm                   '6(F7.2,",",2(F6.3,",")),(F8.2,","),2(F8.4,","),'// &
!-mm                   '(F8.2,","),(F8.3,","))') &
!-mm                   IHOUR,IMIN,IDAY,IYEAR, &
!-mm                   (cp%TBARROW(I,M,J,q)-TFREZ,cp%THLQROW(I,M,J,q), &
!-mm                   cp%THICROW(I,M,J,q),J=1,IGND),TCN, &
!-mm                   cp%RCANROW(I,M,q),cp%SCANROW(I,M,q),TSN,ZSN
!-mm    WRITE(150+k*10+6,'((I2,",")(I3,",")(I5,",")2(F10.2,",")'// &
!-mm                   '(F12.6,",")(F10.2,",")(F8.2,",")(F10.2,",")'// &
!-mm                   '(F15.9,","))') &
!-mm                   IHOUR,IMIN,IDAY,FSDOWN(I),FDLGRD(I), &
!-mm                   PREGRD(I),TAGRD(I)-TFREZ,UVGRD(I),PRESGRD(I), &
!-mm                   QAGRD(I)
!-mm    WRITE(150+k*10+7,'(12(E11.4,","))') &
!-mm                   TROFROW(I,M),TROOROW(I,M),TROSROW(I,M), &
!-mm                   TROBROW(I,M),ROFROW(I,M),ROFOROW(I,M), &
!-mm                   ROFSROW(I,M),ROFBROW(I,M), &
!-mm                   FCS(I,q),FGS(I,q),FC(I,q),FG(I,q)
!-mm    WRITE(150+k*10+10,'((I2,","),(I3,","),(I5,","),(I6,","),'// &
!-mm                   '(F8.2,","),3(F15.9,","))') &
!-mm                   IHOUR,IMIN,IDAY,IYEAR, &
!-mm                   cp%SNOROW(I,M,q), &
!-mm                   DriftROW(I,M,q),SublROW(I,M,q),DepositionROW(I,M,q)
!-mm    FMT = '(14(F12.4,",")'//TRIM(ADJUSTL(IGND_CHAR))//'(F12.4,",")2(F12.4,",")'//TRIM(ADJUSTL(IGND_CHAR))//'(F12.4,","))'
!-mm    WRITE(150+k*10+8,TRIM(ADJUSTL(FMT))) &
!-mm                   FSGVROW(I,M),FSGSROW(I,M),FSGGROW(I,M), &
!-mm                   FLGVROW(I,M),FLGSROW(I,M),FLGGROW(I,M), &
!-mm                   HFSCROW(I,M),HFSSROW(I,M),HFSGROW(I,M), &
!-mm                   HEVCROW(I,M),HEVSROW(I,M),HEVGROW(I,M), &
!-mm                   HMFCROW(I,M),HMFNROW(I,M), &
!-mm                   (HMFGROW(I,M,J),J=1,IGND), &
!-mm                   HTCCROW(I,M),HTCSROW(I,M), &
!-mm                   (HTCROW(I,M,J),J=1,IGND)
!-mm    WRITE(150+k*10+9,'(8(E12.4,",")'//TRIM(ADJUSTL(IGND_CHAR))//'(E12.4,",")7(E12.4,","))') &
!-mm                   PCFCROW(I,M),PCLCROW(I,M),PCPNROW(I,M), &
!-mm                   PCPGROW(I,M),QFCFROW(I,M),QFCLROW(I,M), &
!-mm                   QFNROW(I,M),QFGROW(I,M),(QFCROW(I,M,J), &
!-mm                   J=1,IGND),ROFCROW(I,M),ROFNROW(I,M), &
!-mm                   ROFOROW(I,M),ROFROW(I,M),WTRCROW(I,M), &
!-mm                   WTRSROW(I,M),WTRGROW(I,M)
!-mm    WRITE(150+k*10+11,'((I2,","),(I3,","),(I5,","),(I6,","),10(F12.5,",")' &
!-mm                   //TRIM(ADJUSTL(IGND_CHAR))//'(F12.5,",")'//TRIM(ADJUSTL(IGND_CHAR))//'(F12.5,","))') &
!-mm                   IHOUR,IMIN,IDAY,IYEAR,PREGAT(I_OUT)*DELT,QFSROW(I,M)*DELT, &
!-mm                   ROFROW(I,M)*DELT,ROFOROW(I,M)*DELT,ROFSROW(I,M)*DELT,ROFBROW(I,M)*DELT, &
!-mm                   cp%SNOROW(I,M,q),cp%SCANROW(I,M,q),cp%RCANROW(I,M,q), &
!-mm                   cp%ZPNDROW(I,M,q)*RHOW,(cp%THLQROW(I,M,J,q)*RHOW*DLZWROW(I,M,J),J=1,IGND),&
!-mm                   (cp%THICROW(I,M,J,q)*RHOICE*DLZWROW(I,M,J),J=1,IGND)
...
!> -----------------------------------------------------c
!>
!mmIF(NR2CFILES > 0 .AND. MOD(NCOUNT*30,DELTR2C) == 0)THEN
!mm
!mm  CALL FIND_MONTH (IDAY, IYEAR, ensim_month)
!mm  CALL FIND_DAY (IDAY, IYEAR, ensim_day)
!mm  
!mm  CALL WRITE_R2C_DATA(NML,NLTEST,NMTEST,NCOUNT,IMIN,ACLASS,    &
!mm                      NA,XXX,YYY,XCOUNT,YCOUNT,ILMOS,JLMOS,ILG,&
!mm                      NR2C,NR2CFILES,R2CFILEUNITSTART,GRD,GAT, &
!mm                      GRDGAT,NR2CSTATES,R2C_ATTRIBUTES,FRAME_NO_NEW,IYEAR,&
!mm                      ensim_MONTH,ensim_DAY,IHOUR,IMIN,ICAN,   &
!mm                      ICAN+1,IGND,                             &
!mm                      TBARGAT,THLQGAT,THICGAT,TPNDGAT,ZPNDGAT, &
!mm                      TBASGAT,ALBSGAT,TSNOGAT,RHOSGAT,SNOGAT,  &
!mm                      TCANGAT,RCANGAT,SCANGAT,GROGAT, CMAIGAT, &
!mm                      FCANGAT,LNZ0GAT,ALVCGAT,ALICGAT,PAMXGAT, &
!mm                      PAMNGAT,CMASGAT,ROOTGAT,RSMNGAT,QA50GAT, &
!mm                      VPDAGAT,VPDBGAT,PSGAGAT,PSGBGAT,PAIDGAT, &
!mm                      HGTDGAT,ACVDGAT,ACIDGAT,TSFSGAT,WSNOGAT, &
!mm                      THPGAT, THRGAT, THMGAT, BIGAT,  PSISGAT, &
!mm                      GRKSGAT,THRAGAT,HCPSGAT,TCSGAT,          &
!mm                      THFCGAT,PSIWGAT,DLZWGAT,ZBTWGAT,         &
!mm                      ZSNLGAT,ZPLGGAT,ZPLSGAT,TACGAT, QACGAT,  &
!mm                      DRNGAT, XSLPGAT,XDGAT,WFSFGAT,KSGAT,     &
!mm                      ALGWGAT,ALGDGAT,ASVDGAT,ASIDGAT,AGVDGAT, &
!mm                      AGIDGAT,ISNDGAT,RADJGAT,ZBLDGAT,Z0ORGAT, &
!mm                      ZRFMGAT,ZRFHGAT,ZDMGAT, ZDHGAT, FSVHGAT, &
!mm                      FSIHGAT,CSZGAT, FDLGAT, ULGAT,  VLGAT,   &
!mm                      TAGAT,  QAGAT,  PRESGAT,PREGAT, PADRGAT, &
!mm                      VPDGAT, TADPGAT,RHOAGAT,RPCPGAT,TRPCGAT, &
!mm                      SPCPGAT,TSPCGAT,RHSIGAT,FCLOGAT,DLONGAT, &
!mm                      GGEOGAT,                                 &
!mm                      CDHGAT, CDMGAT, HFSGAT, TFXGAT, QEVPGAT, &
!mm                      QFSGAT, QFXGAT, PETGAT, GAGAT,  EFGAT,   &
!mm                      GTGAT,  QGGAT,  ALVSGAT,ALIRGAT, &
!mm                      SFCTGAT,SFCUGAT,SFCVGAT,SFCQGAT,FSNOGAT, &
!mm                      FSGVGAT,FSGSGAT,FSGGGAT,FLGVGAT,FLGSGAT, &
!mm                      FLGGGAT,HFSCGAT,HFSSGAT,HFSGGAT,HEVCGAT, &
!mm                      HEVSGAT,HEVGGAT,HMFCGAT,HMFNGAT,HTCCGAT, &
!mm                      HTCSGAT,PCFCGAT,PCLCGAT,PCPNGAT,PCPGGAT, &
!mm                      QFGGAT, QFNGAT, QFCLGAT,QFCFGAT,ROFGAT,  &
!mm                      ROFOGAT,ROFSGAT,ROFBGAT,TROFGAT,TROOGAT, &
!mm                      TROSGAT,TROBGAT,ROFCGAT,ROFNGAT,ROVGGAT, &
!mm                      WTRCGAT,WTRSGAT,WTRGGAT,DRGAT,  GFLXGAT, &
!mm                      HMFGGAT,HTCGAT, QFCGAT, MANNGAT,DDGAT,   &
!mm                      IGDRGAT,VMODGAT,QLWOGAT)
!mm
!mm  FRAME_NO_NEW = FRAME_NO_NEW + 1 !UPDATE COUNTERS
!mm
!mmENDIF
...
DO I=1,NA
  DO M=1,NMTEST
    ...
    SNOGRD(I)=SNOGRD(I)+cp%SNOROW(I,M,q)*cp%FAREROW(I,M)
...
! Fixed calculation of basin averages. Needs documenting and testing.
    do I=1,NML
       ...
       basin_SWE = basin_SWE + SNOGAT(I,q)*cp%FAREROW(ilmos(I),jlmos(I))
    enddo
...
DO I = 1, NA
   IF(FRAC(I) /= 0.0)THEN
      PREACC(I,q)  = PREACC(I,q) + PREGRD(I)*DELT
      DO M = 1,NMTEST
         GTACC(I,q)   = GTACC(I,q)  + GTROW(I,M)*  cp%FAREROW(I,M)
         QEVPACC(I,q) = QEVPACC(I,q)+ QEVPROW(I,M)*cp%FAREROW(I,M)
         EVAPACC(I,q) = EVAPACC(I,q)+ QFSROW(I,M)* cp%FAREROW(I,M)*DELT
         HFSACC(I,q)  = HFSACC(I,q) + HFSROW(I,M)* cp%FAREROW(I,M)
         HMFNACC(I,q) = HMFNACC(I,q)+ HMFNROW(I,M)*cp%FAREROW(I,M)
         ROFACC(I,q)  = ROFACC(I,q) + ROFROW(I,M)* cp%FAREROW(I,M)*DELT
         ROFOACC(I,q) = ROFOACC(I,q)+ ROFOROW(I,M)*cp%FAREROW(I,M)*DELT
         ROFSACC(I,q) = ROFSACC(I,q)+ ROFSROW(I,M)*cp%FAREROW(I,M)*DELT
         ROFBACC(I,q) = ROFBACC(I,q)+ ROFBROW(I,M)*cp%FAREROW(I,M)*DELT
         WTBLACC(I,q) = WTBLACC(I,q)+ WTABROW(I,M)*cp%FAREROW(I,M)
         DO J = 1, IGND
            TBARACC(I,J,q) = TBARACC(I,J,q)+cp%TBARROW(I,M,J,q)*ACLASS(I,M)
            THLQACC(I,J,q) = THLQACC(I,J,q)+cp%THLQROW(I,M,J,q)*cp%FAREROW(I,M)
         ...
         IF(cp%SNOROW(I,M,q)>0.0) THEN
            RHOSACC(I,q) = RHOSACC(I,q)+cp%RHOSROW(I,M,q)*cp%FAREROW(I,M)
            TSNOACC(I,q) = TSNOACC(I,q)+cp%TSNOROW(I,M,q)*cp%FAREROW(I,M)
            WSNOACC(I,q) = WSNOACC(I,q)+WSNOROW(I,M,q)*cp%FAREROW(I,M)
            SNOARE(I,q)  = SNOARE(I,q)+cp%FAREROW(I,M)
         ENDIF
         IF(cp%TCANROW(I,M,q) > 0.5) THEN
            TCANACC(I,q) = TCANACC(I,q)+cp%TCANROW(I,M,q)*cp%FAREROW(I,M)
            CANARE(I,q)  = CANARE(I,q)+cp%FAREROW(I,M)
         ENDIF
         SNOACC(I,q)  = SNOACC(I,q)+cp%SNOROW(I,M,q)*cp%FAREROW(I,M)
         RCANACC(I,q) = RCANACC(I,q)+cp%RCANROW(I,M,q)*cp%FAREROW(I,M)
         SCANACC(I,q) = SCANACC(I,q)+cp%SCANROW(I,M,q)*cp%FAREROW(I,M)
         GROACC(I,q)  = GROACC(I,q)+cp%GROROW(I,M,q)*cp%FAREROW(I,M)
         FSINACC(I,q) = FSINACC(I,q)+FSDOWN(I)*cp%FAREROW(I,M)
         FLINACC(I,q) = FLINACC(I,q)+FDLGRD(I)*cp%FAREROW(I,M)
         FLUTACC(I,q) = FLUTACC(I,q)+SBC*GTROW(I,M)**4*cp%FAREROW(I,M)
         TAACC(I,q)   = TAACC(I,q)+TAGRD(I)*cp%FAREROW(I,M)
         UVACC(I,q)   = UVACC(I,q)+UVGRD(I)*cp%FAREROW(I,M)
         PRESACC(I,q) = PRESACC(I,q)+PRESGRD(I)*cp%FAREROW(I,M)
         QAACC(I,q)   = QAACC(I,q)+QAGRD(I)*cp%FAREROW(I,M)
      ENDDO !DO M=1,NMTEST
   ENDIF
ENDDO !DO I=1,NA
...
IF(NCOUNT==48) THEN !48 is the last half-hour period of the day
  ...
  DO I=1,NA
  IF(FRAC(I) /= 0.0)THEN
    PREACC(I,q)=PREACC(I,q)
    GTACC(I,q)=GTACC(I,q)/REAL(NSUM)
    QEVPACC(I,q)=QEVPACC(I,q)/REAL(NSUM)
    EVAPACC(I,q)=EVAPACC(I,q)
    HFSACC(I,q)=HFSACC(I,q)/REAL(NSUM)
    HMFNACC(I,q)=HMFNACC(I,q)/REAL(NSUM)
    ROFACC(I,q)=ROFACC(I,q)
    ROFOACC(I,q)=ROFOACC(I,q)
    ROFSACC(I,q)=ROFSACC(I,q)
    ROFBACC(I,q)=ROFBACC(I,q)
    WTBLACC(I,q)=WTBLACC(I,q)/REAL(NSUM)

    DO J=1,IGND
      TBARACC(I,J,q)=TBARACC(I,J,q)/REAL(NSUM)
      THLQACC(I,J,q)=THLQACC(I,J,q)/REAL(NSUM)
      THICACC(I,J,q)=THICACC(I,J,q)/REAL(NSUM)
      THALACC(I,J,q)=THALACC(I,J,q)/REAL(NSUM)
    ENDDO

    IF(FSINACC(I,q)>0.0) THEN
      ALVSACC(I,q)=ALVSACC(I,q)/(FSINACC(I,q)*0.5)
      ALIRACC(I,q)=ALIRACC(I,q)/(FSINACC(I,q)*0.5)
    ELSE
      ALVSACC(I,q)=0.0
      ALIRACC(I,q)=0.0
    ENDIF

    IF(SNOARE(I,q)>0.0) THEN
      RHOSACC(I,q)=RHOSACC(I,q)/SNOARE(I,q)
      TSNOACC(I,q)=TSNOACC(I,q)/SNOARE(I,q)
      WSNOACC(I,q)=WSNOACC(I,q)/SNOARE(I,q)
    ENDIF

    IF(CANARE(I,q)>0.0) THEN
      TCANACC(I,q)=TCANACC(I,q)/CANARE(I,q)
    ENDIF

    SNOACC(I,q)=SNOACC(I,q)/REAL(NSUM)
    RCANACC(I,q)=RCANACC(I,q)/REAL(NSUM)
    SCANACC(I,q)=SCANACC(I,q)/REAL(NSUM)
    GROACC(I,q)=GROACC(I,q)/REAL(NSUM)
    FSINACC(I,q)=FSINACC(I,q)/REAL(NSUM)
    FLINACC(I,q)=FLINACC(I,q)/REAL(NSUM)
    FLUTACC(I,q)=FLUTACC(I,q)/REAL(NSUM)
    TAACC(I,q)=TAACC(I,q)/REAL(NSUM)
    UVACC(I,q)=UVACC(I,q)/REAL(NSUM)
    PRESACC(I,q)=PRESACC(I,q)/REAL(NSUM)
    QAACC(I,q)=QAACC(I,q)/REAL(NSUM)
!* ALTOT: the average of the visible spectrum and infrared spectrum
    ALTOT=(ALVSACC(I,q)+ALIRACC(I,q))/2.0
    FSSTAR=FSINACC(I,q)*(1.-ALTOT)
    FLSTAR=FLINACC(I,q)-FLUTACC(I,q)
!    QH(q)=HFSACC(I,q)
!    QE(q)=QEVPACC(I,q)
!    BEG(q)=FSSTAR+FLSTAR-QH(q)-QE(q)
    SNOMLT=HMFNACC(I,q)
!    IF(RHOSACC(I,q)>0.0) THEN
!      ZSN(q)=SNOACC(I,q)/RHOSACC(I,q)
!    ELSE
!      ZSN(q)=0.0
!    ENDIF

    IF(TCANACC(I,q)>0.01) THEN 
      TCN=TCANACC(I,q)-TFREZ
    ELSE
      TCN=0.0
    ENDIF

    IF(TSNOACC(I,q)>0.01) THEN 
      TSN=TSNOACC(I,q)-TFREZ
    ELSE
      TSN=0.0
    ENDIF

    IF(ILW==1) THEN
      GTOUT=GTACC(I,q)-TFREZ
    ELSE
      GTOUT=0.0
    ENDIF
...
!-mm        WRITE(150+k*10+1,'((I4,","),(I5,","),9(F8.2,","),2(F8.3,","),'// &
!-mm                       '(F12.4,","),4(F12.4,","))') &
!-mm                        IDAY,IYEAR,FSSTAR,FLSTAR,QH,QE,SNOMLT, &
!-mm                        BEG,GTOUT,SNOACC(I,q),RHOSACC(I,q), &
!-mm                        WSNOACC(I,q),ALTOT,ROFACC(I,q),ROFOACC(I,q), &
!-mm                        ROFSACC(I,q),ROFBACC(I,q)
!-mm        WRITE(150+k*10+2,'((I4,","),(I5,","),'//ADJUSTL(IGND_CHAR)//'((F8.2,","),'// &
!-mm                        '2(F6.3,",")),(F8.2,","),2(F7.4,","),'// &
!-mm                        '2(F8.2,","),(E12.5,","))') &
!-mm                        IDAY,IYEAR,(TBARACC(I,J,q)-TFREZ, &
!-mm                        THLQACC(I,J,q),THICACC(I,J,q),J=1,IGND), &
!-mm                        TCN,RCANACC(I,q),SCANACC(I,q),TSN,ZSN(q)
!-mm        WRITE(150+k*10+3,'((I4,","),(I5,","),3(F9.2,","),(F8.2,","),'// &
!-mm                        '(F10.2,","),(E12.3,","),2(F12.3,","),'// &
!-mm                        '(F8.3,","))') &
!-mm                        IDAY,IYEAR,FSINACC(I,q),FLINACC(I,q), &
!-mm                        TAACC(I,q)-TFREZ,UVACC(I,q),PRESACC(I,q), &
!-mm                        QAACC(I,q),PREACC(I,q),EVAPACC(I,q)
...
!-mm    TOTAL_QEVPACC = TOTAL_QEVPACC + QEVPACC(I,q)
!-mm
!-mm    IF (WF_NUM_POINTS > 0) THEN !SUMMARY VALUES FOR SCREEN
!-mm      DO J = 1, WF_NUM_POINTS !FOR MORE THAN 1 OUTPUT
!-mm        IF (I == op%N_OUT(J)) THEN
 !-mm         PRE_OUT(J) = PREACC(I,q)
 !-mm         EVAP_OUT(J) = EVAPACC(I,q)
!-mm          ROF_OUT(J) = ROFACC(I,q)
!-mm        END IF
!-mm      END DO
!-mm    ELSEIF (I == CEILING(REAL(NA) / 2)) THEN !GENERAL CASE
!-mm      PRE_OUT(1) = PREACC(I,q)
!-mm      EVAP_OUT(1) = EVAPACC(I,q)
...
!-mm                                                TOTAL_EVAPACC/TOTAL_AREA, &
!-mm                                                TOTAL_ROFACC/TOTAL_AREA,  &
!-mm                                                TOTAL_ROFOACC/TOTAL_AREA, &
!-mm                                                TOTAL_ROFSACC/TOTAL_AREA, &
!-mm                                                TOTAL_ROFBACC/TOTAL_AREA, &
!-mm                                                TOTAL_PRE/TOTAL_AREA,  &
!-mm                                                TOTAL_EVAP/TOTAL_AREA, &
!-mm                                                TOTAL_ROF/TOTAL_AREA,  &
!-mm                                                TOTAL_ROFO/TOTAL_AREA, &
!-mm                                                TOTAL_ROFS/TOTAL_AREA, &
!-mm                                                TOTAL_ROFB/TOTAL_AREA, &
!-mm                                                TOTAL_SNO/TOTAL_AREA,    &
!-mm                                                TOTAL_SCAN/TOTAL_AREA,    &
!-mm                                                TOTAL_RCAN/TOTAL_AREA,    &
!-mm                                                TOTAL_ZPND/TOTAL_AREA,    &
!-mm                                                (TOTAL_THLQ(J)/TOTAL_AREA, J = 1, IGND), &
!-mm                                                (TOTAL_THIC(J)/TOTAL_AREA, J = 1, IGND), &
!-mm                                                ((TOTAL_THLQ(J) + TOTAL_THIC(J))/TOTAL_AREA, J = 1, IGND), &
!-mm                                                SUM(TOTAL_THLQ(1:IGND))/TOTAL_AREA, &
!-mm                                                SUM(TOTAL_THIC(1:IGND))/TOTAL_AREA, &
!-mm                                                (SUM(TOTAL_THLQ(1:IGND)) + SUM(TOTAL_THIC(1:IGND)))/TOTAL_AREA, &
...
FINAL_STORE=0.0
!mmDO I = 1, NA
!mm   IF(FRAC(I) /= 0.0)THEN
!mm      DO M = 1, NMTEST
!mm         FINAL_STORE = FINAL_STORE + cp%FAREROW(I,M)*(cp%RCANROW(I,M,q)+cp%SCANROW(I,M,q)+cp%SNOROW(I,M,q)+cp%ZPNDROW(I,M,q)*RHOW)
!mm         DO J = 1, IGND
!mm            FINAL_STORE = FINAL_STORE + cp%FAREROW(I,M)*(cp%THLQROW(I,M,J,q)*RHOW+cp%THICROW(I,M,J,q)*RHOICE)*DLZWROW(I,M,J)
!mm         ENDDO
!mm      ENDDO
!mm   ENDIF
!mmENDDO
...
!mmCALL SIMSTATS(QOBS(1:NCAL,1),QSIM(1:NCAL,1),NCAL,BIAS,NSD,NSW,TPD)
!-------------------------------------------------------------------
...
!  close(unit=250+i*10+7)
!  close(unit=250+i*10+8)
!  close(unit=250+i*10+9)
!  close(unit=250+i*10+10)
!  close(unit=250+i*10+11)
!  close(unit=250+i*10+12)
!  close(unit=250+i*10+13)
ENDDO
