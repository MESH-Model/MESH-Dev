...
!>  would like to work on with a low priority.
    ...
    !> Random fields.
    !> Dan moved MESH_Assimilate to separate file, mesh_perturb.f90.
    use mesh_perturb
...
!so, make them local variables inside each read subroutine.
    ...
    call read_random_fields_config()

400 continue

    call read_random_fields_resume()

500 continue

    !> *********************************************************************
    !> Beginning of the data assimilation loop.
    !> *********************************************************************

    !> Added by Ala Bahrami.
    call perturb_run_config(fls, shd, cm)

505 continue

    !> *********************************************************************
    !> Beginning of the ensemble loop.
    !> *********************************************************************
...
!-    call counter_init()
    ...
    !> Added by Ala Bahrami.
    call perturb_run_init(fls, shd, cm)
...
!-        if (inp > 1 .and. ic%ts_daily == MPIUSEBARRIER) call MPI_Barrier(MPI_COMM_WORLD, ierr)
        ...
        !> Added by Ala Bahrami.
        !> Perturbation for every time step (30 min).
        call perturb_run_pre_ts(fls, shd, cm)
...
!                                    csfv%IGDR, cfi%VMOD, QLWOGAT)
        ...
        if (ipid == 0) then
            ...
            call perturb_run_post_ts(fls, shd, cm, wb_grd)
...
9002    format(/1x, 'ERROR IN READING r2c_output.txt FILE.', &
    ...
    if ((assim_mode .or. .not. RUN_DA) .and. tt .eq. N_ens) then
        call perturb_run_finalize(fls, shd, cm)
        if (ic%now%jday /= ic%stop%jday) then
            goto 400
        end if
    end if
