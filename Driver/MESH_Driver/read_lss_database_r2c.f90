...
subroutine read_lss_db_r2c(shd, iun, fname, ylat, xlng, nrank, ierr)
    ...
    !> Input variables.
    integer, intent(in) :: iun, nrank
    character(len = *), intent(in) :: fname
    real, dimension(nrank), intent(in) :: ylat, xlng
    ...
    integer nkeyword, nattr, l, n, i, x, y, z
    integer, dimension(:), allocatable :: xi, yi
    real, dimension(:, :), allocatable :: dat
    real, dimension(:), allocatable :: ffield, xs, ys, xsn, ysn
...
!+            shd%iyMax = shd%iyMin + shd%GRDN*(shd%yCount - 1)
    ...
    !> Generate order of RANK using provided ylat/xlng.
    allocate( &
        shd%RNKGRD(shd%yCount, shd%xCount), &
        xs(shd%xCount), ys(shd%yCount), xsn(nrank), ysn(nrank), xi(nrank), yi(nrank))
    xsn = huge(xsn)
    xi = 0
    do i = 1, shd%xCount
        xs(i) = (shd%xOrigin + shd%xDelta*i) - shd%xDelta/2.0
        where (abs(xlng - xs(i)) < abs(xlng - xsn))
            xsn = xs(i)
            xi = i
        end where
    end do
    ...
    yi = 0
    do i = 1, shd%yCount
        ys(i) = (shd%yOrigin + shd%yDelta*i) - shd%yDelta/2.0
        where (abs(ylat - ys(i)) < abs(ylat - ysn))
            ysn = ys(i)
            yi = i
        end where
    end do
    ...
    n = 0
    do i = 1, nrank
        if (shd%RNKGRD(yi(i), xi(i)) == 0) then
            n = n + 1
            shd%RNKGRD(yi(i), xi(i)) = n
        end if
    end do

    !> Manually assign grid properties.
    shd%NA = n
    shd%NAA = n
