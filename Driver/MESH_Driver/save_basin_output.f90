        ...
        if (allocated(bno%wb)) then
            do ikey = 1, NKEY
                deallocate(bno%wb(ikey)%PRE, bno%wb(ikey)%EVAP, bno%wb(ikey)%ROF, &
                           ...
                           bno%wb(ikey)%LZS, bno%wb(ikey)%DZS, &
                           bno%wb(ikey)%STG_INI, bno%wb(ikey)%STG_FIN)
            end do
            deallocate(bno%wb)
        end if
        if (allocated(bno%eb)) then
            do ikey = 1, NKEY
                deallocate( &
                    bno%eb(ikey)%IFS, bno%eb(ikey)%ICAN, bno%eb(ikey)%ISNOW, bno%eb(ikey)%IPOND, &
                    bno%eb(ikey)%FSIN, bno%eb(ikey)%ALBT, bno%eb(ikey)%FSOUT, &
                    bno%eb(ikey)%FLIN, bno%eb(ikey)%GTE, bno%eb(ikey)%FLOUT, &
                    bno%eb(ikey)%QH, bno%eb(ikey)%QE, &
                    bno%eb(ikey)%GZERO, &
                    bno%eb(ikey)%TA, bno%eb(ikey)%TCAN, bno%eb(ikey)%CMAS, &
                    bno%eb(ikey)%TSNOW, bno%eb(ikey)%TPOND, &
                    bno%eb(ikey)%TBAR, &
                    bno%eb(ikey)%BAL0, bno%eb(ikey)%BAL1)
            end do
            deallocate(bno%eb)
        end if
