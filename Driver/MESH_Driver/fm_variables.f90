...
module fm_variables
    ...
    subroutine allocate_outlet_location(meta, n, ierr)
        type(outlet_location) meta
        integer n, ierr
        if (allocated(meta%name)) then
            deallocate( &
                meta%name, meta%y, meta%x, &
                meta%iy, meta%jx, &
                meta%rnk)
        end if
        allocate( &
    ...
    subroutine allocate_release_outlet(rls, n, ierr)
        type(release_outlet) rls
        integer n, ierr
        if (allocated(rls%cfn)) then
            deallocate( &
                rls%cfn, &
                rls%b1, rls%b2, rls%b3, rls%b4, rls%b5, &
                rls%b6, rls%b7, &
                rls%area)
        end if
        allocate( &
    ...
    subroutine allocate_abstraction_point_location(absp, n, ierr)
        ...
        if (ierr /= 0) return
        if (allocated(absp%s)) then
            deallocate(absp%s, absp%smin, absp%fsmin)
        end if
