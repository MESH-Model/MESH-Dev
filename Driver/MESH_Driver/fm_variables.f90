...
module fm_variables
    ...
    subroutine allocate_outlet_location(meta, n, ierr)
        type(outlet_location) meta
        integer n, ierr
        if (allocated(meta%name)) deallocate(meta%name, meta%y, meta%x, meta%iy, meta%jx, meta%rnk)
    ...
    subroutine allocate_time_series(ts, n, ierr)
        type(time_series) ts
        integer n, ierr
        if (allocated(ts%val)) deallocate(ts%val)
    ...
    subroutine allocate_release_outlet(rls, n, ierr)
        type(release_outlet) rls
        integer n, ierr
        if (allocated(rls%cfn)) deallocate(rls%cfn, rls%b1, rls%b2, rls%b3, rls%b4, rls%b5, rls%lvlz0, rls%area)
    ...
    subroutine allocate_streamflow_gauge_location(stmg, n, ierr)
        ...
        if (ierr /= 0) return
        if (allocated(stmg%DA)) deallocate(stmg%DA)
    ...
    subroutine allocate_abstraction_point_location(absp, n, ierr)
        ...
        if (ierr /= 0) return
        if (allocated(absp%rr)) deallocate(absp%rr)
