...
!> Values needed for READ_PARAMETERS_HYDROLOGY :
      INTEGER OPTFLAGS, INDEPPAR, DEPPAR, M_C
      REAL WF_R2(M_C)
!> Values already declared above:
!>  NTYPE, NA, RELFLG
!> -----------------------------
...
!> *********************************************************************
        OPEN(UNIT=20,FILE='MESH_drainage_database.txt',
     +       STATUS='OLD', IOSTAT=IOS)
        IF (IOS == 0) THEN
...
!> *********************************************************************
     ...
     + cp%XSLPROW(NA,NTYPE), cp%GRKFROW(NA,NTYPE),
     + cp%MANNROW(NA,NTYPE), cp%WFCIROW(NA,NTYPE),
...
!> Read the mesh_parameters_hydrology.ini file
     ...
     +         hp%ZPLSROW(NA, NTYPE))
      ...
      CALL READ_PARAMETERS_HYDROLOGY(RELFLG, OPTFLAGS, INDEPPAR, DEPPAR,
