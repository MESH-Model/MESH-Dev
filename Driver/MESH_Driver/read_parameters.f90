...
subroutine read_parameters(fls, shd, cm, ierr)
    ...
    if (WF_RTE_flgs%PROCESS_ACTIVE) then
        if (allocated(wfp%r1)) deallocate(wfp%r1, wfp%r2, wfp%aa1, wfp%aa2, wfp%aa3, wfp%aa4)
    ...
    if (IWF == 2 .or. IWF == 3) then
        if (allocated(hp%CMAXROW)) deallocate(hp%CMAXROW, hp%CMINROW, hp%BROW, hp%K1ROW, hp%K2ROW)
        allocate( &
    ...
    if (lzsp%BASEFLOWFLAG == 1) then
        if (allocated(lzsp%dgwsh)) deallocate(lzsp%dgwsh, lzsp%agwsh)
    ...
    if (lzsp%BASEFLOWFLAG == 2) then
        if (allocated(lzsp%WF_LZFA)) deallocate(lzsp%WF_LZFA, lzsp%WF_LZFPWR)
