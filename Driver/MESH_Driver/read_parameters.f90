...
subroutine read_parameters(fls, shd, cm, ierr)
    ...
    if (RUNCLASS36_flgs%PROCESS_ACTIVE) then
        ...
        if (allocated(hp%CMAXROW)) then
            deallocate(hp%CMAXROW, hp%CMINROW, hp%BROW, hp%K1ROW, hp%K2ROW)
        end if
        allocate( &
    ...
    if (WF_RTE_flgs%PROCESS_ACTIVE) then
        if (allocated(wfp%r1)) then
            deallocate(wfp%r1, wfp%r2, wfp%aa1, wfp%aa2, wfp%aa3, wfp%aa4)
        end if
    ...
    if (rteflg%PROCESS_ACTIVE) then
        if (allocated(rtepm%r1n)) then
            deallocate(rtepm%r1n, rtepm%r2n, rtepm%mndr, rtepm%widep, &
                       rtepm%aa2, rtepm%aa3, rtepm%aa4, &
                       rtepm_iak%r1n, rtepm_iak%r2n, rtepm_iak%mndr, rtepm_iak%widep, &
                       rtepm_iak%aa2, rtepm_iak%aa3, rtepm_iak%aa4)
        end if
    ...
    if (pbsm%PROCESS_ACTIVE) then
        if (allocated(pbsm%pm_gru%fetch)) then
            deallocate( &
                pbsm%pm_gru%fetch, pbsm%pm_gru%Ht, pbsm%pm_gru%N_S, pbsm%pm_gru%A_S, &
                pbsm%pm_gru%Distrib, &
                pbsm%pm_grid%fetch, pbsm%pm_grid%Ht, pbsm%pm_grid%N_S, pbsm%pm_grid%A_S, pbsm%pm_grid%Distrib, &
                pbsm%pm%fetch, pbsm%pm%Ht, pbsm%pm%N_S, pbsm%pm%A_S, pbsm%pm%Distrib)
        end if
        allocate( &
    ...
    if (bflm%BASEFLOWFLAG == 1) then
        if (allocated(bflm%pm%dgw)) then
            deallocate(bflm%pm%dgw, bflm%pm%agw, bflm%pm_gru%dgw, bflm%pm_gru%agw)
        end if
    ...
    if (bflm%BASEFLOWFLAG == 2) then
        if (allocated(bflm%pm%pwr)) then
            deallocate(bflm%pm%pwr, bflm%pm%flz, &
                       bflm%pm_iak%pwr, bflm%pm_iak%flz, bflm%pm_gru%pwr, bflm%pm_gru%flz, &
                       bflm%pm_grid%pwr, bflm%pm_grid%flz)
        end if
    ...
    if (cifg%PROCESS_ACTIVE) then
        if (allocated(ciprot%jdsow)) then
            deallocate( &
                ciprot%jdsow, ciprot%ldini, ciprot%lddev, ciprot%ldmid, ciprot%ldlate, &
                ciprot%Kcini, ciprot%Kcdev, ciprot%Kcmid, ciprot%Kclate)
        end if
        allocate( &
        ...
        if (allocated(cip%jdsow)) then
            deallocate( &
                cip%jdsow, cip%ldini, cip%lddev, cip%ldmid, cip%ldlate, &
                cip%Kcini, cip%Kcdev, cip%Kcmid, cip%Kclate)
        end if
        allocate( &
