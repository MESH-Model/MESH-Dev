...
!      use climate_forcing
     ...
     &     iostat = ierr)
      ...
      if (ierr /= 0) then
        print *
        print *, 'MESH_parameters_CLASS.ini could not be opened.',
...
1000  format(2x, 6a4)
      ...
      read(iun, *) DEGLAT, DEGLON, cp%ZRFMGRD(1), cp%ZRFHGRD(1),
     &  cp%ZBLDGRD(1), cp%GCGRD(1), shd%wc%ILG, NLTEST, NMTEST
...
!todo - fix this so that we only use one of the variables (use NA and ignore NLTEST - doc)
      if (NLTEST /= shd%NA) then
        print *
        print *, 'ERROR: The number of grid squares in the class ',
     ...
     &    (cp%PAMXROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%LNZ0ROW(1, m, j), j = 1, ICAN + 1),
     &    (cp%PAMNROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%ALVCROW(1, m, j), j = 1, ICAN + 1),
     &    (cp%CMASROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%ALICROW(1, m, j), j = 1, ICAN + 1),
     &    (cp%ROOTROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%RSMNROW(1, m, j), j = 1, ICAN),
     &    (cp%QA50ROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%VPDAROW(1, m, j), j = 1, ICAN),
     &    (cp%VPDBROW(1, m, j), j = 1, ICAN)
        read(iun, *) (cp%PSGAROW(1, m, j), j = 1, ICAN),
     &    (cp%PSGBROW(1, m, j), j = 1, ICAN)
        read(iun, *) cp%DRNROW(1, m), cp%SDEPROW(1, m),
     &    cp%FAREROW(1, m), cp%DDROW(1, m)
        read(iun, *) cp%XSLPROW(1, m), cp%XDROW(1, m),
     &    cp%MANNROW(1, m), cp%KSROW(1, m), cp%MIDROW(1, m)
        read(iun, *) (cp%SANDROW(1, m, j), j = 1, ig)
        read(iun, *) (cp%CLAYROW(1, m, j), j = 1, ig)
        read(iun, *) (cp%ORGMROW(1, m, j), j = 1, ig)
        read(iun, *) (cp%TBARROW(1, m, j), j = 1, ig),
     &    cp%TCANROW(1, m), cp%TSNOROW(1, m), cp%TPNDROW(1, m)
        read(iun, *) (cp%THLQROW(1, m, j), j = 1, ig),
     &    (cp%THICROW(1, m, j), j = 1, ig), cp%ZPNDROW(1, m)
        read(iun, *) cp%RCANROW(1, m), cp%SCANROW(1, m),
     &    cp%SNOROW(1, m), cp%ALBSROW(1, m), cp%RHOSROW(1, m),
     &    cp%GROROW(1, m)
      end do
...
!todo - make sure these variables are documented properly
     ...
     &  DAILY_STOP_DAY
      read(iun, *) HOURLY_START_YEAR, HOURLY_STOP_YEAR,
