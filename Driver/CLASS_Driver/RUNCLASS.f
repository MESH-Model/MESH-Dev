      PROGRAM RUNCLASS
C AUG 22,08 - D.HOLMAN ADDED WATROF AND WATDRAIN. 
C          AND ADDED 2 NEW OUTPUT FILES: "overlandBaseInter_flow_perDay.out" 
C          AND "overlandBaseInter_flow_perTimestep.out"

C     * SHELL PROGRAM TO RUN "CLASS" ("CANADIAN LAND SURFACE SCHEME")
C     * VERSION 3.4 IN STAND-ALONE MODE USING SPECIFIED BOUNDARY 
...
C
C D.HOLMAN * ADDING WATROF VARIABLES
C DD (DDEN): DRAINAGE DENSITY (CLASS.INI)
C MANN (WFSF): MANNING'S n (CLASS.INI)
      REAL    MANNROW(NLAT,NMOS),DDROW(NLAT,NMOS), 
     1        DDGAT(ILG), MANNGAT(ILG)
C
...
C D.HOLMAN ADDING ACCUMULATORES FOR BASE OVERLAND AND INTER FLOW
      REAL,DIMENSION(NLAT) ::
     1           ROFOACC, ROFSACC, ROFBACC
...
C      
      ...
      ITCG=2
      ITG=2
C D.HOLMAN * setting IWF to 3 instead of 0 to make watrof and watdrain work.
      IWF=3
      IPAI=0
      IHGT=0
      IALC=0
...
C     * OPEN FILES FOR READING AND WRITING.
     ...
     1             STATUS='OLD')
      ...
      OPEN(UNIT=61,FILE='CLASS_recheck_v3.4.of1')
      OPEN(UNIT=62,FILE='CLASS_recheck_v3.4.of2')
      OPEN(UNIT=63,FILE='CLASS_recheck_v3.4.of3')
      OPEN(UNIT=64,FILE='CLASS_recheck_v3.4.of4')
      OPEN(UNIT=65,FILE='CLASS_recheck_v3.4.of5')
      OPEN(UNIT=66,FILE='CLASS_recheck_v3.4.of6')
      OPEN(UNIT=67,FILE='CLASS_recheck_v3.4.of7')
      OPEN(UNIT=68,FILE='CLASS_recheck_v3.4.of8')
      OPEN(UNIT=69,FILE='CLASS_recheck_v3.4.of9') 
      
C D.HOLMAN ADDED OUTPUT FILE 70,71 FOR OVERLANDFLOW BASEFLOW AND INTERFLOW
      OPEN(UNIT=70,FILE='overlandBaseInter_flow_perDay.out')
      OPEN(UNIT=71,FILE='overlandBaseInter_flow_perTimestep.out')

C     * READ AND PROCESS INITIALIZATION AND BACKGROUND INFORMATION.
C     * FIRST, MODEL RUN SPECIFICATIONS.

C D.HOLMAN ADDING OUTPUT FILE 70,71 FOR OVERLANDFLOW BASEFLOW AND INTERFLOW
      WRITE(70,"('  DAY YEAR        ROFO',
     1    '        ROFS        ROFB        ROF')")
      WRITE(71,"('  HOUR  MIN  DAY YEAR        ',
     1   'ROFO        ROFS        ROFB         ROF')")
...
C D.HOLMAN * ADDING DDROW TO THE FOLLOWING READ STATEMENT
          READ(50,'(3F8.3,F8.2)') DRNROW(I,M),SDEPROW(I,M),
     1                            FAREROW(I,M), DDROW(1,M)
C D.HOLMAN * ADDING MANNROW TO THE FOLLOWING READ STATEMENT
          READ(50,'(4E8.1,I8)') XSLPROW(I,M),GRKFROW(I,M),
     1                  MANNROW(1,M),WFCIROW(I,M),MIDROW(I,M)
...
100   CONTINUE
      ...
      DO 150 I=1,NLTEST
C D.HOLMAN * ADDING ACCUMULATED OVERLANDFLOW BASEFLOW AND INTERFLOW
          ROFOACC(I)=0.
          ROFSACC(I)=0.
          ROFBACC(I)=0.
...
C D.HOLMAN * ADD PERTIMESTEP OUTPUT FILE FOR OVERLAND,INTER,BASE FLOW.
          WRITE(71,'(1X,I5,I5,I5,I5,4E12.4)') IHOUR, IMIN, IDAY, IYEAR,
     1           ROFOROW(I,M), ROFSROW(I,M), ROFBROW(I,M), ROFROW(I,M)
...
C     * ACCUMULATE OUTPUT DATA FOR DIURNALLY AVERAGED FIELDS.
      ...
      DO 675 I=1,NLTEST
      DO 650 M=1,NMTEST
C D.HOLMAN * ADDING ACCUMULATORS FOR OUTPUT OF BASE INTER OVERLAND FLOW
          ROFOACC(I)=ROFOACC(I)+ROFOGRD(I)*FAREROW(I,M)*DELT
          ROFSACC(I)=ROFSACC(I)+ROFSGRD(I)*FAREROW(I,M)*DELT
          ROFBACC(I)=ROFBACC(I)+ROFBGRD(I)*FAREROW(I,M)*DELT
...
C D.HOLMAN * ADD OUTPUT FILE WITH DAILY OVERLANDFLOW BASEFLOW AND INTERFLOW
              WRITE(70,'(1X,I4,I5,4E12.4)') IDAY, IYEAR,
     1               ROFOACC(I), ROFSACC(I), ROFBACC(I), ROFACC(I)
...
C D.HOLMAN * ADDING OVERLAND BASE AND INTER FLOW 
          ROFOACC(I)=0.
          ROFSACC(I)=0.
          ROFBACC(I)=0.
