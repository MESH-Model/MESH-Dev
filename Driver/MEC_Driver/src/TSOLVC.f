     ...
     F                 TPOTG,RESID,RESIDL,RESIDO,TZEROL,TZEROO,TCANL,
...
C     * INTEGER CONSTANTS.
C
      INTEGER ISNOW,ISLFD,ILW,ITC,ITCG,ILG,IL1,IL2,JL,I
C
...
C
     ...
     6     RBCOEF(ILG),    AILCAN(ILG),    ZOSCLH(ILG),    ZOSCLM(ILG),
...
C
     ...
     4     TPOTG (ILG),    RESID (ILG),    RESIDL(ILG),    RESIDO(ILG),    
     5     TZEROL(ILG),    TZEROO(ILG),    TCANL (ILG),    TCANO (ILG),    
     6     TRTOP (ILG),    QSTOR (ILG),    A     (ILG),    B     (ILG),    
     7     ZOMS  (ILG),    ZOHS  (ILG),    LZZ0  (ILG),    LZZ0T (ILG),    
     8     FM    (ILG),    FH    (ILG),    WZERO (ILG),    XEVAPM(ILG),    
     9     DCFLXM(ILG),    WC    (ILG),    DRAGIN(ILG),    CFLUXM(ILG),
     A     CFSENS(ILG),    CFEVAP(ILG),    QSGADD(ILG),    CFLX  (ILG)
C
...
C
     ...
     2                TCGLAC,CLHMLT,CLHVAP
      ...
      COMMON /SURFCON/ AS,ASX,CI,BS,BETA,FACTN,HMIN,ANGMAX
C
C-----------------------------------------------------------------------
...
C
     ...
     1                QLWOC(I)-QLWOG(I)-QSENSG(I)-QEVAPG(I)-GZERO(I)
              ...
              IF(ABS(TSTEP(I)).LT.1.0E-3)                  ITER(I)=0
      ...
      DO 300 I=IL1,IL2
          IF(FI(I).GT.0.)                                          THEN
              IF(ABS(EVAPG(I)).LT.1.0E-8) THEN
                  ...
                  QEVAPG(I)=0.0
              ENDIF
              IF(RESID(I).GT.15. .AND. QEVAPG(I).GT.10.)    THEN
                  QEVAPG(I)=QEVAPG(I)+RESID(I)
              ELSE
C                  QSENSG(I)=QSENSG(I)+0.5*RESID(I)
...
C
     ...
     1              (3-ILW)-RHOAIR(I)*SPHAIR*(CFLX(I)+MAX(0.,
...
C
      DO 700 I=IL1,IL2
          IF(FI(I).GT.0. .AND. ITER(I).EQ.1)                       THEN    
              IF(QAC(I).GT.QCAN(I))                         THEN
                  IF(SNOCAN(I).GT.1.0E-5) THEN
                      XEVAP(I)=FSNOWC(I)/RB(I)
                  ELSE
                      XEVAP(I)=1.0/RB(I)      
                  ENDIF
              ELSE                       
                  IF(SNOCAN(I).GT.0.0)               THEN 
                      XEVAP(I)=(FRAINC(I)+FSNOWC(I))/RB(I)  
                  ELSE                                     
                      XEVAP(I)=FRAINC(I)/RB(I)+(1.0-FRAINC(I))/
     1                         (RB(I)+RC(I))                            
