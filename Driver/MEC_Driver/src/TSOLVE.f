     ...
     B                  TZEROL,TZEROO,DCFLXM,CFLUXM,WZERO,TRTOP,A,B,
...
C     * INTEGER CONSTANTS.
C
      INTEGER ISNOW,ISLFD,ILW,ITG,ILG,IL1,IL2,JL,I
C
...
C
     ...
     2     TZEROO(ILG),    DCFLXM(ILG),    CFLUXM(ILG),    TRTOP (ILG),    
     3     A     (ILG),    B     (ILG),    ZOMS  (ILG),    ZOHS  (ILG),    
     4     LZZ0  (ILG),    LZZ0T (ILG),    FM    (ILG),    FH    (ILG),
     5     WZERO (ILG)
C
...
C
     ...
     1              (TZERO(I)-B(I)))/PADRY(I)           
              ...
              IF(ISNOW.EQ.1 .OR. QA(I).GT.Q0SAT(I))           THEN
                  EVBETA(I)=1.0        
                  QZERO(I)=Q0SAT(I)
...
C
     ...
     1                (TZERO(I)-B(I)))/PADRY(I)
                  ...
                  ELSE
                      QEVAP(I)=RESID(I)*0.5
                  ENDIF
C                  QEVAPT=CPHCH(I)*(QZERO(I)-QA(I))
C                  BOWEN=SPHAIR*(TZERO(I)-TPOTA(I))/
C     1                SIGN(MAX(ABS(QEVAPT),1.E-6),QEVAPT)
C                  QEVAP(I)=RESID(I)/SIGN(MAX(ABS(1+BOWEN),0.1),1+BOWEN)
