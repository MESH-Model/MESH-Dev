...
c     Declarations resulting from trying to deal with the code fork of October 14, 2006
      REAL trunoff(ILG),thliq0,thlinf0,thliq_eff,
...
c
c
          thliq_avail = max(1.0e-8,thliq0-thlmin(i,j))
          thpor_avail = max(thlmin(i,j),thpor(i,j)-thice(i,j))
...
c
		  ...
		  H0 = 1.0
c
cric 11oct06 - operational call to wat_drain
