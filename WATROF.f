...
C
     ...
     2      RUNOFF(ILG),     TRUNOF(ILG), asat_fc
C
C     * INPUT ARRAYS.
C
...
c bjd/eds/bmt/frs - check units of DD - Ric would like to see km/km^2 and we probably have m/m^2
...
c
		  ...
		  cc=2.*BI(I,j)+3
            H = delzw(i,j)
		  H0 = 1.0
      asat_fc=THFC(i,j)/thpor_avail 
c
c      operational call to wat_drain
            call WAT_DRAIN(3,H,H0,ztop,xdrainh,cc,grk_eff,
     +        asat_fc,bsat_t0,asat_t0,asat_t1)
