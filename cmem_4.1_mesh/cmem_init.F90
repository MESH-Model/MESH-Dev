...
USE YOMCMEMPAR, ONLY : CIDVEG,CIRGHR, CIATM, CIVEG, LGPRINT,nobs_atm,kcm, ip_rgh_surf, tfreeze,  fghz 
USE YOMCMEMFIELDS, ONLY: N_CMEM, JJ, ftl_lsm,ftair, fTVL, fTVH, fwc_veg, fb, fs_laiL, ftfrac &
...
REAL(KIND=JPRM) :: hr
REAL(KIND=JPRM) :: fc(N_CMEM)
REAL(KIND=JPRM) :: wt(N_CMEM)
REAL(KIND=JPRM) :: xlc
...
SELECT CASE (CIVEG)
   ...
   CASE ('Wigneron')
   ! 2.5 b1 and b2 parameters used to compute taunadir (Wigneron 2007)
 
      ftauN(:,1) = b1(fTVL(:)) * fs_laiL(:) + b2(fTVL(:)) 
      ftauN(:,2) = b3(fTVH(:))  ! for high veg cste value assigned

   ! 2.6 tth and ttv function of vegetation type (Wigneron 2007)
   ...
   ! 2.7  Wigneron et al. (2007) single diffusion albedo
...
! 3.0 rougness parameters (Wigneron 2007)
...
SELECT CASE (CIRGHR)
  ...
  CASE ( 'Choudhury' )
    ...
    fhrmodel(:,2) = fh(:)  ! on high veg
     ...
     fNrv_L(:) = 0.0_JPRM 
     fNrv_H(:) = 0.0_JPRM 
  CASE ( 'Wsimple' )   !    Wigneron et al., 2001  
    xlc = 6._JPRM
  ...
  CASE ( 'Wegmueller' )
    ...
    fhrmodel(:,1) = fh(:)  ! on low veg
    fhrmodel(:,2) = fh(:)  ! on high veg
  CASE ( 'Wtexture' )  !    Wigneron et al., 2001
    ...
    fhrmodel(:,1) = fh(:)  ! on low veg
    fhrmodel(:,2) = fh(:)  ! on high veg 
  CASE ( 'Wigneron' )   !    Wigneron et al., 2007; Saleh et al., 2006 
     fhrmodel(:,1) = Zhr(fTVL(:))
     fhrmodel(:,2) = Zhr(fTVH(:))
