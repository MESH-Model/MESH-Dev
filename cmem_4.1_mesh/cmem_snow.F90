...
USE YOMCMEMPAR, ONLY : fghz, omega, k_cmem, sintheta, costheta, tfreeze, rhowat, pi_cmem, LGPRINT
...
REAL(KIND=JPRM), PARAMETER :: PPDIAM = 5.
...
ALFA = k_cmem*ABS(AIMAG(SQRT(REWS-CIM*XIEWS)))
BETA = k_cmem*REAL(SQRT(REWS-CIM*XIEWS))

K2X = k_cmem * sintheta
PPP = 2. * ALFA * BETA
QQ = BETA**2. - ALFA**2. - (k_cmem**2.)*(sintheta)**2.
XJAKAJA = (1./(SQRT(2.)))*SQRT(SQRT(PPP**2.+QQ**2.)+QQ)
! -------- PROPAGATION ANGLE IN SNOW
O2 = ATAN(K2X/XJAKAJA)                             
!ETKULLUMI=O2*180./pi_cmem;      % NON USEFUL
