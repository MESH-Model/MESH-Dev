...
!                                                                      *
!   Input: RHO  density of water vapor                                 *
...
IMPLICIT NONE
REAL(KIND = JPRM) ::  F0O2, A,RHO,t,pp,f,alpha,tc,v,e,pb,p,ao,an,gammao,s1,fpp
INTEGER(KIND=JPIM) :: i
COMMON /O2line/ F0O2(48),A(6,48)
REAL(KIND = JPRM) ::  S(48),GAMMA(48),DELTA(48)
REAL(KIND = JPRM) :: NDPP,NOXPP
...
Tc = T - 273.15_JPRM 
V  = 300._JPRM / T
E  = (RHO * T * 461.52_JPRM) * 1.0E-3_JPRM
Pb = PP / 1000._JPRM
P  = Pb - E
...
DO I=1,48
  S(I) = A(1,I) * P * V**3._JPRM * EXP(A(2,I)*(1._JPRM-V)) * 1.E-6_JPRM
  GAMMA(I) = A(3,I) * (P * V**(0.8_JPRM-A(4,I)) + 1.1_JPRM * E * V) * 1.E-3_JPRM

  ! Zeeman approximation for .6 Gauss, [10]
  IF (Pb .LT. 0.7_JPRM) THEN
    GAMMA(I) = ( GAMMA(I)**2.0_JPRM + (25.0_JPRM * 0.6E-4_JPRM)**2 ) ** 0.5_JPRM
  ENDIF
  DELTA(I) = A(5,I) * P * V**A(6,I) * 1.E-3_JPRM
ENDDO
...
Ao = 6.14E-4_JPRM
An = 1.40E-10_JPRM * (1._JPRM - 1.2E-5_JPRM * F**1.5_JPRM)
GAMMAo = 4.8E-3_JPRM * (P + 1.1_JPRM * E) * V**.8_JPRM
Ndpp = (Ao * P * V**2_JPRM * GAMMAo / (F*F + GAMMAo**2._JPRM) + An * P * P * V**3.5_JPRM) * F 
...
ENDDO
Noxpp = S1
IF (Noxpp .LT. 0.0) Noxpp = 0._JPRM
ALPHA = 0.000182_JPRM * F * (Noxpp + Ndpp)
ALPHA = ALPHA / 4.343_JPRM
...
IMPLICIT NONE
INTEGER(KIND=JPIM) :: i
REAL(KIND = JPRM) :: F0water, B, RHO, T, PP, F, ALPHA,Tc,V,E,PB,P,BF,BS,S1,FPP 
COMMON /H2O/ F0water(30),B(6,30)
REAL(KIND=JPRM) ::  GAMD2(30),SH(30),GAMH(30),DELH(30)
REAL(KIND = JPRM) :: Nipp, Nvpp
...
DO I = 1,30
  SH(I) = B(1,I) * E * V**3.5_JPRM * EXP(B(2,I)*(1.-V))  
  GAMH(I) = B(3,I) * (P * V**B(4,I) + B(5,I) * E * V**B(6,I)) * 1.E-3_JPRM
  IF (Pb .LT. 0.7_JPRM) THEN
    GAMD2(I) = (2.14_JPRM * F0water(I)**2_JPRM) * 1E-12_JPRM / V
    GAMH(I) = (GAMH(I)**2_JPRM + GAMD2(I))**0.5_JPRM
  ENDIF
  DELH(I) = 0._JPRM
ENDDO 
...
Bf = 1.13E-6_JPRM
Bs = 3.57E-5_JPRM
Nipp = (Bf * E * P * V**3.0_JPRM + Bs * E**2_JPRM * V**10.8_JPRM) * F
...
ALPHA = 0.000182_JPRM * F * (Nvpp + Nipp)
ALPHA = ALPHA / 4.343_JPRM
...
IMPLICIT NONE
INTEGER(KIND=JPIM) :: i,IFLAG
REAL(KIND=JPRM) :: F,Vo,GAMMA,DELTA,Fpp,G2,VMF,VPF,CUTOFF
...
IF (IFLAG .EQ. 1) THEN
  IF (F .GT. (Vo+CUTOFF*GAMMA)) Fpp=0._JPRM
  !  SET ABSORPTION TO ZERO IF F IS > NU+40.0_JPRM*GAMMA
