...
!     Forcing data variable
      real, dimension(:,:), allocatable :: fldr2c,fldfst,uu,vv
...
!     External functions
      ...
      integer, external :: ezqkdef,ezdefset,ezsetopt,ezsint
...
!     R2C grid parameters from MESH_drainage_database.r2c file
      call read_grid_parameters()
      allocate (fldr2c(xCount,yCount),uu(xCount,yCount), &
                vv(xCount,yCount),stat=pas)
      if (pas .ne. 0) then
         print*
         print*,'Error in allocating R2C two dimensional array.'
         print*,'xCount ', xCount
         print*,'yCount ', yCount
         stop
      endif

!     Define the R2C grid (uniform lat/lon grid)
      if (Projection == "LATLONG")then
          call cxgaig('L',ig1_des,ig2_des,ig3_des,ig4_des, &
                          yOrigin,xOrigin,yDelta,xDelta)
          gdr2c = ezqkdef(xCount,yCount,'L', &
                          ig1_des,ig2_des,ig3_des,ig4_des,0)
      else
          print*
          print*,'Currently the LATLONG projection of MESH grid ', &
                 'is functional.'
          stop
      endif
