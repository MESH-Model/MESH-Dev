...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      ...
      character(20), dimension(:), allocatable :: sta_name
      real*4,        dimension(:), allocatable :: sta_lat
      real*4,        dimension(:), allocatable :: sta_long
      real*4,        dimension(:), allocatable :: sta_area
      logical    :: exists
...
!     PROGRAM SIMPLE
      ...
      if(.not.allocated(nclass))then
        allocate(nclass(ntype+1),stat=iAllocate)
        if(iAllocate.ne.0) STOP
...
d	print*,'no of reaches found       =',maxr
	if(maxr.lt.noresv)then
...
c        channel_area(n)=chawid(n)*rl(n)
        ...
        if(aclass(n,ntype).lt.chaarea(n)/grid_area(n))then
...
!         this will make the sum of the class fractions > 0.0
	    aclass(n,ntype)=chaarea(n)/grid_area(n)
	  elseif(ireach(n).eq.0)then
...
!       is taken away from the other classes
	  do ii=1,ntype+1
          if(ii.ne.ntype)then
 	      if(abs(1.0-class_sum-aclass(n,ntype)).gt.0.000001)then
              aclass(n,ii)=aclass(n,ii)/(class_sum+aclass(n,ntype))
            endif
	    endif
	  end do
        class_sum=0.0
...
!     rev. 9.2.32  Feb.  10/06  - NK: Added area_check.csv to output
      INQUIRE(FILE='basin\flow_station_info.txt',EXIST=exists)
      IF(exists)THEN
        open(unit=99,file='basin\flow_station_info.txt',
     *                              status='unknown')
        do l=1,no
          read(99,*,iostat=ios)sta_id(l),
     *                        sta_lat(l),sta_long(l),sta_area(l)
          if(ios.ne.0)then
            print*,'Problems reading basin\flow_station_info.txt'
	      if(l.ge.1)then
	        print*,'Last line read:'
              write(*,*)l-1,'  ',sta_id(l-1),
     *          sta_lat(l-1),sta_long(l-1),sta_area(l-1),area(l-1)
	      else
	        print*,'Problem is in the first line'
	      endif
            print*
            stop 'Program aborted in flowinit @ 350'
          endif
        end do
...
!     *        sta_area(l),area(l),(area(l)-sta_area(l))/area(l)*100.
          write(99,99001)xstr(l),ystr(l),l,sta_id(l),
     *      sta_area(l),area(l),(area(l)-sta_area(l))/sta_area(l)*100.
        end do
        close(unit=99,status='keep')
      end if
...
!     subtract the reservoir releases from the recorded flows at gauges
      do k=1,noread
         ...
         if(n.le.0.and.nnn.le.0)then
            print*, 'Reservoir no.',k,' at i=',i,' j=',j
            print*, '     grid no ',n
            print*, 'is located outside the watershed'
            print*, 'Please note this grid is not in the .shd file'
            inbsnflg(no+k)=0
         endif
         resflag=.false.
...
!        LOCATION OF DOWNSTREAM ELEMENT:
         ...
         if(nnx.le.0)then
            ...
            print*,' watershed'
            print*
            STOP ' Program aborted in flowinit at line ~339'
         endif
         inx=yyy(nnx)
         jnx=xxx(nnx)
         if(nnx.gt.0)then
...
! * * * * * * *  TS  - added initializations * * * * * * * * 
          ...
          print*,'Zero or negative drainage area found in grid no. ',n
          print*,'To accept this, hit return - but check outcome'
          print*,'Program may crash'
...
!         There is no lzs under laks & reservoirs so adjust:
          lzs(n)=lzs(n)*(1.0-aclass(n,ntype))
