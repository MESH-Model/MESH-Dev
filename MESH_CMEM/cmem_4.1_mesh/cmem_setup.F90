...
CFINOUT = 'gribapi'        ! Default Input/output is GribAPI

IF (LOFFCMEM) NULTMP = 77
IF (LOFFCMEM) NULNAM = 14 

! Open namelist:
OPEN(NULNAM,FILE='input',STATUS='OLD')
...
! Default number of levels in soil and atm (for CIDATM= 'Liebe')
nobs_atm = 60.0_JPRM
...
IF ( nlay_soil_ls .ne. nlay_soil_ls_default ) Then
 ...
 !
  CASE ('gribex')
 !
 CALL ABOR1('Wrong LSM Layer choice. When using Grib use default layers.')
 !
  CASE ('gribapi')
 !
 CALL ABOR1('Wrong LSM Layer choice. When using Grib use default layers.')
 ...
 !
  DEALLOCATE(z_lsm)
  ALLOCATE (z_lsm(nlay_soil_ls))
  OPEN(NULTMP,FILE='LSM_VERTICAL_RESOL.asc',status='old')
  READ (NULTMP,*) (z_lsm(i),i=1,nlay_soil_ls) 
  WRITE(NULOUT,*) ' ASCII IO: LSM layers depth: ',z_lsm(:)
  CLOSE(NULTMP)   
 !
  CASE ('netcdf')
 !   
 CALL ABOR1('Wrong LSM Layer choice. When using NetCDF use default layers.')
 !
 !CASE ('ifs')
 !
 ! > ****************ADD FOR COUPLING WITH MESH****************************
  CASE ('MESH')
  CALL ABOR1('Wrong LSM Layer choice. When using MESH use default layers.')
!>=========================================================================
 END SELECT
...
IF (CITEFF/= 'Tsoil'.AND.CITEFF/= 'Choudhury'.AND.CITEFF/= 'Wigneron'.AND.CITEFF/= 'Holmes') CALL ABOR1('Wrong CITEFF choice.')
...
IF(CIATM/= 'No'.AND.CIATM/= 'Pellarin'.AND.CIATM/= 'Liebe'.AND.CIATM/= 'Ulaby') CALL ABOR1('Wrong CIATM choice.')
...
IF(CFINOUT/= 'gribapi'.AND.CFINOUT/= 'gribex'.AND.CFINOUT/= 'netcdf'.AND.CFINOUT/= 'ascii' .AND.CFINOUT/= 'MESH')&   !KCK-> ADD MESH
& CALL ABOR1('Wrong CFINOUT choice.  Choose gribapi, gribex, netcdf, ascii, or MESH') !> KCK -> ADD FOR MESH
!-------------------------------------
...
!-------------------------------------
 ...
 tfreeze = 273.15_JPRM                          !
 rhowat = 1000._JPRM  
 pi_cmem = acos(-1._JPRM)
 f = fghz * 1E9_JPRM
 omega = 2._JPRM * pi_cmem * f
 lam = c / f           
 k_cmem = 2._JPRM * pi_cmem / lam    
 lamcm = lam * 100._JPRM 
 kcm = k_cmem / 100._JPRM 
