...
USE YOMCMEMPAR, ONLY : costheta, k_cmem, rhowat, LGPRINT
...
DO i = 1, 2
  tau_veg(i) = a_geo(i) * k_cmem * (wc_veg / rhowat) * aimag(eps_vw) / costheta
ENDDO
...
USE YOMCMEMPAR, ONLY : theta, pi_cmem, k_cmem, costheta, sintheta, LGPRINT
USE YOMCMEMVEG, ONLY : wc_veg, tau_veg, eps_vw, a_geo
...
!    -----------------------------------------
kz0 = k_cmem * costheta
kz1 = k_cmem * sqrt(eps_veg - sintheta * sintheta)
...
tau_veg(1) = a_geo(1) * k_cmem * ( BB / rho_veg ) * aimag(eps_veg) * th / costheta
tau_veg(2) = a_geo(2) * k_cmem * ( BB / rho_veg )  * aimag(eps_veg) * tv / costheta
...
! tauV and tauH both related to tauN (Wigneron et al., 2007)
...
!   Remote Sensing of Environment. Vol. 107, pp. 637-651
!  Wigneron, et al., "A simple algorithm to retrieve soil moisture 
!   and vegetation biomass using passive 
!   microwave measurements over crop fields," Remote sens. environ., 
!   vol. 51, pp. 331-341, 1995.
...
! Purpose :
!  CALCULATE VEGETATION OPACITY for frequencies < 10GHz
! tauN related to wc_veg (Jackson and Schmugge, 1991)
! tauV and tauH both related to tauN (Wigneron, )

! Reference :
!  JACKSON, T J; SCHMUGGE, T J 1991, Vegetation effects on the microwave 
!   emission of soils, Remote Sensing of Environment. Vol. 36, pp. 203-212. 
!   June 1991 
!  J.-P. Wigneron, A. Chanzy, J.-C. Calvet, and N. Bruguier, "A simple 
!   algorithm to retrieve soil moisture and vegetation biomass using passive 
!   microwave measurements over crop fields," Remote sens. environ., 
!   vol. 51, pp. 331-341, 1995.
