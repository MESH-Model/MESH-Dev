...
!>       MESH DRIVER
!>
!>       AUG 2014 - K.C. KORNELSEN. 
!>				  - COUPLED MESH WITH CMEM
!>       JAN 2014 - K.C.KORNELSEN
...
!> ********* ADD IF USING CMEM******************************************
  WRITE (58,*) 'CMEMFLAG             = ', CMEMFLAG   !KCK ADDED FOR CMEM
  DO N = 1,CMEMFLAG
  WRITE (58,*) 'CMEMTHETA            = ', CMEMTHETA(N) !KCK ADDED FOR CMEM
  ENDDO
  WRITE (58,*) 'CMEMR2COUTPUTFLAG    = ',CMEMR2COUTPUTFLAG
!>=======================================================================
!  ENDDO
!ENDIF
...
CALL INIT_MESH_CMEM(ILG,NML,NA,NTYPE,NMTEST,IGND,ICAN,ILMOS,JLMOS, sl%ZBOT,ELEV) 

END IF ! IF CMEM
!>=====================================================================

!> WRITE R2C FILE HEADERS
IF (CMEMR2COUTPUTFLAG == 1) THEN
CALL WRITE_CMEM_R2C_HEADER(NMTEST,coordsys1,datum1,zone1,XORIGIN,YORIGIN, &
...
!****************ADD FOR RUNNING CMEM*************************************

IF (CMEMFLAG > 0) THEN
!> KCK - CALL CMEM SUBROUTINES
!> 
!> CONVERT MESH VARIABLES TO CMEM FOR READING
CALL RDCMEMMESHSTATIC(NA,NTYPE,ILG,NMTEST,IGND,ICP1, &
              ZSNOW, RHOSGAT, THLQGAT, TBARGAT, TAGAT, TSFSGAT, &
			  FCANGAT, SANDGAT, CLAYGAT)
			  
! Loop for each incidence angle
DO M = 1,CMEMFLAG
  CALL RUN_MESHCMEM(M)



!> NATIVE CMEM OUTPUTS ARE:
! LEVEL 1OUTPUTS OF CMEM WILL BE TBH(K), TBV(K), TEFF_H(K)
!                       ftb_toa(JJ,1), ttb_toa(JJ,2), fteffC(JJ,2)
! LEVEL 2 OUTPUTS OF CMEM WILL BE Tau_Veg(H,V), Total bare soil fraction, Total VWC, Atmospheric optical depth &
!					TB Atmosphere Upward
!                       ftau_veg(JJ,1), ftau_veg(JJ,2), ftfrac(JJ,1)+ftfrac(JJ,2), &
!			fwc_veg(JJ,1)*(ftfrac(JJ,3)+ftfrac(JJ,4)) +  fwc_veg(JJ,2)*(ftfrac(JJ,5)+ftfrac(JJ,6)), &
!			ftau_atm(JJ), ftb_au(JJ)
! LEVEL 3 OUTPUTS OF CMEM WILL BE Pure (no snow) bare soil fraction, Pure(no snow) Low veg fraction, &
!			Low Veg WC, High Veg WC, Low veg b param, High veg b param, Roughness param h, Wilting point SM, &
!			Horizontal emissivity, vertical emissivity, C parameter
!							ftfrac(JJ,1), ftfrac(JJ,3), fwc_veg(JJ,1), fwc_veg(JJ,2), fb(JJ,1),fb(JJ,2), &
!							fh(JJ), fWP(JJ), fsurf_emis(JJ,1), fsurf_emis(JJ,2), fteffC(JJ,1)
!> FOR MESH THESE CANNOT ALL BE IMPLEMENTED. THEREFORE ONLY TBh, TBv, Tau_h, and Tau_v will be available.

DO JJ = 1,N_CMEM !WRITE TO MESH GAT VARIABLES
  TBH(JJ,M) = ftb_toa(JJ,1)
  TBV(JJ,M) = ftb_toa(JJ,2)
  TAUH(JJ,M) = ftau_veg(JJ,1)
  TAUV(JJ,M) = ftau_veg(JJ,2)
END DO

END DO ! END OF CMEMFLAG LOOP

! WRITE CMEM VALUES TO R2CDATA
IF (CMEMR2COUTPUTFLAG == 1) THEN

IF(NR2CFILES_CMEM > 0 .AND. MOD(NCOUNT*30,DELTR2C_CMEM) == 0)THEN

  CALL FIND_MONTH (IDAY, IYEAR, ensim_month)
  CALL FIND_DAY (IDAY, IYEAR, ensim_day)
  
  CALL WRITE_CMEM_R2C_DATA(NML,NLTEST,NMTEST,NCOUNT,IMIN,ACLASS,    &
                      NA,XXX,YYY,XCOUNT,YCOUNT,ILMOS,JLMOS,ILG,&
                      NR2C_CMEM,NR2CFILES_CMEM,R2CFILEUNITSTART_CMEM,GRD_CMEM,GAT_CMEM, &
                      GRDGAT_CMEM,NR2CSTATES_CMEM,R2C_ATTRIBUTES_CMEM,FRAME_NO_CMEM,IYEAR,&
                      ensim_MONTH,ensim_DAY,IHOUR,IMIN,ICAN,   &
                      ICAN+1,CMEMFLAG,TBH,TBV,TAUH,TAUV)

  FRAME_NO_CMEM = FRAME_NO_CMEM + 1 !UPDATE COUNTERS
