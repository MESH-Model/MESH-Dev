...
!> if IOS is set to zero, then the file was opened successfully.
     ...
     +          "READING: MESH_parameters_CLASS.ini"
      ...
      READ(50,'(5F10.2,F7.1,3I5)') DEGLAT,DEGLON,cp%ZRFMGRD(1),
...
!todo - fix this so that we only use one of the variables (use NA and ignore NLTEST - doc)
     ...
     2  "from the shed file."
      ...
      I=1
      DO M=1,NMTEST
        READ(50,'(9F8.3)') (cp%FCANROW(1,M,J),J=1,ICAN+1),
     1                  (cp%PAMXROW(1,M,J),J=1,ICAN)
        READ(50,'(9F8.3)') (cp%LNZ0ROW(1,M,J),J=1,ICAN+1),
     1                  (cp%PAMNROW(1,M,J),J=1,ICAN)
        READ(50,'(9F8.3)') (cp%ALVCROW(1,M,J),J=1,ICAN+1),
     1                  (cp%CMASROW(1,M,J),J=1,ICAN)
        READ(50,'(9F8.3)') (cp%ALICROW(1,M,J),J=1,ICAN+1),
     1                  (cp%ROOTROW(1,M,J),J=1,ICAN)
        READ(50,'(4F8.3,8X,4F8.3)') (cp%RSMNROW(1,M,J),J=1,ICAN),
     1                  (cp%QA50ROW(1,M,J),J=1,ICAN)
        READ(50,'(4F8.3,8X,4F8.3)') (cp%VPDAROW(1,M,J),J=1,ICAN),
     1                  (cp%VPDBROW(1,M,J),J=1,ICAN)
        READ(50,'(4F8.3,8X,4F8.3)') (cp%PSGAROW(1,M,J),J=1,ICAN),
...
c     Convert DDEN from km/km^2 to m/m^2
