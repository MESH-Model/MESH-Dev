     ...
     8            PAIDAT,HGTDAT,THLIQ,THICE,TBAR,rncan,SNCAN,
...
C                                                                                 
      REAL rncan  (ILG),     SNCAN (ILG),     TCAN  (ILG),     
...
C
      IF (ICTEMMOD.EQ.0) THEN
        DO 120 I=IL1,IL2
          indx = iNT( (RADJ(I)+PI/2.0)*180.0/PI/10.0 ) + 1
          IF(DLON(I).GT.190. .AND. DLON(I).LT.330.)            THEN           
              NL=2                                                            
          ELSE                                                                
              NL=1                                                            
          ENDIF                                                               
          IF(GROWYR(indx,1,NL).LT.0.1)                      THEN
              GROWA(I)=1.0                                                    
          ELSE                                                                
            IF(indx.GT.9)                                 THEN
              IF(DAY.GE.GROWYR(indx,2,NL).AND.DAY.LT.GROWYR(indx,3,NL))           
     1              GROWA(I)=1.0                                                
                IF(DAY.GE.GROWYR(indx,4,NL).OR.DAY.LT.GROWYR(indx,1,NL))            
     1              GROWA(I)=0.0                                
            ELSE
               IF(DAY.GE.GROWYR(indx,2,NL).OR.DAY.LT.GROWYR(indx,3,NL))           
     1              GROWA(I)=1.0                                                
               IF(DAY.GE.GROWYR(indx,4,NL).AND.DAY.LT.GROWYR(indx,1,NL))            
     1              GROWA(I)=0.0                                
              ENDIF                
              IF(DAY.GE.GROWYR(indx,1,NL).AND.DAY.LT.GROWYR(indx,2,NL))           
     1            GROWA(I)=(DAY-GROWYR(indx,1,NL))/(GROWYR(indx,2,NL)-            
     2                     GROWYR(indx,1,NL))                                   
              IF(DAY.GE.GROWYR(indx,3,NL).AND.DAY.LT.GROWYR(indx,4,NL))           
     1            GROWA(I)=(GROWYR(indx,4,NL)-DAY)/(GROWYR(indx,4,NL)-            
     2                     GROWYR(indx,3,NL))                                   
...
C
          RRESID(I)=0.0
          IF(rncan(I).LT.1.0E-5 .OR. (FC(I)+FCS(I)).LT.1.0E-5) THEN
              RRESID(I)=RRESID(I)+rncan(I)
              rncan(I)=0.0
          ENDIF
C
          IF(rncan(I).GT.0. .AND. (FC(I)+FCS(I)).GT.0.)        THEN                
              rncan(I)=rncan(I)/(FC(I)+FCS(I))                                      
              IF(PAICAN(I).GT.0.0)                 THEN                           
                  RAICAN(I)=rncan(I)*(FC(I)+FCS(I))/(FC(I)+FCS(I)*                 
     1                      PAICNS(I)/PAICAN(I))                                  
              ELSE                                                                
                  RAICAN(I)=0.0                                                   
              ENDIF                                                               
              IF(PAICNS(I).GT.0.0)                 THEN                           
                  RAICNS(I)=rncan(I)*(FC(I)+FCS(I))/(FCS(I)+FC(I)*                 
     1                      PAICAN(I)/PAICNS(I))                                  
              ELSE                                                               
                  RAICNS(I)=0.0                                                   
              ENDIF                                                               
