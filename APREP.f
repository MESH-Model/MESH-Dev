      SUBROUTINE APREP(FC,FG,FCS,FGS,AILCAN,AILCNS,FSVF,FSVFS, 
     1            FRAINC,FSNOWC,RAICAN,RAICNS,SNOCAN,SNOCNS,DISP,
     2            DISPS,ZOMLNC,ZOMLCS,ZOELNC,ZOELCS,ZOMLNG,ZOMLNS, 
     3            ZOELNG,ZOELNS,CHCAP,CHCAPS,CMASSC,CMASCS,CWLCAP,
     4            CWFCAP,CWLCPS,CWFCPS,RBCOEF,FROOT,ZPLIMC,ZPLIMG,
     5            ZPLMCS,ZPLMGS,HTCC,HTCS,HTC,WTRC,WTRS,WTRG,CMAI,
     6            AIL,AILS,FCAN,FCANS,PSIGND,
     7            FCANMX,ZOLN,AILMAX,AILMIN,CWGTMX,ZRTMAX,
     8            AILDAT,HGTDAT,THLIQ,THICE,TBAR,RCAN,SNCAN,
...
C                                                                                 
     ...
     6     RBCOEF(ILG),   CHCAP (ILG),   CHCAPS(ILG),   
     7     CMASSC(ILG),   CMASCS(ILG),   CWLCAP(ILG),   CWFCAP(ILG),   
     8     CWLCPS(ILG),   CWFCPS(ILG),   ZPLIMC(ILG),   ZPLIMG(ILG),   
     9     ZPLMCS(ILG),   ZPLMGS(ILG),   HTCC  (ILG),   HTCS  (ILG),   
     A     WTRC  (ILG),   WTRS  (ILG),   WTRG  (ILG),   CMAI  (ILG)  
C                                                                                 
...
C                                                                                 
     ...
     2                   FCS(I)                                                   
          ...
          ENDIF                                                                   
C                                                                                 
          CWLCAP(I)=0.20*AILCAN(I)                                                
          CWLCPS(I)=0.20*AILCNS(I)                                                
C
          RRESID(I)=0.0
...
C                                                                                 
          IF(FC(I).GT.0.)                                     THEN                
              AILCAN(I)=(0.7*FCAN(I,1)*AIL(I,1)+FCAN(I,2)*AIL(I,2)+                   
     1                   FCAN(I,3)*AIL(I,3)+FCAN(I,4)*AIL(I,4))/FC(I)             
          ELSE                                                                    
              AILCAN(I)=0.0                                                       
          ENDIF                                                                   
          IF(FCS(I).GT.0.)                                    THEN                
              AILCNS(I)=(0.7*FCANS(I,1)*AILS(I,1)+FCANS(I,2)*AILS(I,2)+               
     1                   FCANS(I,3)*AILS(I,3)+FCANS(I,4)*AILS(I,4))/              
     2                   FCS(I)                                                   
          ELSE                                                                    
              AILCNS(I)=0.0                                                       
          ENDIF                                                                   
C
          CWFCAP(I)=6.0*AILCAN(I)*(0.27+46.0/RHOSNI(I))
          CWFCPS(I)=6.0*AILCNS(I)*(0.27+46.0/RHOSNI(I))
C
          SRESID(I)=0.0
...
C                                                                                 
          ...
          ELSEIF(CWFCPS(I).GT.0.0)                              THEN
              FSNOWC(I)=MIN(SNOCNS(I)/CWFCPS(I),1.0)
          ELSE
              FSNOWC(I)=0.0
          ENDIF
C
          ...
          ELSEIF(CWLCPS(I).GT.0.0)                              THEN
              FRAINC(I)=MIN(RAICNS(I)/CWLCPS(I),1.0)
          ELSE                                                                    
              FRAINC(I)=0.0                                                       
          ENDIF                                                                   
...
C
          IF((FC(I)+FCS(I)).GT.0.)                               THEN             
              RBCOEF(I)=RBCOEF(I)+
     1                  (FCAN(I,J)*XLEAF(J)*(SQRT(AIL(I,J))/0.75)*
     2                  (1.0-EXP(-0.75*SQRT(AIL(I,J))))+
     3                  FCANS(I,J)*XLEAF(J)*(SQRT(AILS(I,J))/0.75)*
     4                  (1.0-EXP(-0.75*SQRT(AILS(I,J)))))/
     5                  (FC(I)+FCS(I))                                          
          ENDIF                                                                   
  450 CONTINUE                                                                    
C                                                                                 
...
C                                                                                 
  ...
  700 CONTINUE                                                                    
C 
C     * RECALCULATE EFFECTIVE LEAF AREA INDICES FOR TRANSPIRATION.
C
      DO 800 I=IL1,IL2                                                            
          IF(FC(I).GT.0.)                                     THEN                
              AILCAN(I)=(FCAN(I,1)*AIL(I,1)+FCAN(I,2)*AIL(I,2)+                   
