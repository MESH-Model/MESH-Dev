...
C     * OPEN FILES FOR READING AND WRITING.
     ...
     +           FORM='UNFORMATTED')
      ...
      OPEN(UNIT=53,FILE='D100.ini',STATUS='OLD')
      OPEN(UNIT=70,FILE='MESH_output_streamflow.csv')
      OPEN(unit=84,file='basin_SCA.csv')
      OPEN(unit=85,file='basin_SCA_alldays.csv')
...
c for now set all latitudes, longitudes, and references the same for all grids
	...
	ZBLDGRD(i)=ZBLDGRD(1)
      ...
      enddo
	      ...
	      I=1
      DO 55 M=1,NMTEST
c read D100 variables from d100.ini file
          READ(53,5340) ZSNLROW(I,M)
55    CONTINUE
...
c  set apprpriate values for all D100's in the basin	
...
c new format statement for D100 variable
5340  FORMAT(F6.4)
5050  FORMAT(6F10.2)
5060  FORMAT(7F10.3)
...
c            for a given day - IDAY in the if statement
      ...
      IF((IHOUR.EQ.12).AND.(IMIN.EQ.0))  THEN
          ...
          do I=1,NLTEST
             if(fudge_factor(I).NE.0.0) then
                basin_SCA = basin_SCA + FSNOGRD(I)/fudge_factor(I) ! not generally applicable
                continue
             endif
          enddo

         basin_SCA = basin_SCA/84.0

      ENDIF

c also write basin average SCA for key dates.
      IF((IDAY.EQ.144.OR.
     +    IDAY.EQ.146.OR.
     +    IDAY.EQ.149.OR.
     +    IDAY.EQ.153.OR.
     +    IDAY.EQ.157.OR.
     +    IDAY.EQ.160).AND.
     +    (IHOUR.EQ.12).AND.(IMIN.EQ.0))  THEN

             write(84,8400) IDAY, basin_SCA

      ENDIF

      IF ((IHOUR.EQ.12).AND.(IMIN.EQ.0))  THEN

             write(85,8400) IDAY, basin_SCA
      ENDIF

8400  format(I5, ',', F10.3)
...
875   FORMAT(2X,10X,6I10)
	...
	close(unit=53)
      close(unit=70)
      close(unit=84) !only for watclass3.0f5
