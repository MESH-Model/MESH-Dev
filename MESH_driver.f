...
6600  FORMAT((I2,','),(I3,','),(I5,','),2(F10.2,','),(F12.3,',')
     +,(F10.2,','),(F8.2,','),(F10.2,','),(F12.3,','))
...
c write out final totals to screen
     ...
     +        TOTAL_PREACC/REAL(NLTEST)
         WRITE(6,'(A,F11.3)') '  Total Evaporation           (mm) = ',
     +        TOTAL_EVAPACC/REAL(NLTEST)
         WRITE(6,'(A,F11.3)') '  Total Runoff                (mm) = ',
     +        TOTAL_ROFACC/REAL(NLTEST)
         WRITE(6,'(A,3F11.3)') '  Storage(Change/Init/Final)  (mm) = ',
     +        (FINAL_STORE-INIT_STORE)/REAL(NLTEST),
     +        INIT_STORE/REAL(NLTEST),
     +        FINAL_STORE/REAL(NLTEST)
         WRITE(6,*)
         WRITE(6,'(A,F11.3)') '  Total Overland flow         (mm) = ',
     +        TOTAL_ROFOACC/REAL(NLTEST)
         WRITE(6,'(A,F11.3)') '  Total Interflow             (mm) = ',
     +        TOTAL_ROFSACC/REAL(NLTEST)
         WRITE(6,'(A,F11.3)') '  Total Baseflow              (mm) = ',
     +        TOTAL_ROFBACC/REAL(NLTEST)
         WRITE(6,*)
