      SUBROUTINE CLASSB(THPOR,THLRET,THLMIN,BI,PSISAT,ksat,
     ...
     6                  WC_PSISAT,WC_ksat,WC_HCPS,WC_TCS)
C
...
C
      ...
      INTEGER ILG,IL1,IL2,JL,N,NA,NTYPE
C
C     * OUTPUT ARRAYS.
C
      REAL THPOR (NL,NM,IG),  THLRET(NL,NM,IG),  THLMIN(NL,NM,IG),
     1     BI    (NL,NM,IG),  PSISAT(NL,NM,IG),  ksat(NL,NM,IG),  
...
C     * VARIABLES FOR SOIL.INI FILE	  
     ...
     2     WC_PSISAT(NL,NM,IG),WC_ksat(NL,NM,IG),
     ...
     1            THPOR(I,M,J))**(-BI(I,M,J))
          ELSEIF(SAND(I,M,J).GE.0) THEN
			...
			ELSE
              ...
              THLRET(I,M,J)=0.04
              THLMIN(I,M,J)=0.04
              BI    (I,M,J)=0.159*CLAY(I,M,J)/100.0+2.91
              PSISAT(I,M,J)=0.01*EXP(-0.0302*SAND(I,M,J)/100.+4.33)
              ksat(I,M,J)=7.0556E-6*(EXP(0.0352*SAND(I,M,J)/100.-2.035))
c              print *, "sand(i,m,j),bi(i,m,j),psisat(i,m,j),ksat(i,m,j)"
c             print *, sand(i,m,j),bi(i,m,j),psisat(i,m,j),ksat(i,m,j)
c             pause  "101"
     ...
     1            TCFINE*THFINE(I,M,J))/(1.0-THPOR(I,M,J))
			ENDIF
              THLRAT(I,M,J)=0.5**(1.0/(2.0*BI(I,M,J)+3.0))
              THFC(I,M,J)=THPOR(I,M,J)*(1.157E-9/ksat(I,M,J))**
...
ccc     1                (PSISAT(I,M,J)*BI(I,M,J)/SDEPTH(I,M))**
ccc     1                (PSISAT(I,M,J)*BI(I,M,J)*2.0*DD(I,M,j)/SDEPTH(I,M))**
     2                (1.0/BI(I,M,J))
              ENDIF
