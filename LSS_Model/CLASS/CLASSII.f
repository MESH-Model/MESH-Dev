      SUBROUTINE CLASSII(VPD,TADP,PADRY,RHOAIR,RHOSNI,
     1                  RPCP,TRPCP,SPCP,TSPCP,
     2                  TA,QA,PCPR,RRATE,SRATE,PRESSG,
     3                  IPCP,ILG,IL1,IL2,ZRFH,ILMOS,NA)
C
C     * OCT 27/11 - M.MACDONALD.CLASSII FOR GRU-LEVEL CALCULATIONS.
...
C     * INTEGER CONSTANTS.
C
      INTEGER IPCP,ILG,IL1,IL2,I,NA
C
C     * OUTPUT ARRAYS.
C
      REAL VPD   (ILG),  TADP  (ILG),  PADRY (ILG),  RHOAIR(ILG),
     1     RHOSNI(ILG),  RPCP  (ILG),  TRPCP (ILG),  
     2     SPCP  (ILG),  TSPCP (ILG)  
C
C     * INPUT ARRAYS.
C
      REAL TA    (ILG),  QA    (ILG),  PRESSG(ILG), 
     1     PCPR  (ILG),  RRATE (ILG),  SRATE (ILG),
     2     ZRFH (ILG)
C
C     * WORK ARRAYS.
C
...
C
      DO 100 I=IL1,IL2
        IF (I<18) THEN !Dirty fix to work for Matt's Marmot case
          IF(ZRFH(ILMOS(I))>2.) THEN !Matt: use Ta,pot @ ~2m for calculations
           TA(I)=TA(I)+(ZRFH(ILMOS(I))-2.)*GRAV/1004.64
          ELSEIF(ZRFH(ILMOS(I))<10.) THEN
           TA(I)=TA(I)-(2.-ZRFH(ILMOS(I)))*GRAV/1004.64
          ENDIF
