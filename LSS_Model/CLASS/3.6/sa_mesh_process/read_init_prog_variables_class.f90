...
!> 5)   RCAN        - Intercepted liquid water stored on canopy [kg m-2]
!> 6)   RHOS        - Density of snow [kg m-3]
!> 7)   SCAN/SNCAN  - Intercepted frozen water stored on canopy [kg m-2]
!> 8)   SNO         - Mass of snow pack [kg m-2]
!> 9)   TAC         - Temp of air within vegetation canopy [K]
!> 10)  TBAR        - Temp of soil layers [K]
!> 11)  TBAS        - Temp of bedrock in third soil layer [K]
!> 12)  TCAN        - Temp vegetation canopy [K]
!> 13)  THIC        - Vol. frozen water content of soil layers [m3 m-3]
!> 14)  THLQ        - Vol. liquid water content of soil layers [m3 m-3]
...
subroutine read_init_prog_variables_class(fls, shd)
    ...
    resume_fn = trim(adjustl(fls%fl(mfk%f883)%fn)) // '.runclass36'
    open(iun, file = resume_fn, status = 'old', action = 'read', form = 'unformatted', access = 'sequential', iostat = ierr)
    if (ierr /= 0) then
    ...
    !> Read resume values from the file.
    read(iun) ALBS      !1 (NML)
    read(iun) CMAI      !2 (NML)
    read(iun) GRO       !3 (NML)
    read(iun) QAC       !4 (NML)
