...
!>    end type
        ...
        read(iun) vs%tile%albsno    !1 (NML)
        read(iun) vs%tile%cmas      !2 (NML)
        read(iun) vs%tile%gro       !3 (NML)
        read(iun) vs%tile%qacan     !4 (NML)
        read(iun) vs%tile%lqwscan   !5 (NML)
        read(iun) vs%tile%rhosno    !6 (NML)
        read(iun) vs%tile%fzwscan   !7 (NML)
        read(iun) vs%tile%sno       !8 (NML)
        read(iun) vs%tile%tacan     !9 (NML)
        read(iun) vs%tile%tsol      !10 (NML, NSL)
        read(iun) vs%tile%tbas      !11 (NML)
        read(iun) vs%tile%tcan      !12 (NML)
        read(iun) vs%tile%thicsol   !13 (NML, NSL)
        read(iun) vs%tile%thlqsol   !14 (NML, NSL)
        read(iun) vs%tile%tpnd      !15 (NML)
        read(iun) vs%tile%tsfs      !16 (NML, 4)
        read(iun) vs%tile%tsno      !17 (NML)
        read(iun) vs%tile%lqwssno   !18 (NML)
