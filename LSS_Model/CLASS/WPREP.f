     ...
     U                 BI, PSISAT, DD, XSLOPE, BULK_FC,q  )
C
...
C
     ...
     1     FSVF  (ILG),   FSVFS (ILG),   RAICAN(ILG),  SNOCAN(ILG,Nmod),   
     2     RAICNS(ILG),  SNOCNS(ILG,Nmod),   EVAPC (ILG),   EVAPCG(ILG),
     3     EVAPG (ILG),   EVAPCS(ILG),   EVPCSG(ILG),   EVAPGS(ILG),
     4     RPCP  (ILG),   TRPCP (ILG),   SPCP  (ILG),   TSPCP (ILG),
     5     RHOSNI(ILG,Nmod),   ZPOND (ILG,Nmod),   ZSNOW (ILG,Nmod),   
     +     ALBSNO(ILG,Nmod),WSNOCS(ILG),WSNOGS(ILG),RHOSCS(ILG),
     7     RHOSGS(ILG),   TBASE (ILG,Nmod),   TSURX(ILG,4,Nmod)
C
...
!$omp parallel do
      DO 100 I=IL1,IL2
          ...
          TRNFGS(I)=0.0
          TRUNOF(I)=0.0
          TBASC (I)=TBASE(I,q)-TFREZ
          TBASG (I)=TBASE(I,q)-TFREZ
          TBASCS(I)=TBASE(I,q)-TFREZ
          TBASGS(I)=TBASE(I,q)-TFREZ
          DT    (I)=DELT
          RDUMMY(I)=0.
          ZERO  (I)=0.
          IZERO (I)=0 
...
C
          IF(RPCP(I).GT.0.)                                      THEN 
              PCLC(I)=(FCS(I,q)*(1.0-FSVFS(I))+FC(I,q)*(1.0-FSVF(I)))*
     1                RPCP(I)*RHOW
              PCPN(I)=(FCS(I,q)*FSVFS(I)+FGS(I,q))*RPCP(I)*RHOW
              PCPG(I)=(FC(I,q)*FSVF(I)+FG(I,q))*RPCP(I)*RHOW
          ENDIF
C
          IF(SPCP(I).GT.0.)                                      THEN 
              PCFC(I)=(FCS(I,q)*(1.0-FSVFS(I))+FC(I,q)*(1.0-FSVF(I)))*
     1                SPCP(I)*RHOSNI(I,q)
              PCPN(I)=PCPN(I)+(FCS(I,q)*FSVFS(I)+FGS(I,q)+
     1                FC(I,q)*FSVF(I)+FG(I,q))*SPCP(I)*RHOSNI(I,q)
          ENDIF
  100 CONTINUE
C
          ...
          DO 250 I=IL1,IL2
              IF(FCS(I,q).GT.0.)                           THEN  
                  ...
                  IF(SUBLCS(I).GT.0.0) THEN
                      QFCF(I)=QFCF(I)+FCS(I,q)*SUBLCS(I)*RHOW
                  ELSE
                      QFCF(I)=QFCF(I)+FCS(I,q)*(1.0-FSVFS(I))*
     1                        SUBLCS(I)*RHOW
                      QFN(I)=QFN(I)+FCS(I,q)*FSVFS(I)*SUBLCS(I)*RHOW
                  ENDIF
                  IF(EVAPCS(I).GT.0.0) THEN
                      QFCL(I)=QFCL(I)+FCS(I,q)*EVAPCS(I)*RHOW
                  ELSE
                      QFCL(I)=QFCL(I)+FCS(I,q)*(1.0-FSVFS(I))*
     1                        EVAPCS(I)*RHOW
                      QFN(I)=QFN(I)+FCS(I,q)*FSVFS(I)*EVAPCS(I)*RHOW
                  ENDIF
C
                  IF(SPCP(I).GT.0. .OR. SUBLCS(I).LT.0.) THEN                                      
                      SADD(I)=SPCP(I)-SUBLCS(I)*RHOW/RHOSNI(I,q)
                      IF(ABS(SADD(I)).LT.1.0E-12) SADD(I)=0.0
                      IF(SADD(I).GT.0.0) THEN                                                
                          IF(SUBLCS(I).GT.0.) THEN
                              QFCF(I)=QFCF(I)-FCS(I,q)*FSVFS(I)*
     1                                SUBLCS(I)*RHOW
                              QFN(I)=QFN(I)+FCS(I,q)*FSVFS(I)*
     1                                SUBLCS(I)*RHOW
                          ENDIF
                          SPCCS (I)=SADD(I)                                                        
                          ...
                          ELSE
                              TSPCCS(I)=MIN(TSURX(I,1,q),TFREZ)
                          ENDIF
                          SUBLCS(I)=0.0                                                      
                      ELSE                                                                
                          PCPN(I)=PCPN(I)-FCS(I,q)*FSVFS(I)*SPCP(I)*
     1                        RHOSNI(I,q)
                          PCFC(I)=PCFC(I)+FCS(I,q)*FSVFS(I)*SPCP(I)*
     1                        RHOSNI(I,q)
                          SUBLCS(I)=-SADD(I)*RHOSNI(I,q)/RHOW                                        
                          SPCCS (I)=0.0                                                         
                          TSPCCS(I)=0.0                                                        
                      ENDIF                                                               
                  ELSE                                                                    
...
C
     ...
     1                                EVAPCS(I)*RHOW
                          ...
                          ELSE
                              TRPCCS(I)=MAX(TSURX(I,1,q),TFREZ)
                          ENDIF
                          EVAPCS(I)=0.0                                                      
                      ELSE                                                                
                          PCPN(I)=PCPN(I)-FCS(I,q)*FSVFS(I)*RPCP(I)
     +                              *RHOW
                          PCLC(I)=PCLC(I)+FCS(I,q)*FSVFS(I)*RPCP(I)
     +                              *RHOW
                          EVAPCS(I)=-RADD(I)                                                    
                          RPCCS (I)=0.0                                                         
                          TRPCCS(I)=0.0                                                        
     ...
     1                      (RHOW*ZSNOCS(I))
                  QFN   (I)=QFN(I)+FCS(I,q)*EVPCSG(I)*RHOW
              ENDIF
  250     CONTINUE
      ENDIF
C
...
!$omp parallel do
          DO 350 I=IL1,IL2
              IF(FGS(I,q).GT.0.)                              THEN 
                  QFN(I)=QFN(I)+FGS(I,q)*EVAPGS(I)*RHOW
                  IF(SPCP(I).GT.0. .OR. EVAPGS(I).LT.0.) THEN                                      
                      SADD(I)=SPCP(I)-EVAPGS(I)*RHOW/RHOSNI(I,q)
                      IF(ABS(SADD(I)).LT.1.0E-12) SADD(I)=0.0
                      IF(SADD(I).GT.0.0) THEN                                                
                          SPCGS (I)=SADD(I)                                                        
                          IF(SPCP(I).GT.0.0) THEN
                              TSPCGS(I)=TSPCP(I)
                          ELSE
                              TSPCGS(I)=MIN((TSURX(I,2,q)-TFREZ),0.0)
                          ENDIF
                          EVAPGS(I)=0.0                                                      
                      ELSE                                                                
                          EVAPGS(I)=-SADD(I)*RHOSNI(I,q)/RHOW                                        
                          SPCGS (I)=0.0                                                         
                          TSPCGS(I)=0.0                                                        
                      ENDIF                                                               
                  ELSE                                                                    
          ...
          DO 450 I=IL1,IL2
              IF(FC(I,q).GT.0.)                            THEN 
                  ...
                  IF(SUBLC(I).GT.0.0) THEN
                      QFCF(I)=QFCF(I)+FC(I,q)*SUBLC(I)*RHOW
                  ELSE
                      QFCF(I)=QFCF(I)+FC(I,q)*(1.0-FSVF(I))*SUBLC(I)*
     1                        RHOW
                      QFN(I)=QFN(I)+FC(I,q)*FSVF(I)*SUBLC(I)*RHOW
                  ENDIF
                  IF(EVAPC(I).GT.0.0) THEN
                      QFCL(I)=QFCL(I)+FC(I,q)*EVAPC(I)*RHOW
                  ELSE
                      QFCL(I)=QFCL(I)+FC(I,q)*(1.0-FSVF(I))*EVAPC(I)*
     1                        RHOW
                      QFG(I)=QFG(I)+FC(I,q)*FSVF(I)*EVAPC(I)*RHOW
                  ENDIF
C
                  IF(SPCP(I).GT.0. .OR. SUBLC(I).LT.0.)  THEN                                      
                      SADD(I)=SPCP(I)-SUBLC(I)*RHOW/RHOSNI(I,q)
                      IF(ABS(SADD(I)).LT.1.0E-12) SADD(I)=0.0
                      IF(SADD(I).GT.0.0) THEN                                                
                          IF(SUBLC(I).GT.0.) THEN
                              QFCF(I)=QFCF(I)-FC(I,q)*FSVF(I)*
     1                                SUBLC(I)*RHOW
                              QFN(I)=QFN(I)+FC(I,q)*FSVF(I)*SUBLC(I)
     1                                *RHOW
                          ENDIF
                          SPCC  (I)=SADD(I)                                                        
                          IF(SPCP(I).GT.0.0) THEN
                          ...
                          ELSE
                             TSPCC(I)=MIN(TSURX(I,3,q),TFREZ)
                          ENDIF
                          SUBLC (I)=0.0                                                      
                      ELSE                                                                
                          PCPN(I)=PCPN(I)-FC(I,q)*FSVF(I)*SPCP(I)*
     1                        RHOSNI(I,q)
                          PCFC(I)=PCFC(I)+FC(I,q)*FSVF(I)*SPCP(I)*
     1                        RHOSNI(I,q)
                          SUBLC (I)=-SADD(I)*RHOSNI(I,q)/RHOW                                        
                          SPCC  (I)=0.0                                                         
                          TSPCC (I)=0.0                                                        
                      ENDIF                                                               
                  ELSE                                                                    
                  ...
                  IF(RPCP(I).GT.0. .OR. EVAPC(I).LT.0.)  THEN                                      
                      ...
                      IF(RADD(I).GT.0.)   THEN                                                
                          IF(EVAPC(I).GT.0.) THEN
                              QFCL(I)=QFCL(I)-FC(I,q)*FSVF(I)*
     1                                EVAPC(I)*RHOW
                              QFG(I)=QFG(I)+FC(I,q)*FSVF(I)*EVAPC(I)*
     1                                RHOW
                          ENDIF
                          RPCC  (I)=RADD(I)                                                        
                          IF(RPCP(I).GT.0.0) THEN
                          ...
                          ELSE
                              TRPCC (I)=MAX(TSURX(I,3,q),TFREZ)
                          ENDIF
                          EVAPC (I)=0.0                                                      
                      ELSE   
                          PCPG(I)=PCPG(I)-FC(I,q)*FSVF(I)*RPCP(I)*
     +                              RHOW
                          PCLC(I)=PCLC(I)+FC(I,q)*FSVF(I)*RPCP(I)*
     +                              RHOW
                          EVAPC (I)=-RADD(I)                                                    
                          RPCC  (I)=0.0                                                         
                          TRPCC (I)=0.0                                                        
                  ...
                  RHOSC (I)=0.
                  HCPSC (I)=0.
                  QFG   (I)=QFG(I)+FC(I,q)*EVAPCG(I)*RHOW
              ENDIF
  450     CONTINUE
      ENDIF
C
...
!$omp parallel do
          DO 550 I=IL1,IL2
              IF(FG(I,q).GT.0.)                              THEN 
                  QFG(I)=QFG(I)+FG(I,q)*EVAPG(I)*RHOW
                  IF(SPCP(I).GT.0.)                 THEN                                      
                      SADD(I)=SPCP(I)-EVAPG(I)*RHOW/RHOSNI(I,q)
                      IF(ABS(SADD(I)).LT.1.0E-12) SADD(I)=0.0
                      IF(SADD(I).GT.0.0) THEN                                                
                          QFN(I)=QFN(I)+FG(I,q)*EVAPG(I)*RHOW
                          QFG(I)=QFG(I)-FG(I,q)*EVAPG(I)*RHOW
                          SPCG  (I)=SADD(I)                                                        
                          TSPCG (I)=TSPCP(I)
                          EVAPG (I)=0.0                                                      
                      ELSE                                                                
                          PCPN(I)=PCPN(I)-FG(I,q)*SPCP(I)*
     +                              RHOSNI(I,q)
                          PCPG(I)=PCPG(I)+FG(I,q)*SPCP(I)*
     +                              RHOSNI(I,q)
                          EVAPG (I)=-SADD(I)*RHOSNI(I,q)/RHOW                                        
                          SPCG  (I)=0.0                                                         
                          TSPCG (I)=0.0                                                        
                      ENDIF                                                               
                  ELSE                                                                    
                  ...
                  IF(RPCP(I).GT.0. .OR. EVAPG(I).LT.0.)   THEN                                      
                      ...
                      IF(RADD(I).GT.0.)   THEN                                                
                          ...
                          IF(RPCP(I).GT.0.0) THEN
                              TRPCG (I)=TRPCP(I)
                          ELSE
                              TRPCG (I)=MAX((TSURX(I,4,q)-TFREZ),0.0)
                          ENDIF
                          EVAPG (I)=0.0                                                      
                      ELSE                                                                
                          EVAPG (I)=-RADD(I)                                                    
