...
C     * INPUT ARRAYS.
C
      REAL ZSNOW (ILG),  TCSNOW(ILG), HCPSNO(ILG),   FI(ILG)
...
C
      DO 100 I=IL1,IL2
          IF(FI(I).GT.0. .AND. HB(I).GT.0.)                       THEN
              !GZERO(I): positive into snowpackK
              HA=FS(I)*HB(I)*(1-PS(I))/PS(I)
              HFSS(I)=HFSS(I)-FI(I)*HA !check +-
              TSNOW(I)=TSNOW(I)+HA*DELT/(HCPSNO(I)*ZSNOW(I)) !check +-
