      SUBROUTINE TNPOSTlinear(TBARPR,G12,G23,TPOND,GZERO,QFREZG,GCONST,
...
C
      REAL ZPOND (ILG,Nmod), TSURF (ILG), TBASE (ILG,Nmod),TBAR1P(ILG),
     1     A1    (ILG),    A2    (ILG),    B1    (ILG),    B2    (ILG),    
     2     C2    (ILG),    FI    (ILG,Nmod),    GCONST(ILG),GCOEFF(ILG)
C
...
!     1               C2(I)                                    
     ...
     1                     (ZPOND(I,q)*ZPOND(I,q)-DELZ1*DELZ1)/
     2                     (6.0*DELZ1)-GZROLD*(ZPOND(I,q)-DELZ1)/
     3                     (2.0*TCTOP(I,1))+TBAR1P(I)-TFREZ
                  TBARPR(I,1)=((HCP(I,1)*DELZW(I,1)+HCPSND*(DELZ(1)-
     1                        DELZW(I,1))+HCPW*ZPOND(I,q))*TBAR1P(I)-
     2                        HCPW*ZPOND(I,q)*(TPOND(I)+TFREZ))/
     3                        (HCP(I,1)*DELZW(I,1)+HCPSND*(DELZ(1)-
     4                        DELZW(I,1)))-TFREZ
              ELSE                                                                        
                  TPOND(I)=0.                                                               
                  TBARPR(I,1)=TBAR(I,1,q)-TFREZ                                             
              ENDIF           
C
...
C 
      DO 200 I=IL1,IL2
          IF(FI(I,q).GT.0.)                                         THEN
              TBARPR(I,2)=TBAR(I,2,q)-TFREZ
              IF(DELZW(I,3).GT.0.0 .AND. DELZW(I,3).LT.DELZ(3)
     1                             .AND. IG.EQ.3)              THEN
                  TBARPR(I,3)=(TBAR(I,3,q)*(HCP(I,3)*DELZW(I,3)+
     1                         HCPSND*(DELZ(3)-DELZW(I,3)))-TBASE(I,q)*
     2                         HCPSND*(DELZ(3)-DELZW(I,3)))/(HCP(I,3)*
     3                         DELZW(I,3))-TFREZ
              ELSE
                  DO 150 J=3,IG
                      TBARPR(I,J)=TBAR(I,J,q)-TFREZ
  150             CONTINUE
              ENDIF
          ENDIF
  200 CONTINUE                                                                    
