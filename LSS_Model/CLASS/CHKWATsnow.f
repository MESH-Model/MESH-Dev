...
C     * INTEGER CONSTANTS.
C
      INTEGER ISFC,IG,ILG,IL1,IL2,JL,I,J,K,q
C
      ...
      DO 100 I=IL1,IL2
          IF(FI(I,q).GT.0. .AND. ISAND(I,1).GT.-4)                  THEN
              IF(ISFC.EQ.1 .OR. ISFC.EQ.3)                   THEN      
                  ...
                  IF(SNOCAN(I,q).LT.(-1.0*ACCLMT)) JPTBAD=I
              ENDIF
...
C
      IF(ISFC.EQ.1 .OR. ISFC.EQ.3)                                  THEN      
          IF(IPTBAD.NE.0)                                    THEN
             WRITE(6,6100) IPTBAD,JL,ISFC,RAICAN(IPTBAD),q
 6100        FORMAT('0AT (I,JL)=(',I3,',',I3,'), ISFC=',I2,' RAICAN = ',
     1               E13.5,'q = ', I5)
             CALL XIT('CHKWAT',-1)
          ENDIF
          IF(JPTBAD.NE.0)                                           THEN
             WRITE(6,6150) JPTBAD,JL,ISFC,SNOCAN(JPTBAD,q),q
 6150        FORMAT('0AT (I,JL)=(',I3,',',I3,'), ISFC=',I2,' SNOCAN = ',
     1               E13.5,'q = ', I5)
      ...
      ENDIF
      IF(KPTBAD.NE.0)                                           THEN
         WRITE(6,6200) KPTBAD,JL,ISFC,RUNOFF(KPTBAD),q
...
C
     ...
     1                      ZPOND(I),ZPONDI(I,q),ISAND(I,J)
...
C
     ...
     1                  THPOR(IPTBDI,IPTBDJ),IPTBDJ,q
 6250 FORMAT('0AT (I,JL)=(',I6,',',I6,'), ISFC=',I2,' STEP =',I8,
     ...
     +            'q = ', I5)
      ...
      ENDIF
      IF(JPTBDI.NE.0)                                               THEN
          WRITE(6,6300) JPTBDI,JL,ISFC,THLIQ(JPTBDI,JPTBDJ),JPTBDJ,q
 6300     FORMAT('0AT (I,JL)=(',I3,',',I3,'), ISFC=',I2,' THLIQ = ',
     1            E13.5,' FOR J=',I2,'q = ', I5)
          CALL XIT('CHKWAT',-5)
      ENDIF
      IF(KPTBDI.NE.0)                                               THEN
          WRITE(6,6350) KPTBDI,JL,ISFC,THICE(KPTBDI,KPTBDJ),
     1                  THPOR(KPTBDI,KPTBDJ),KPTBDJ,q
 6350     FORMAT('0AT (I,JL)=(',I3,',',I3,'), ISFC=',I2,' THICE = ',
     1            E13.5,' THPOR = ',E13.5,' FOR J=',I2,'q = ', I5)
          CALL XIT('CHKWAT',-6)
      ENDIF
      IF(LPTBDI.NE.0)                                               THEN
          WRITE(6,6400) LPTBDI,JL,ISFC,THICE(LPTBDI,LPTBDJ),LPTBDJ,q
...
C
      ...
      DO 300 I=IL1,IL2
          IF(FI(I,q).GT.0. .AND. ZSNOW(I).GT.0.) XSNOW(I)=1.0
          IF(FI(I,q).GT.0. .AND. ISAND(I,1).GT.-4)                  THEN
             IF(ISFC.EQ.1 .OR. ISFC.EQ.2)                 THEN
                SNOFAC=1.0/(FCS(I,q)+FGS(I,q))
             ELSE
                SNOFAC=0.0
             ENDIF
             BAL(I)=PCPR(I)*DELT-                                               
  ...
  300 CONTINUE
     ...
     3        SNOCAN(IPTBAD,q),(ZPOND(IPTBAD)-ZPONDI(IPTBAD,q))*RHOW
  ...
  350     CONTINUE
          WRITE(6,6470) FCS(IPTBAD,q),FGS(IPTBAD,q),q
6450      FORMAT('0AT (I,JL)=(',I8,',',I8,'),  TIME=',I8,' ISFC=',I2,
     1        ' BAL = ',E13.5)
6460      FORMAT(2X,8F15.8)
6470      FORMAT(2X,4E20.6,'q = ', I5)
          CALL XIT('CHKWAT',-8)
      ENDIF
