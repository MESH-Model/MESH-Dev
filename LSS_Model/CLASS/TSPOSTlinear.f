...
C
      DO 100 I=IL1,IL2
          IF(FI(I).GT.0.)                                         THEN
              ...
              IF(ZSNOW(I).GT.0.0) THEN !matt_new
              ...
              ENDIF !matt_new
              TCZERO=2/(ZSNOW(I)/TCSNOW(I)+DELZ(1)/TCTOP(I,1))
              DT=(GSNOW(I)+TCZERO*(TBAR(I,1) - TSNOW(I)))*DELT/ !matt_new SHOULD THIS BE TBAR1P??
     1            (HCPSNO(I)*ZSNOW(I) + 1*TCZERO*DELT) !matt_new
              GZERO(I)=TCZERO*(TSNOW(I)+0.5*DT-TBAR(I,1)) !matt_new CHECK THIS MULTIPLY BY -1
