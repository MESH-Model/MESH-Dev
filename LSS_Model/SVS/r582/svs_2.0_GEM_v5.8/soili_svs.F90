...
!-------------------------------------- LICENCE END --------------------------------------
      SUBROUTINE SOILI_SVS (WD, &
           ...
           CG, PSOILHCAPZ, PSOILCONDZ, PSNGRVL,  & 
           Z0H, ALGR, EMGR, PSNVH, PSNVHA,   &
           ALVA, LAIVA, CVPA, EVA, Z0HA, RGLA, STOMRA ,&  
           GAMVA, VGHEIGHT, CONDSLD, CONDDRY, N )
!
        use svs_configs
        USE MODD_CSTS_SNOWES,    ONLY : XCL, XCI, XRHOLW, XRHOLI, XCONDI, XCONDW
...
! Z0HA     average local roughness associated with exposed (no snow)
...
! 
      ...
      REAL RHOSNOW, RHOSNOWV, SNOWD, SNOWDV

      REAL PETIT
      DATA PETIT/1.E-7/

      real, dimension(n) :: a, b, cnoleaf, cva, laivp, lams, lamsv, &
                            psnground, psnlowveg, zcs, zcsv, z0hatmp
...
!                                           be put in a COMMON LAMI=XCONDI 
!
      LAMI   = 2.22
      CI     = 2.106E3
      DAY    = 86400.
...
!!$          PSNLOWVEG(I) = MIN (TANH( SNOWD(I)/(2.5*Z0MVL(I)*((RHOSNOW(I)/100.)**1.6)) ) , 1.0)
...
!!$          SNOWDV(I) = SVM(I)/RHOSNOWV(I)
!!$          PSNVH(I) = MIN (TANH( SNOWDV(I)/(2.5*0.05*((RHOSNOWV(I)/100.)**1.6)) ) , 1.0)
...
!           ancienne approche
            Z0HATMP(I) = ( VEGH(I)                    * LOG(Z0MVH(I))  &
                     + VEGL(I) * (1.-PSNGRVL(I))   * LOG(Z0MVL(I))) &     
                               / (VEGH(I)+VEGL(I)*(1.-PSNGRVL(I)))

            VGHEIGHT(I) =  10. *  EXP(Z0HATMP(I))
...
!               FOR FORCE-RESTORE IN VEGETATION 
!               ----------------------------------
...
!
!
!       6.      SOIL THERMAL PROPERTIES PROFILE using PL98
!               ------------------------------------------
!
!
       LOG_CONDI = LOG(XCONDI)
       LOG_CONDW = LOG(XCONDW)
...
!                        Kersten parameter for thermal conductivity
             ...
             SATDEG   = MAX(0.1, WD(I,K)/WSAT(I,K))       ! degree of saturation
