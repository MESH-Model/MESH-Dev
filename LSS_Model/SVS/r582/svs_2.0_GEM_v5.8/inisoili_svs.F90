...
!VV FIN MODIFICATION POUR MESH
   ...
   real, pointer, dimension(:,:) :: zbcoef, zclay, zconddry, zcondsld, zksat, zpsisat, zquartz, zrhosoil, zsand, zwfc, zwfcint, zwsat, zwwilt
...
! assign pointers
      ...
      zconddry   (1:ni,1:nl_svs) => f(conddry:(conddry+n2d-1))
      zcondsld   (1:ni,1:nl_svs) => f(condsld:(condsld+n2d-1))
      zksat      (1:ni,1:nl_svs) => f(ksat:(ksat+n2d-1))
      zpsisat    (1:ni,1:nl_svs) => f(psisat:(psisat+n2d-1))
      zquartz    (1:ni,1:nl_svs) => f(quartz:(quartz+n2d-1))
      zrhosoil   (1:ni,1:nl_svs) => f(rhosoil:(rhosoil+n2d-1))
...
!       Quartz content (ref : NL95 & PL98)): 
        zquartz(i,k)  = 0.038 + 0.0095*zsand(i,k)
...
!       ZGAMMAD(:,:)   = (1.0-WSAT(:,:))*XDRYWGHT  
        zrhosoil(i,k) = (1.0-zwsat(i,k))*2700.

!       Soil solids conductivity:
        if (zquartz(i,k).gt.0.20) then
           zcondsld(i,k) = (7.7**zquartz(i,k)) *  &
                           (2.0**(1.0-zquartz(i,k))) 
        endif
        if (zquartz(i,k).le.0.20) then
           zcondsld(i,k) = (7.7**zquartz(i,k)) *  &
                           (3.0**(1.0-zquartz(i,k)))
        endif

!       Soil dry conductivity:
        zconddry(i,k) = (0.135*zrhosoil(i,k) + 64.7) / &
                        (2700. - 0.947*zrhosoil(i,k))
