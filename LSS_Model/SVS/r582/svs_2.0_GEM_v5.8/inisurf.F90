...
#include <arch_specific.hf>
   ...
   ! SVS
   real, pointer, dimension(:) :: zdrainaf, zdraindens, zdrnden, zfvapliqaf, zlaivh, zlaivhen, zlaivl, zlaivlen, zqaf, zresagrv, zresavg, zslop, zslopen, zsnodpl,zsnodplen, zsnomaen, zsnval, zsnvalen, zsnvden, zsnvdp, zsnvdpen, zsnvma, zsnvmaen, zsnvro, ztaf, ztperm, ztpermen, zvegh, zveghen, zvegl, zveglen, zwsnv, zwsnven
   real, pointer, dimension(:,:) :: zhumoins, zrunofftotaf, zsnoage_es, zsnoage_esen, zsnoage_esv, zsnoage_esven, zsnogran1_es, zsnogran1_esen, zsnogran1_esv, zsnogran1_esven, zsnogran2_es,zsnogran2_esen, zsnogran2_esv, zsnogran2_esven, zsnoma_es, zsnoma_esen, zsnoma_esv, zsnoma_esven, zsnoden_es, zsnoden_esen, zsnoden_esv, zsnoden_esven, ztground, ztgrounden, ztpsoil, ztpsoilen, ztpsoilv, ztpsoilven, ztsnow, ztsnowen, ztsnowveg, ztsnowvegen, ztvege, ztvegeen, zwsnow_es, zwsnow_esen, zwsnow_esv, zwsnow_esven
...
#define MKPTR2D(NAME1,NAME2) nullify(NAME1); if (vd%NAME2%i > 0 .and. associated(busptr(vd%NAME2%i)%ptr)) NAME1(1:ni,1:vd%NAME2%mul*vd%NAME2%niveaux) => busptr(vd%NAME2%i)%ptr(:,trnch)
   ...
   MKPTR1D(ztaf,taf)
   MKPTR1D(ztperm,tperm)
   MKPTR1D(ztpermen,tpermen)
   ...
   MKPTR2D(zhumoins,humoins)
   MKPTR2D(zisoil,isoil)
   MKPTR2D(zisoilen,isoilen)
   MKPTR2D(zrunofftotaf,runofftotaf)
   MKPTR2D(zsand,sand)
   MKPTR2D(zsanden,sanden)
   MKPTR2D(zsnoage_es,snoage_es)
   MKPTR2D(zsnoage_esen,snoage_esen)
   MKPTR2D(zsnoage_esv,snoage_esv)
   MKPTR2D(zsnoage_esven,snoage_esven)
   MKPTR2D(zsnodp,snodp)
   MKPTR2D(zsnodpen,snodpen)
   MKPTR2D(zsnogran1_es,snogran1_es)
   MKPTR2D(zsnogran1_esen,snogran1_esen)
   MKPTR2D(zsnogran1_esv,snogran1_esv)
   MKPTR2D(zsnogran1_esven,snogran1_esven)
   MKPTR2D(zsnogran2_es,snogran2_es)
   MKPTR2D(zsnogran2_esen,snogran2_esen)
   MKPTR2D(zsnogran2_esv,snogran2_esv)
   MKPTR2D(zsnogran2_esven,snogran2_esven)
   MKPTR2D(zsnoma_es,snoma_es)
   MKPTR2D(zsnoma_esen,snoma_esen)
   MKPTR2D(zsnoma_esv,snoma_esv)
   MKPTR2D(zsnoma_esven,snoma_esven)
   MKPTR2D(zsnoden_es,snoden_es)
   MKPTR2D(zsnoden_esen,snoden_esen)
   MKPTR2D(zsnoden_esv,snoden_esv)
   MKPTR2D(zsnoden_esven,snoden_esven)
   ...
   MKPTR2D(ztpsoilen,tpsoilen)
   MKPTR2D(ztpsoilv,tpsoilv)
   MKPTR2D(ztpsoilven,tpsoilven)
   ...
   MKPTR2D(zwsnow_es,wsnow_es)
   MKPTR2D(zwsnow_esen,wsnow_esen)
   MKPTR2D(zwsnow_esv,wsnow_esv)
   MKPTR2D(zwsnow_esven,wsnow_esven)
         ...
         DO i=1,ni
            if (kount == 0) then
               Do k=1,nl_svs
                  ...
                  ztpsoil(i,k) = ztpsoilen(i,k)
                  ztpsoilv(i,k)= ztpsoilven(i,k)
...
!              Read in snow mass and snow depth, calc. densities in coherence
               ...
               ! Modification for snowES
               do k=1,nsl
                 zsnoma_es (i,k)  = zsnoma_esen(i,k)
                 zsnoage_es(i,k)  = zsnoage_esen(i,k)
                 zsnogran1_es(i,k)  = zsnogran1_esen(i,k)
                 zsnogran2_es(i,k)  = zsnogran2_esen(i,k)
                 zsnoden_es (i,k)  = zsnoden_esen(i,k)
                 zsnoden_es (i,k)  = zsnoden_esen(i,k)
                 ztsnow(i,k)      = ztsnowen(i,k)
!                 ztsnow_es (i,k)  = ztsnow_esen(i,k)
                 zwsnow_es (i,k)  = zwsnow_esen(i,k)

                 zsnoma_esv (i,k)  = zsnoma_esven(i,k)
                 zsnoage_esv(i,k)  = zsnoage_esven(i,k)
                 zsnogran1_esv(i,k)  = zsnogran1_esven(i,k)
                 zsnogran2_esv(i,k)  = zsnogran2_esven(i,k)
                 zsnoden_esv (i,k)  = zsnoden_esven(i,k)
                 zsnoden_esv (i,k)  = zsnoden_esven(i,k)
                 ztsnowveg(i,k)    = ztsnowvegen(i,k)
!                 ztsnow_esv (i,k)  = ztsnow_esven(i,k)
                 zwsnow_esv (i,k)  = zwsnow_esven(i,k)
               enddo
