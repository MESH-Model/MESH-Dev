...
subroutine inisoili_svs_gsde(ni, trnch)
   use sfcbus_mod
   use svs_configs
   implicit none
#include <arch_specific.hf>
   ...
   real, dimension(ni,nl_gsde) :: wsat_gsde, wwilt_gsde, wfc_gsde, b_gsde, psisat_gsde, &
           ...
           quartz_gsde, rhosoil_gsde, condsld_gsde, conddry_gsde
...
!     Computer soil properties for GSDE layers
   do i=1,ni
      do k=1,nl_gsde
         ...
         quartz_gsde(i,k)  =  0.038 + 0.0095*zsand(i,k)
        ...
        rhosoil_gsde(i,k) = (1.0-wsat_gsde(i,k))*2700.

        ! Soil solids conductivity
        if (quartz_gsde(i,k).gt.0.20) then
           condsld_gsde(i,k) = (7.7**quartz_gsde(i,k)) * (2.0**(1.0-quartz_gsde(i,k)))  
        endif
        if (quartz_gsde(i,k).le.0.20) then
           condsld_gsde(i,k) = (7.7**quartz_gsde(i,k)) * (3.0**(1.0-quartz_gsde(i,k)))  
        endif
        
        ! Soil dry conductivity
        conddry_gsde(i,k) = (0.135*rhosoil_gsde(i,k) + 64.7) /  &
                            (2700. - 0.947*rhosoil_gsde(i,k))  
   ...
   Do i = 1 , ni
         Do k = 1, nl_svs
            do kk = 1 , nl_gsde
               ...
               zwsat   (i,k)  = zwsat   (i,k) + wsat_gsde  (i,k)  * weights( k , kk)
               zwwilt  (i,k)  = zwwilt  (i,k) + wwilt_gsde (i,k)  * weights( k , kk)
             
               zwfc    (i,k)  = zwfc    (i,k) + wfc_gsde   (i,k)  * weights( k , kk)
               zbcoef  (i,k)  = zbcoef  (i,k) + b_gsde     (i,k)  * weights( k , kk)
               zfbcof  (i,k)  = zfbcof  (i,k) + fb_gsde    (i,k)  * weights( k , kk)
               zpsisat (i,k)  = zpsisat (i,k) + psisat_gsde(i,k)  * weights( k , kk)
               zksat   (i,k)  = zksat   (i,k) + ksat_gsde  (i,k)  * weights( k , kk)
               zwfcint (i,k)  = zwfcint (i,k) + wfcint_gsde(i,k)  * weights( k , kk)
               
               zquartz (i,k)  = zquartz (i,k) + quartz_gsde (i,k) * weights( k , kk)
               zrhosoil(i,k)  = zrhosoil(i,k) + rhosoil_gsde(i,k) * weights( k , kk)
               zcondsld(i,k)  = zcondsld(i,k) + condsld_gsde(i,k) * weights( k , kk) 
               zconddry(i,k)  = zconddry(i,k) + conddry_gsde(i,k) * weights( k , kk)
