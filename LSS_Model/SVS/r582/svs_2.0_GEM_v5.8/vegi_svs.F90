...
!-------------------------------------- LICENCE END --------------------------------------
      SUBROUTINE VEGI_SVS ( RG, T, TVEG, HU, PS, &
           ...
           SUNCOS, DRZ, D50, D95, RS, SKYVIEW, SKYVIEWA, VTR, VTRA, &    
           FCD, ACROOT, WRMAX, N  )
!
        use svs_configs
      implicit none
...
!
      ...
      REAL WFC(N,NL_SVS), RS(N), SKYVIEW(N), SKYVIEWA(N), VTR(N), VTRA(N), DRZ(N)
...
! SKYVIEWA Sky view factor based on LAIVA (av. LAI)
