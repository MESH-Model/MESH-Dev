...
!     associated subroutines. To compile the program use the following 
!     commands:
!     1) make clean - This is optional. It cleans existing objects 
!                     and the fst2r2c executable.
!     2) make       - builds the fst2r2c executable. 

!     Or, for the new profile:
!     s.compile -src fst2r2c.f90 -o ../bin/fst2r2c -librmn rmn_014a ; rm fst2r2c.o
!***********************************************************************
...
!     http://collaboration.cmc.ec.gc.ca/science/si/eng/si/libraries/rmnlib/ezscint/functions.html
      ...
      integer, external :: gdllsval  !Interpolation at arbitrary locations 
      integer, external :: gdrls     !Free memory obtained using ezqkdef
      integer, external :: gdll      !Returns lat-lon for L grid 
...
!     Keywords for the R2C header file
      ...
      real*8       xOrigin           !the x-coordinate of the point in the bottom left corner of the grid
      real*8       yOrigin           !the y-coordinate of the point in the bottom left corner of the grid
      real*8       xDelta            !distance between two adjacent points in a row
      real*8       yDelta            !distance between two adjacent points in a column
...
!     Arguements used by the FST2R2C program
      ...
      real*8        cfactora         !Additive - Conversion factor to extract the R2C records with a
                                     ...
                                     !required to have the same unit than the corresponding FST record.
      character*4   varName          !Name of the FST record to be extracted
      ...
      character*10  degre_interp     !Interpolation type

!     Local variables
      integer       i,j,ier,iunfst,iunr2c,ifh,ni,nj,nk,n
      integer       key,nifst,njfst,nkfst 
      integer       gdfst,gdr2c
      logical       I_EXIST
      character*100 arg              
      real*8        xOrigin360       !xOrigin from 0 to 360

      real, dimension(:,:), allocatable ::fldr2c,fldfst,latr2c,lonr2c
    
!     get command line arguments
      if(iargc() .ne. 12)then
         print*,'ERROR: 12 arguments needed: '
         print*,'1 - FST file name (Input file)',                      &
                '2 - FST variable name to be extracted ',              &
                '3 - Forecast starting hour',                          &
                '4 - Forecast ending hour',                            &
                '5 - R2C file name (output file)',                     &
                '6 - Conversion factor - Multiplier',                  &
                ...
                '11 - Interpolation type (NEAREST|LINEAIR|CUBIC)'     ,&
                '12 - Variable level'
         call exit(1)
      else
...
!     Allocate R2C 2D field data and lat lon
      allocate (fldr2c(xCount,yCount))
      allocate (latr2c(xCount,yCount))
      allocate (lonr2c(xCount,yCount))

!     Compute lat-lon of each R2C grid point
      do i = 1, xCount
         do j = 1, yCount
!           WRONG BUT THIS IS WHAT THE VANILLA VERSION DID
!           lat-lon need to be shifted by half a grid cell and NOT ROUNDED
!            latr2c(i,j) = INT(yOrigin*100)/100. + (j-1)*INT(yDelta*100)/100.
!            lonr2c(i,j) = INT((360+xOrigin)*100)/100. + (i-1)*INT(xDelta*100)/100.
!           CORRECT: NO ROUNDING, (xOrigin,yOrigin) is lower-left corner of grid,
!           not location of lower-left pixel
            if (xOrigin .LT. 0) then
               xOrigin360 = xOrigin + 360.
            else
               xOrigin360 = xOrigin
            endif
            latr2c(i,j) = yOrigin + yDelta/2. + (j-1)*yDelta
            lonr2c(i,j) = xOrigin360 + xDelta/2. + (i-1)*xDelta
         enddo
      enddo
...
!        Search for a record of nomvar = varName and ip2 = ifh
         key = fstinf(iunfst,ni,nj,nk,-1,' ',ip1lev,ifh,-1,'',trim(varName))

!        Extract the attributes if the record exists
         if(key .gt. 0)then
            ier = fstprm(key,dateo,deet,npas,nifst,njfst,nkfst,nbits,  &
...
!            Obtain lat-lon for the L grid
!            Notice how they are truncated at 2 decimal places
!             ier = gdll(gdr2c,latr2c,lonr2c)
             print*,"Latitude of grid points:"
!             do j=1,yCount
!               write(*,*)(latr2c(i,j),i=1,xCount)
!             enddo
             write(*,*)latr2c(1,1),latr2c(xCount,yCount)
             print*,"Longitude of grid points:"
!             do j=1,yCount
!               write(*,*)(lonr2c(i,j),i=1,xCount)
!             enddo
             write(*,*)lonr2c(1,1),lonr2c(xCount,yCount)
!            Define a set of grids for interpolation
!             ier    = ezdefset(gdr2c, gdfst)

!            Set interpolation option
             ier    = ezsetopt('INTERP_DEGREE',degre_interp)

!            Scalar interpolation into the R2C grid
!             ier    = ezsint(fldr2c, fldfst)
             n = xCount*yCount
             ier = gdllsval(gdfst,fldr2c,fldfst,latr2c,lonr2c,n)

!            Deallocate FST field, we won't need it
             deallocate(fldfst)
             ier = gdrls(gdfst)
...
!            write(iunr2c,'(A,A,A,A,A,A,A,A,A)'), &
             write(iunr2c,'(A,I8,I8,A,A,A,A,A,A)'), &
!               ':Frame var. name: ',trim(attrName),' unit: ',trim(attrUnit),' "',strymd,' ',strhms,'"'
...
!     Writes the header of R2C file
      ...
      character*3  varName
      character*70 attrName, attrUnit, fstName
      character*20 Projection,Ellipsoid
      integer      iunr2c,xCount,yCount
      real*8       xOrigin,yOrigin,xDelta,yDelta

      integer       varid,now(8)
      character*10  today(3)
    
!     Get today's data and time
      call date_and_time(today(1),today(2),today(3),now)
    
!     Write header
      write(iunr2c,10)now(3),now(2),now(1),now(5),now(6),now(7), &
                      varName,                                         &
                      Projection,                                      &
                      Ellipsoid,                                       &
                      xOrigin,                                         &
...
10    format('########################################',/              &
             ...
             ':CreationDate            ',                              &
             i2.2,'/',i2.2,'/',i4.4,x,i2.2,':',i2.2,':',i2.2,/         &
             '#',/                                                     &
             '#---------------------------------------',/              &
             '#',/                                                     &
             ':Name                    ',A,/                           &
...
!              Projection - 1st grid parameter
               if(keyword(1:keyLen) .eq. ':projection')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read Projection type'
                  endif
...
!              Ellipsoid - 2nd grid parameter
               elseif(keyword(1:keyLen) .eq. ':ellipsoid')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read Ellipsoid type'
                  endif
...
!              xOrigin - 3rd grid parameter
               elseif(keyword(1:keyLen) .eq. ':xorigin')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read xOrigin value'
                  endif
...
!              yOrigin - 4th grid parameter
               elseif(keyword(1:keyLen) .eq. ':yorigin')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read yOrigin value'
                  endif
...
!              xCount - 5th grid parameter
               elseif(keyword(1:keyLen) .eq. ':xcount')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read xCount value'
                  endif
...
!              yCount - 6th grid parameter
               elseif(keyword(1:keyLen) .eq. ':ycount')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read yCount value'
                  endif
...
!              xDelta - 7th grid parameter
               elseif(keyword(1:keyLen) .eq. ':xdelta')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read xDelta value'
                  endif
...
!              yDelta - 8th grid parameter
               elseif(keyword(1:keyLen) .eq. ':ydelta')then
                  ...
                  if(wordCount == 1)then
                     ...
                     print*,'Could not read yDelta value'
                  endif
               endif
            endif
         endif
