      SUBROUTINE CLASSW(THLIQ,  THICE,  TBAR,   TCAN,   rncan,   SNCAN,
...
C     * MAR 14/05 - D.VERSEGHY. RENAME sncan TO SNCAN (RESERVED NAME
C     *                         IN F90).
C     * NOV 04/04 - D.VERSEGHY. ADD "IMPLICIT NONE" COMMAND.
C     * JUL 08/04 - D.VERSEGHY. NEW LOWER LIMITS FOR rncan, sncan, ZPOND
C     *                         AND SNOW.
...
C                                                                                  
     ...
     1     GFLUX (ILG,IG)
C
      REAL TCAN  (ILG),    rncan  (ILG),    SNCAN (ILG),    RUNOFF(ILG),
...
C
     ...
     C     trnfC(ILG),   trnfG(ILG),   TRNFCS(ILG),   TRNFGS(ILG),
...
C
     ...
     1     DIDRN (ILG,IG), DIDRNMX(ILG,IG),  dden(ILG),
     2     MANNING_N(ILG)
C
...
C
     ...
     B           trnfC, trnfG, TRNFCS, TRNFGS, TBASC,  TBASG,  
     ...
     V           BI, PSISAT, dden, XSLOPE, BULK_FC,THLMIN  )
C
...
!Craig Thompson added call to watrof, june 2008.
     ...
     2               ZPLMCS,XSLOPE,XDRAINH,MANNING_N,dden,GRKSCS,TBRWCS,
...
!Craig Thompson added call to watrof, june 2008.
     ...
     2               ZPLMGS,XSLOPE,XDRAINH,MANNING_N,dden,GRKSGS,TBRWGS,
     3               DELZW,THPOR,THLMIN,BI,DODRN,DOVER,DIDRN,
     4               ISAND,IWF,IG,ILG,IL1,IL2,BULK_FC,
     6               NA,NTYPE,ILMOS,JLMOS)
          ENDIF
...
C
     ...
     1                HTCS,WLOSTC,TRNFC,RUNFC,TOVRFL,OVRFLW,
     ...
     1                trnfC,ZFAV,LZFAV,THLINV,QFG,WLOSTC,
     ...
     2                   OVRFLW,   TOVRFL, RUNFC, trnfC,  TFREZ,
     3                   CMIN,     CMAX,   B,      K1,      K2,
...
!Craig Thompson added call to watrof, june 2008.
     ...
     1                SUBFLW,TSUBFL,BASFLW,TBASFL,RUNFC,TRNFC,FC,
     2                ZPLIMC,XSLOPE,XDRAINH,MANNING_N,dden,GRKSC,TBARWC,
     3                DELZW,THPOR,THLMIN,BI,DODRN,DOVER,DIDRN,
     4                ISAND,IWF,IG,ILG,IL1,IL2,BULK_FC,
     6                NA,NTYPE,ILMOS,JLMOS)
          ENDIF
     ...
     2                OVRFLW,TOVRFL,RUNFC,TRNFC,HMFG,HTC,HTCS,
...
C
     ...
     1                HTCS,WLOSTG,trnfG,RUNFG,TOVRFL,OVRFLW,
     ...
     2                    RUNFG,trnfG,OVRFLW,TOVRFL,ZPLIMG,GGEO,
     ...
     1                trnfG,ZFAV,LZFAV,THLINV,QFG,WLOSTG,
     ...
     2                   OVRFLW,   TOVRFL, RUNFG, trnfG,  TFREZ,
     3                   CMIN,     CMAX,   B,      K1,      K2,
...
!Craig Thompson added call to watrof, june 2008.
     ...
     1                SUBFLW,TSUBFL,BASFLW,TBASG,RUNFG,TRNFG,FG,
     2                ZPLIMG,XSLOPE,XDRAINH,MANNING_N,dden,GRKSG,TBARWG,
     3                DELZW,THPOR,THLMIN,BI,DODRN,DOVER,DIDRN,
     4                ISAND,IWF,IG,ILG,IL1,IL2,BULK_FC,
     6                NA,NTYPE,ILMOS,JLMOS)
          ENDIF
     ...
     2                OVRFLW,TOVRFL,RUNFG,trnfG,HMFG,HTC,HTCS,
...
C
     ...
     3                   FC (I)*RUNFC (I)*trnfC(I) + 
     4                   FG (I)*RUNFG (I)*trnfG(I))/RUNOFF(I) 
     ...
     1                CHCAP(I))/(FCS(I)*CHCAPS(I)+FC(I)*CHCAP(I))                 
              rncan(I)= FCS(I)*RAICNS(I) + FC (I)*RAICAN(I)                            
              IF(TCAN(I).LT.173.16 .OR. TCAN(I).GT.373.16) JPTBAD=I
              IF(rncan(I).LT.0.0) rncan(I)=0.0
              IF(rncan(I).LT.1.0E-5 .AND. rncan(I).GT.0.0) THEN
                  TOVRFL(I)=(TOVRFL(I)*OVRFLW(I)+TCAN(I)*rncan(I)/
     1                DELT)/(OVRFLW(I)+rncan(I)/DELT)
                  OVRFLW(I)=OVRFLW(I)+rncan(I)/DELT
                  TRUNOF(I)=(TRUNOF(I)*RUNOFF(I)+TCAN(I)*rncan(I)/
     1                DELT)/(RUNOFF(I)+rncan(I)/DELT)
                  RUNOFF(I)=RUNOFF(I)+rncan(I)/DELT
                  ROFC(I)=ROFC(I)+rncan(I)/DELT
                  ROVG(I)=ROVG(I)+rncan(I)/DELT
                  PCPG(I)=PCPG(I)+rncan(I)/DELT
                  HTCC(I)=HTCC(I)-TCAN(I)*SPHW*rncan(I)/DELT
                  rncan(I)=0.0
              ENDIF
