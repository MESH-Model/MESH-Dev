...
!     rev. 9.?.??  Jul.  30/08  - AS: revised timer to calculate correct day/month/year
...
!     SAVES THE LOCAL VARIABLES FROM ONE RUN TO NEXT
      SAVE
        ...
        CHARACTER(128) :: qstr
        INTEGER        :: result1,ntest,mohours(24),ju_mon(12),
     *                    ju_mon_l(12),m,iz,ik,jz,mz,ju,i,nchr
        real(4)        :: time,t1
     ...
     *             744,672,744,720,744,720,744,744,720,744,720,744/
      ...
      DATA ju_mon_l/1, 32, 61, 92,122,153,183,214,245,275,306,336/
...
!          reset the julian day on Jan 1 each year
	     if(leapflg.eq.'0')then
	       if(ju.gt.365)then
	         ju=1
!     rev. 9.?.??  Jul.  30/08  - AS: increment the year and set leapflg as appropriate
	         year1=year1+1
	         if(mod(year1, 400) == 0)then
	           leapflg='1'
	         elseif(mod(year1,100) == 0)then
	         elseif(mod(year1,4) == 0)then
	           leapflg='1'
	         endif
	       endif
	     else
...
!        FIND OUT WHAT MONTH WE ARE IN:
         if (leapflg == '0') then !not a leap year, use ju_mon
           do mo=12,1,-1
...
1000       month_now=mo
         ...
         else                     !leap year, use ju_mon_l
           do mo=12,1,-1
             if(ju.ge.ju_mon_l(mo))then
               go to 1001
             endif
           end do
1001       month_now=mo
           day_now=ju-ju_mon_l(mo)+1
         endif
