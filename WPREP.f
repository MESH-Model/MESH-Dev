     ...
     U                 BI, PSISAT, dden, XSLOPE, BULK_FC,THLMIN  )
C
...
C
     ...
     2     BI(ILG,IG),PSISAT(ILG,IG),dden(ILG),XSLOPE(ILG),BULK_FC(ILG,IG)
C
...
C     * BULKFC WORK VARIABLES
      REAL    xreliefi,xlengthi,xbasei,xanglei,THLMIN(ILG,IG)
C-----------------------------------------------------------------------
...
c         calculate BULK_FC, make sure we don't divide by 0 (for slope = 0 or bedrock i.e. %sand = -2)
     ...
     1                      (PSISAT(I,J)*BI(I,J)/xreliefi)**(1/BI(I,J))*
     +         ((2*dden(I)*PSISAT(I,J)*BI(I,J)/XSLOPE(I))**(1/BI(I,J)))*
     +                            ((3*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J))-
     +                             (2*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J)))
              ELSEif (Delzw(i,j).gt.0.0) then
                  BULK_FC(I,J) =               (THPOR(i,j)/(BI(I,J)-1))*
     1                    (PSISAT(I,J)*BI(I,J)/DELZW(I,J))**(1/BI(I,J))*
     +                            ((3*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J))-
     +                             (2*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J)))
                  else
c                    bulk_fc(i,j)  = thfc(i,j)
c                  primt*,"****erreor: bi<1 *****""thfc(i,j)=",thfc(i,j)
                  pause
              ENDIF
          ELSE
              BULK_FC(i,j) = thpor(i,j)
c              print*,"****erreor: bi<1 *****"
c              pause
          ENDIF
              BULK_FC(I,J)=MAX(THLMIN(I,J),MIN(THPOR(I,J),BULK_FC(I,J)))
          
CCCCCC          IF(XSLOPE(I).gt.0.0.AND.THPOR(I,J).gt.0.0) THEN
CCCCCC              BULK_FC(I,J)= (THPOR(I,J)/(BI(I,J)-1))*
CCCCCC     +          ((2*dden(I)*PSISAT(I,J)*BI(I,J)/XSLOPE(I))**(1/BI(I,J)))*
CCCCCC     +          ((3*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J))-
CCCCCC     +           (2*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J)))
CCCCCC          ELSE
CCCCCCc         make sure that BULK_FC is high to shut off interflow
CCCCCC              BULK_FC(I,J) = 2.0
CCCCCC          ENDIF
