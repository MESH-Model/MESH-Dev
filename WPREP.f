...
C     *                         WHEN PRECuIPITATION IS OCCURRING.
...
C     *                         ksat FOR TEMPERATURE AND PRESENCE 
C     *                         OF ICE.
...
C
      REAL THPOR (ILG,IG),HCPS  (ILG,IG),ksat(ILG,IG),
     1     DELZZ (ILG,IG),DELZW (ILG,IG),DELZ  (IG), 
     2     BI(ILG,IG),PSISAT(ILG,IG),DDEN(ILG),XSLOPE(ILG),
     3     bulkfc(ILG,IG),satfc(ilg,ig)
C
...
C
     ...
     2     SPHAIR,RHOW,RHOICE,TCGLAC,CLHMLT,CLHVAP,satmx,satmn
C
...
c         calculate bulkfc, make sure we don't divide by 0 (for slope = 0 or bedrock i.e. %sand = -2)
     ...
     +        (2*BI(I,J)+2)**((BI(I,J)-1)/BI(I,J)))
            satmx=1.0-2.0/(2*bi(i,j)+3)
            satmn=0.04
      satfc(i,j)
     1      =max(satmn,min(satmx,bulkfc(i,j)/thpor(i,j)))/(2*dden(i))
          ELSE
c         make sure that bulkfc is high to shut off interflow
            satfc(i,j)=1.0-2.0/(2*bi(i,j)+3)
          ENDIF
          ...
          DO 200 I=IL1,IL2
              IF(FCS(I).GT.0.)                                THEN 
                  ...
                  IF(THPOR(I,J).GT.0.0001)               THEN
                      GRKSCS(I,J)=ksat(I,J)*(1.7915E-03/
...
!$omp parallel do
          ...
          DO 300 I=IL1,IL2
              IF(FGS(I).GT.0.)                                 THEN 
                  ...
                  HCPGS (I,J)=HCPG  (I,J)
                  IF(THPOR(I,J).GT.0.0001)               THEN
                      GRKSGS(I,J)=ksat(I,J)*(1.7915E-03/
     1                    (2.0319E-04+1.5883E-03*EXP(-((MAX(0.0,
