...
C
     ...
     2     VPD   (ILG),   TA    (ILG),   ALVSGC(ILG),   ALIRGC(ILG),
     3     ALVSSC(ILG),   ALIRSC(ILG)
C
C     * OTHER DATA ARRAYS.
C
...
C
      REAL FACT,ALVSCX,ALIRCX,ALVSN,ALIRN,ALVSS,ALIRS,
     1     TRTOT,FRMAX
C
C     * COMMON BLOCK AND OTHER PARAMETERS.
C
      REAL DELT,TFREZ,ALVSWG,ALIRWG,ALVSWS,ALIRWS,ALVSWC,ALIRWC,ALAVG,
     1     ALCLR,ALTOT,TRCLRS,TRCLDS,TRCLRT,TRCLDT  
C                                                                                  
...
C
     ...
     1               AIL(I,J)/0.7071)+0.20*EXP(-0.3*AIL(I,J)/0.2588)   
              TRVS(I)=FCLOUD(I)*TRCLDS+(1.0-FCLOUD(I))*TRCLRS
              CXTEFF(I,J)=-LOG(TRVS(I))/MAX(AIL(I,J),1.0E-5)
      ...
      DO 200 I=IL1,IL2                                               
          IF(COSZS(I).GT.0. .AND. FCAN(I,J).GT.0.)               THEN
              ...
              IF(IALC.EQ.0) THEN
                  ...
                  ALVSN=(1.0-FACT)*ALVSCX+FACT*TRVS(I)*ALVSGC(I)
                  ALIRN=(1.0-FACT)*ALIRCX+FACT*TRIR(I)*ALIRGC(I)
              ELSE
                  ALVSCX=FSNOWC(I)*ALVSWC+(1.0-FSNOWC(I))*ACVDAT(I,J)
                  ALIRCX=FSNOWC(I)*ALIRWC+(1.0-FSNOWC(I))*ACIDAT(I,J)
                  ALVSN=(1.0-FACT)*ALVSCX+FACT*ACVDAT(I,J)
                  ALIRN=(1.0-FACT)*ALIRCX+FACT*ACIDAT(I,J)
              ENDIF
...
C
      DO 550 I=IL1,IL2                                               
          IF(COSZS(I).GT.0. .AND. FCANS(I,J).GT.0.)             THEN
              ...
              ALVSS=(1.0-FACT)*ALVSCX+FACT*TRVS(I)*ALVSSC(I)
              ALIRS=(1.0-FACT)*ALIRCX+FACT*TRIR(I)*ALIRSC(I)
...
925           CONTINUE
              ...
              ELSE                                                                
                  RCS(I)=5000.0
                  IF(RC(I).GT.0.0) RC(I)= 2./((FCS(I)+FC(I))/RC(I)+
     1                GC(I)/(FCS(I)+FC(I)))
              ENDIF                                                               
          ELSE
              RC(I)=0.0
