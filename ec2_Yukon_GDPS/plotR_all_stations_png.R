...
Rplots_path <- file.path("/home/ec2-user/Yukon/Streamflow/Plots", Date_Today)
dir.create( Rplots_path )

#Reading the names of stations and associated river.
stations <- read.csv ("/home/ec2-user/Yukon/scripts/stations.txt", header=FALSE, sep=",")
stations
...
o <- file.path("/home/ec2-user/Yukon/Streamflow", stations[i,1], "Streamflow_Observations.csv")
obs <- read.csv (o, header=FALSE, sep=" ")
date1 <- paste(obs[, 1], obs[, 2])
date2 <- as.POSIXlt(date1)
flow1 <- obs[,3]
fig_obs <- data.frame(date2, flow1)

##Prepare to plot model values##
m <- file.path("/home/ec2-user/Yukon/Streamflow", stations[i,1], "MESH_output_streamflow_ts.csv")
...
image_path <- file.path("/home/ec2-user/Yukon/Streamflow/Plots", Date_Today, name)
png(image_path, width = 600, height = 400 )
#Plotting model values
par(mar=c(5.1,4.5,4.1,2.1))
plot(fig_mesh[,1],mesh_nudged, type="l",lwd=c(2), ylim=c(0, maxy), xlab = "Date and Time (PST)", ylab=expression ('Streamflow '('m'^3*'s'^-1)), main=paste("Streamflow Forecast for", stations[i,2], "\nStation", stations[i,1], sep=" "), cex=0.5, col="red", xaxt="n", yaxt="n")
...
text(x=xrange,  par("usr")[3], 
     labels = strftime(xrange, format="%d %m %Y %H:%M"), adj = c(0.5,2.8), xpd = TRUE, cex=0.9) #instead of adj you can use pos = 1
...
legend(x="bottomleft", inset=0.05, c("Forecast","Measured"), col=c("red","blue"), lwd=c(2), cex=1.0)
...
Source_path_1 <- file.path("/home/ec2-user/Yukon/Streamflow",  stations[i,1], "Streamflow_Observations.csv")
Archive_path_1 <- file.path("/home/ec2-user/Yukon/Streamflow", stations[i,1], "Archive", file_name_1)
file.rename(Source_path_1, Archive_path_1)

#Archiving Mesh streamflow output file.
file_name_2 <- paste('MESH_output_streamflow_ts', stations[i,1], '_', Date_Today, '.csv', sep='')
Source_path_2 <- file.path("/home/ec2-user/Yukon/Streamflow",  stations[i,1], "MESH_output_streamflow_ts.csv")
Archive_path_2 <- file.path("/home/ec2-user/Yukon/Streamflow",  stations[i,1], "Archive", file_name_2)
file.rename(Source_path_2, Archive_path_2)
...
Source_path_3 <- file.path("/home/ec2-user/Yukon/Streamflow",  stations[i,1], file_name_3)
Archive_path_3 <- file.path("/home/ec2-user/Yukon/Streamflow",  stations[i,1], "Archive", file_name_4)
