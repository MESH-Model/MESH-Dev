module model_output
    ...
    !* rncan: Rainfall intercepted by the canopy [kg m-2]
...
!        procedure :: deallocate => deallocate_water_balance
    ...
    type out_bal_intg
        ...
        !real, dimension(:)  ,allocatable :: TOTAL_ZPND  , TOTAL_rncan , TOTAL_sncan
...
!    end type
    ...
    type out_flds
        ...
        !real, dimension(:,:), allocatable :: rncan_y, rncan_m, rncan_s ! Rainfall intercepted by the canopy
        !real, dimension(:,:), allocatable :: sncan_y, sncan_m, sncan_s ! Snowfall intercepted by the canopy
...
!    end subroutine !info_out_allocate_var_out
    ...
    subroutine init_out_flds(vr, bi, ts)
        ...
        call vr%engt_h%init(bi, max(1, 3600/public_ic%timestep))        
    ...
    subroutine init_water_balance_series(wbt, bi, nts)
        ...
        allocate( &
            ...
            wbt%rncan(nts, bi%na), wbt%sncan(nts, bi%na), &
...
!        if (allocated(wb%rncan)) deallocate(wb%rncan)
...
!    end subroutine Init_Internal_resp
    ...
    subroutine Init_OutBal_Intg(bal, ts, ignd, area)
        ...
        allocate(bal%wb%pre(ts%nr_days), bal%wb%evap(ts%nr_days), &
                 ...
                 bal%wb%rncan(ts%nr_days), bal%wb%sncan(ts%nr_days), &
    ...
    subroutine Update_OutBal_Intg(bal, ts, ignd, &
        ...
        real, intent(in) :: pre, evap, rof, pndw, rncan, sncan
        ...
        bal%wb%rncan(idate) = bal%wb%rncan(idate) + rncan
        if (isavg) &
            bal%wb%rncan(idate) = bal%wb%rncan(idate)/real(nhours)
    ...
    subroutine init_out(vr, ts, ifo, bi)
        ...
        call public_ic%init(ts%start_date(1) , &
                            ts%start_date(2) , &
                            ts%start_date(3) , &
                            ts%start_date(4) )
        ...
        do i = 1, ifo%nr_out
            select case (trim(adjustl(ifo%var_out(i)%name)))
                ...
                case ("rncan")
                    if (ifo%var_out(i)%out_h) then
                        freq = "H"
                        call check_write_var_out(ifo, i, vr%wbt_h%rncan, freq, public_ic%now_hour, now_hour, 882114, .true.)
                        vr%wbt_h%rncan((now_timestep/public_ic%timestep + 1), :) = wb%rncan
                    end if
...
!            vId = trim(adjustl(ifo%ids_var_out(i, 1)))
            ...
            select case (vId)
                ...
                case ('rncan')
                    ...
                    if (ifo%var_out(i)%out_y) & !trim(adjustl(ifo%ids_var_out(i, 2))) == 'Y') &
                        vr%wbt_y%rncan(iy, :) = vr%wbt_y%rncan(iy, :) + rncan

                    if (ifo%var_out(i)%out_m) & !trim(adjustl(ifo%ids_var_out(i, 3))) == 'M') &
                        vr%wbt_m%rncan(im, :) = vr%wbt_m%rncan(im, :) + rncan

                    if (ifo%var_out(i)%out_s) & !trim(adjustl(ifo%ids_var_out(i, 4))) == 'S') &
                        vr%wbt_s%rncan(iss, :) = vr%wbt_s%rncan(iss, :) + rncan
            ...
            select case (vId)
             ...
             case ('THIC')
                ...
                case ('rncan')
                    ...
                    if (ifo%var_out(i)%out_h) then
                        freq = "H"
                        call check_write_var_out(ifo, i, vr%wbt_h%rncan, freq, public_ic%now_hour - 1, public_ic%now_hour, &
                            882114, .false.)
                    end if
...
!        tfunc = trim(adjustl(ifo%ids_var_out(indx, 5)))
        ...
        select case (trim(adjustl(ifo%var_out(indx)%name)))
            ...
            case ('rncan')
                ...
                if (trim(adjustl(freq)) == 'Y') then
                    do i = 1, nt
                        fld(:, i) = vr%wbt_y%rncan(i, :)
                    end do
                end if

                if (trim(adjustl(freq)) == 'M') then
                    do i = 1, nt
                        fld(:, i) = vr%wbt_m%rncan(i, :)
                    end do
                end if

                if (trim(adjustl(freq)) == 'S') then
                    do i = 1, nt
                        fld(:, i) = vr%wbt_s%rncan(i, :)
                    end do
                end if
