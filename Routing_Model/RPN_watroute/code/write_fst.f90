...
subroutine write_fst(unitNum,infname,var_name,iyear,imonth,iday,ihour)
   ...
   character flname*60! Filename for output
   ...
   integer j ! Index
   real     latgrid(ycount),  & ! Latitude grid
            longrid(xcount),  & ! Longtitude grid
   ...
   if (key > 0) then
      ...
      do j=1,ycount,1
         latgrid(j) = yorigin + ydelta*(j-0.5)
      end do
      do j=1,xcount,1
         longrid(j) = xorigin + xdelta*(j-0.5)
      end do
      ...
      ! Write the grids
      ier = fstecr(latgrid,work,-32,unitNum,dateo, &
      ...
      ier = fstecr(longrid,work,-32,unitNum,dateo, &
                   0,0, & ! deet = 0, 0 timesteps taken
