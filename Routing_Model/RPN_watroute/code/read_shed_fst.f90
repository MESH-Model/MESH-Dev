...
subroutine read_shed_fst(unitNum, iflname)
   ...
   character flname*99   ! Internal variable for de-padded iflname
   ...
   real, allocatable :: & ! Local variables that need to be allocated
         latgrid(:), longrid(:), & ! Latitude and longtitude
   ...
   ! And that is all the lat/lon grids were needed for.  If IC/recharge/
   ! runoff interpolation is implemented, then it will be worthwhile to
   ! export these to the remainder of the code.
   ...
   call readit('chnl')
   allocate(ichnl(na))
   forall(ii=1:na) ichnl(ii) = int(in_array(xxx(ii),yyy(ii)))
   ...
   allocate(flz(na),flz2(na), pwr(na),r1n(na),r2n(na),mndr(na),aa2(na), &
           aa3(na), aa4(na), theta(na), widep(na), kcond(na))
   ...
   allocate(qi1(na), qi2(na), qo1(na), qo2(na), qold2(na), &
...
contains 
   ...
   ! Helper subroutine to read in a floating-point variable from
