...
!        integer j, i
        ...
        if (allocated(xxx)) then
            deallocate(xxx, yyy, s, &
                       next, da, bnkfll, slope, &
!                       elev
                       rl, ibn, &
!                       sl1, sl2
                       ichnl, ireach, &
                       grid_area, frac, aclass, &
                       glacier_flag)
        end if
...
!+        sl1 = shd%SLOPE_INT
        ...
        if (allocated(r1n)) then
            deallocate(r1n, r2n, rlake, &
                       mndr, aa2, aa3, aa4, theta, widep, kcond)
        end if
        ...
        if (allocated(qi1)) then
            deallocate(qi1, qi2, qo1, qo2, qo2sim, &
                       qr, d2, qda, cap, over, &
                       qmax, res, sump, store1, &
                       store2, att, qbase, nreach, &
                       totd1, totuzs, totsnw, qstream, &
                       totchnl, totgrid, netflow, storinit, &
                       sumrechrg, sumrff, rechrg, &
                       qdrng, qdrngfs, qstrm, sumq1, &
                       sumqint, sumq1fs, sumqintfs, strloss, &
                       qdrng2, qdrngfs2, wetwid, chawid, &
                       chadep, wstore1, wstore2, wcap, &
                       flowxa, chaxa, satxa, wetxa, &
                       hcha1, hcha2, hwet1, hwet2, qin, &
                       qswevp, qswrain, qiwet1, qiwet2, &
                       qowet1, qowet2, wetarea, chaarea, &
                       bin_precip, wsat, wetfrac, qo2rem, &
!tied to irr
                       qo2remirr)
        end if
...
!tied to irr
        ...
        if (allocated(store2_strt)) deallocate(store2_strt)
...
!            end where
            ...
            if (allocated(reach_last)) deallocate(reach_last)
        ...
        if (fms%rsvr%n > 0 .or. fms%stmg%n > 0) then
            ...
            if (fms%stmg%n > 0) then
                if (allocated(iy)) then
                    deallocate(iy, jx, nhyd, nlow, nxtbasin, area)
                end if
...
!temp: Override for diversions.
!        nodiv = 4
!        if (allocated(val1div)) then
!            deallocate( &
!                val1div, val2div, val3div, val4div, &
!                divstrindex, divendindex, divname, qdiv, qdiv2)
!        end if
!        allocate( &
