...
C Search for and read r2c file header
     ...
     &		(len_trim(line) .eq. 0))) 	
			...
			if(ios .eq. -1)then
				print*, 'ERROR: Premature EndOfFile encountered line 119'
				STOP ' Stopped in read_r2c'
			end if
...
c      write(654,*)line
			if(ios.gt.0)then
                    write(6,'((A))')
     *                'ERROR: Premature EndOfFile encountered line 195'
			  STOP ' Stopped in read_r2c'
...
!               data fits in the allocated memory
                if(xcount_temp.gt.xcount_max.or.ycount_temp.gt.
     *             ycount_max)then
...
cccccccccccc	if(modelHour.eq.dataHour)then
