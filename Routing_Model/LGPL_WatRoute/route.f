...
!       April 19, 2002 Commented this line AB
!        if(iopt.eq.2)write(55,8493)1,l,b1(l),b2(l),qda(n),store1(n)
...
!       But we must initilaize the storage only once so use id=1
...
!                 then use bisection to zero in on the flow
!               bisection not yet coded
!                    store1(n)=0.5*store1(n)                      
!                    try1=b1(l)*store1(n)+
!     *                   b2(l)*store1(n)**2+
!     *                   b3(l)*store1(n)**3+
!     *                   b4(l)*store1(n)**4+
!     *                   b5(l)*store1(n)**5
      ...
      endif           ! END OF firstpass CONDITIONAL

!      index=2
      indexx=2
!Craig Thompson renamed index to indexx to avoid variable name
! conflicts with mesh/class
...
c      do l=1,nresvo
c	  strloss_sum(l,jz)=0.0
c	  qstream_sum(l,jz)=0.0
c	end do
...
c            if(iz.eq.1)then
!             iz is used here as a flag
!	        this summation has to be done just once an hour
!             since qr & qlz stay the same for the hour.
...
!Craig Thompson changed this
!                  if(iopt.ge.3)write(55,6004)n,res(n),index
                  if(iopt.ge.3)write(55,6004)n,res(n),indexx
...
!     rev. 9.1.43  Jun.  01/03  - Fixed the qdwpr.txt function - c!     rev. 9.1.43  Jun.  01/03  - Fixed the qdwpr.txt function - c
...
!               also,nudge only if there is observed flow 
                if(jz.ge.kt.and.qhyd(l,jz).ge.0.0)then
                  flowsta='y'
                  lsta=l
                endif
              endif
...
! 6004 format(' gone to rerout - n,res(n),index/',3i5)
 6004 format(' gone to rerout - n,res(n),indexx/',3i5)
